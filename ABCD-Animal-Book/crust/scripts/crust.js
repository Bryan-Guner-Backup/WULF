...
function bookSlider($superbook) {
  ...
  var turnAndSlide = function (event, page, view) {
    isUserInteracting = true;
    var viewNumber = doubleDisplay
      ? parseInt((page || $superbook.turn("page")) / 2 + 1, 10)
      : page;
...
$(document).ready(function () {
    ...
    mutations.forEach(function (mutation) {
      if (mutation.type === "childList") {
        if (
          ...
          mutation.addedNodes[0].className === "page-wrapper"
        ) {
  ...
  function setEventsInIframe(thisIframe) {
    thisIframe
      .contents()
      .unbind(
        "tap doubletap mouseover vmouseover mouseout vmouseout pinch mouseup vmouseup mousemove vmousemove swipe mousedown vmousedown drag touchstart touchmove touchend dragstart dragend dragover"
      );
    ...
    if (Turn.isTouchDevice) {
      thisIframe
        .contents()
        .bind(
          "vmousedown vmouseover vmouseout vmouseup vmousemove",
          function (e) {
      ...
      thisIframe
        .contents()
        .bind(
          "mouseover vmouseover mouseout vmouseout mouseup vmouseup mousemove vmousemove",
          function (e) {
            e.pageX += thisIframe.offset().left;
            e.pageY += thisIframe.offset().top;
            $(document).trigger(e);
    ...
    $(".turnoff", $("iframe").contents()).on(
      "touchend doubletap mouseover vmouseover mouseout vmouseout pinch mouseup vmouseup mousemove vmousemove swipe mousedown vmousedown drag touchstart touchmove dragstart dragend dragover",
      function (e) {
        e.stopPropagation();
      }
    );

    $("a", $("iframe").contents()).on(
      "touchend doubletap mouseover vmouseover mouseout vmouseout pinch mouseup vmouseup mousemove vmousemove swipe mousedown vmousedown drag touchstart touchmove dragstart dragend dragover",
      function (e) {
        return false;
      }
  ...
  key(
    "⌘ + left, ⌘ + pageup, ⌘ + up, ctrl + left, ctrl + pageup, ctrl + up",
    function (e) {
      e.preventDefault(e);
  ...
  key(
    "⌘ + right, ⌘ + pagedown, ⌘ + down, ctrl + right, ctrl + pagedown, ctrl + down",
    function (e) {
      e.preventDefault(e);
