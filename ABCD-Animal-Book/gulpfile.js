...
gulp.task("templates", function () {
  return gulp
    .src(path.join("templates", "*.+(js|css|html)"))
...
function renderPage(page) {
  var bodyPath = path.join(".", "build", "manuscript", page, "body.html"),
  ...
  if (fs.existsSync(templateStylePath)) {
    templateStyleContent = fs
      .readFileSync(templateStylePath, "utf-8")
      .toString();
  }
  ...
  gulp
    .src(path.join(".", "crust", "page-template.html"))
...
gulp.task("indexPage", function () {
  ...
  gulp
    .src(path.join(".", "crust", "index-template.html"))
    .pipe(handlebars(templateData, {}))
