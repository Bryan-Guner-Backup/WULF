      ...
      if (cmd === "enable" && args.length === 1) {
        ...
        if (args.length === 3) {
          if (process.env.ADDRESS === args[0]) {
            console.log(
              "[Error] It is not recommended for admins to add routes, adding routes are for service providers. If you want to act as a service provider, create a separate account (create a new folder and run 'bit init') and run 'route add' again."
            );
          } else {
          ...
          console.log(
            "Syntax: 'bit route add [BITCOM_ADDRESS] [ROUTE] [SERVICE_ENDPOINT]'"
          );
        }
      }
    }
