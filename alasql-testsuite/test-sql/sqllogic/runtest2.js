...
function test(filename, show) {
  ...
  for (var i = 0; i < Math.min(limit, f.length); i++) {
    ...
    if (w[0] == "query") {
      ...
      if (ast.statements[0].joins && ast.statements[0].joins.length > 3) {
        // We skip queries with joining more than 3 tables
        process.write("0");
        continue;
      }
      ...
      if (success) {
        ...
        if (res.length * qtype.length == explen) {
      ...
      if (passed != "passed" && show)
        console.log("#", i, sql, expect, res, reason);
