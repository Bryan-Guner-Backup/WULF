...
describe("amp-pixel", () => {
  ...
  it("replace $CANONICAL_URL", () => {
    return getPixel("https://foo.com?href=$CANONICAL_URL").then((p) => {
      expect(p.querySelector("img")).to.not.be.null;
      expect(p.children[0].src).to.equal(
  ...
  it("replace $CANONICAL_HOST", () => {
    return getPixel("https://foo.com?host=$CANONICAL_HOST").then((p) => {
      expect(p.querySelector("img")).to.not.be.null;
      expect(p.children[0].src).to.equal("https://foo.com/?host=pinterest.com");
    });
  });

  it("replace $CANONICAL_PATH", () => {
    return getPixel("https://foo.com?path=$CANONICAL_PATH").then((p) => {
      expect(p.querySelector("img")).to.not.be.null;
  ...
  it("replace $CANONICAL_TITLE", () => {
    return getPixel("https://foo.com?title=$TITLE").then((p) => {
      expect(p.querySelector("img")).to.not.be.null;
      expect(p.children[0].src).to.equal("https://foo.com/?title=Pixel%20Test");
    });
  });

  it("replace $AMPDOC_URL", () => {
    return getPixel("https://foo.com?ref=$AMPDOC_URL").then((p) => {
      expect(p.querySelector("img")).to.not.be.null;
  ...
  it("replace $AMPDOC_HOST", () => {
    return getPixel("https://foo.com?ref=$AMPDOC_HOST").then((p) => {
      expect(p.querySelector("img")).to.not.be.null;
      let query = parseQueryString(parseUrl(p.children[0].src).search);
  ...
  it("replace $UNKNOWN", () => {
    return getPixel("https://foo.com/?a=$UNKNOWN").then((p) => {
      expect(p.querySelector("img")).to.not.be.null;
