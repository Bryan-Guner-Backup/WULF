    ...
    var ordinalize = function (num, hint) {
      ...
      if (hint == "friction") {
        ...
        if (num < 11000) {
          ...
          norm =
            ("" + Math.floor(num / 1000)).replace(_ordinalize_re, "$1,") + "m";
        }
      }
      return norm;
    };
    ...
    var that = {
      ...
      beginFrobbing: function (e) {
        ...
        var prop =
          param == "friction"
            ? val / max
            : (Math.log(val / max) / Math.PI + Math.PI) / Math.PI;

        if (val / max > (param == "friction" ? 0.9 : 0.333))
          frob.addClass("huge");
        if (val / max <= (param == "friction" ? 0.4 : 0.05))
          frob.addClass("tiny");
      ...
      stillFrobbing: function (e) {
        ...
        if (new_prop > (_state.param == "friction" ? 0.9 : 0.333))
          _state.elt.addClass("huge");
        else _state.elt.removeClass("huge");

        if (new_prop <= (_state.param == "friction" ? 0.4 : 0.05))
          _state.elt.addClass("tiny");
