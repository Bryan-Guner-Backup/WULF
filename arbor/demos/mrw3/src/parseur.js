(function () {
    ...
    var yack = function (statements) {
      ...
      $.each(statements, function (i, st) {
        ...
        if (types.match(/ident-arrow-ident(-style)?/)) {
          ...
          if (nodes[edge.src] === undefined)
            nodes[edge.src] = $.isEmptyObject(nodestyle)
              ? -2600
              : objcopy(nodestyle);
          if (nodes[edge.dst] === undefined)
            nodes[edge.dst] = $.isEmptyObject(nodestyle)
              ? -2600
              : objcopy(nodestyle);
          ...
          if (st[1] && st[1].style) {
            ...
            nodes[node] = $.isEmptyObject(nodestyle)
              ? -2600
              : objcopy(nodestyle); // use defaults
          }
        } else if (types == "style") {
  ...
  var precompile = function (s) {
    ...
    replaceConfigOdered = replaceConfigOdered.sort(function (a, b) {
