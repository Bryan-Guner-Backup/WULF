...
var Physics = function (dt, stiffness, repulsion, friction, updateFn) {
  ...
  var that = {
    ...
    theta: 0.4, // the criterion value for the barnes-hut s/d calculation
    ...
    modifyPhysics: function (param) {
      $.each(
        ["stiffness", "repulsion", "friction", "gravity", "dt", "precision"],
        function (i, p) {
      ...
      $.each(active.particles, function (id1, point1) {
        $.each(active.particles, function (id2, point2) {
          if (point1 !== point2) {
            ...
            var direction = (
              d.magnitude() > 0 ? d : Point.random(1)
            ).normalize();
            ...
            point1.applyForce(
              direction
                .multiply(that.repulsion * (point2._m || point2.m) * 0.5)
                .divide(distance * distance * 0.5)
            );
            point2.applyForce(
              direction
                .multiply(that.repulsion * (point1._m || point1.m) * 0.5)
                .divide(distance * distance * -0.5)
            );
          }
        });
    ...
    updateVelocity: function (timestep) {
      ...
      $.each(active.particles, function (id, point) {
        ...
        point.v = point.v
          .add(point.f.multiply(timestep))
          .multiply(1 - that.friction);
