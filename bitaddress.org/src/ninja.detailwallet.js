ninja.wallets.detailwallet = {
  isOpen: function () {
    return (
      document.getElementById("detailwallet").className.indexOf("selected") !=
      -1
    );
  },
  ...
  viewDetails: function () {
    ...
    var bip38CommandDisplay = document.getElementById("detailbip38commands")
      .style.display;
    ...
    if (ninja.privateKey.isBIP38Format(key)) {
      document.getElementById("detailbip38commands").style.display =
        bip38CommandDisplay;
      ...
      ninja.privateKey.BIP38EncryptedKeyToByteArrayAsync(
        ...
        function (btcKeyOrError) {
          ...
          if (btcKeyOrError.message) {
            ...
            ninja.wallets.detailwallet.populateKeyDetails(
              new Bitcoin.ECKey(btcKeyOrError)
            );
          }
        }
      );
      ...
      if (btcKey.priv == null) {
        ...
        if (key.length >= ninja.wallets.brainwallet.minPassphraseLength) {
          ...
          var usePassphrase = confirm(
            ninja.translator.get("detailconfirmsha256")
          );
          if (usePassphrase) {
