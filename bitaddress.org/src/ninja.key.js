...
ninja.privateKey = {
  ...
  BIP38EncryptedKeyToByteArrayAsync: function (
    ...
    try {
      ...
      callback(
        new Error(ninja.translator.get("detailalertnotvalidprivatekey"))
      );
      return;
    }
    ...
    var verifyHashAndReturn = function () {
      ...
      if (
        ...
        callback(
          new Error(ninja.translator.get("bip38alertincorrectpassphrase"))
  ...
  BIP38GenerateECAddressAsync: function (intermediate, compressed, callback) {
    ...
    var generatedPoint = ec.decodePointHex(
      ninja.publicKey.getHexFromByteArray(passpoint)
    );
...
ninja.publicKey = {
  isPublicKeyHexFormat: function (key) {
    key = key.toString();
    return (
      ninja.publicKey.isUncompressedPublicKeyHexFormat(key) ||
