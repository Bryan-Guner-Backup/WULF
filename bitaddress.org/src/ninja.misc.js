(function (ninja) {
  var status = (ninja.status = (function () {
    ...
    var showCrypto = function () {
      document.getElementById("statuscrypto" + cryptoCase).style.display =
        "block";
    };

    var showProtocol = function () {
      document.getElementById("statusprotocol" + protocolCase).style.display =
        "block";
    };
  ...
  select: function (walletTab) {
    ...
    if (walletTab.className.indexOf("selected") == -1) {
      ...
      if (
        ...
        walletType == "detailwallet"
      ) {
        walletTab.className += " selected";
        document.getElementById("generate").style.display = "none";
        ninja.wallets[walletTab.getAttribute("id")].open();
      } else if (
        ninja.seeder.isStillSeeding == true &&
        !(walletType == "brainwallet" || walletType == "detailwallet")
      ) {
