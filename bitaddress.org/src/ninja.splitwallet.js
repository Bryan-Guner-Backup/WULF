ninja.wallets.splitwallet = {
  isOpen: function () {
    return (
      document.getElementById("splitwallet").className.indexOf("selected") != -1
    );
  },
  ...
  stripLeadZeros: function (hex) {
    return hex.split(/^0+/).slice(-1)[0];
  },
  ...
  splitKey: function () {
    try {
      ...
      var m = {};
      output.appendChild(
        this.mkOutputRow(bitcoinAddress, "split_addr", "Bitcoin Address:    ")
      );
      ...
      for (var i = 0; i < shares.length; i++) {
        var id = "split_qr_" + i;
        output.appendChild(
          this.mkOutputRow(shares[i], id, "Share " + (i + 1) + ":          ")
        );
        m[id] = shares[i];
      }
  ...
  combineShares: function () {
    try {
      ...
      var shares = document
        .getElementById("combineinput")
        .value.trim()
        .split(/\W+/);
      var combinedBytes =
        ninja.wallets.splitwallet.combineFormattedShares(shares);
      var privkeyBase58 = new Bitcoin.ECKey(
        combinedBytes
  ...
  getFormattedShares: function (key, numshares, threshold) {
    var shares = secrets
      .share(key, numshares, threshold)
      .map(ninja.wallets.splitwallet.hexToBytes)
      .map(Bitcoin.Base58.encode);
    return shares;
  },

  // combine base58 formatted shares and return a bitcoin byte array
  combineFormattedShares: function (shares) {
    var combined = secrets.combine(
      shares
  ...
  openCloseStep: function (num) {
    // do close
    if (
      document.getElementById("splitstep" + num + "area").style.display ==
      "block"
    ) {
      document.getElementById("splitstep" + num + "area").style.display =
        "none";
      document
        .getElementById("splitstep" + num + "icon")
