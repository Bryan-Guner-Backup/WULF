...
function Expression(node, context) {
  ...
  switch (node.type) {
    ...
    case "Var":
      ...
      assert(
        identifier in context.variableTypes,
...
function compileReturnExpression(node, context) {
  ...
  if (expr.isConstant && types.equal(expr.type, types.int)) {
    ...
    switch (expr.type.category) {
      case "int":
        /* expr|0 */
        return estree.BinaryExpression("|", expr.compile(), estree.Literal(0));
      default:
  ...
  switch (statement.type) {
    ...
    case "Return":
      ...
      return estree.ReturnStatement(
        compileReturnExpression(returnValue, context)
      );
    default:
...
function compileBlock(block, parentContext, returnBlockStatement) {
  ...
  for (i = 0; i < declarationList.length; i++) {
    ...
    var declarationType = types.getTypeFromDeclarationSpecifiers(
    ...
    for (j = 0; j < initDeclaratorList.length; j++) {
      ...
      if (initialValue === null) {
        ...
        if (types.equal(declarationType, types.int)) {
          ...
          throw (
            "Unsupported declaration type: " + util.inspect(declarationType)
          );
        }
      } else {
...
function FunctionDefinition(node) {
  ...
  this.returnType = types.getTypeFromDeclarationSpecifiers(
    declarationSpecifiers
  );
  ...
  if (!parameterListIsVoid(parameterList)) {
    for (var i = 0; i < parameterList.length; i++) {
      ...
      var parameterType = types.getTypeFromDeclarationSpecifiers(
        parameterDeclaration.params[0]
      );
...
FunctionDefinition.prototype.compile = function (parentContext) {
  ...
  for (var i = 0; i < this.parameters.length; i++) {
    ...
    switch (param.type.category) {
      ...
      default:
        throw (
          "Parameter type annotation not yet implemented: " +
          util.inspect(param.type)
        );
    }
  }
...
function compileModule(name, ast) {
  ...
  var moduleBody = [estree.ExpressionStatement(estree.Literal("use asm"))];
  ...
  moduleBody.push(
    estree.ReturnStatement(estree.ObjectExpression(exportsTable))
  );
