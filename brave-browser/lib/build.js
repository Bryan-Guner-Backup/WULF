...
const touchOverriddenFiles = () => {
  ...
  const applyFileFilter = (file) => {
    // Exclude test files
    if (file.indexOf("browsertest") > -1 || file.indexOf("unittest") > -1) {
      return false;
    }
  ...
  sourceFiles.forEach((chromiumSrcFile) => {
    ...
    if (fs.existsSync(overriddenFile)) {
      ...
      if (
        fs.statSync(chromiumSrcFile).mtimeMs -
          fs.statSync(overriddenFile).mtimeMs >
        0
      ) {
        const date = new Date();
