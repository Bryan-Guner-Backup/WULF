...
const Config = function () {
  ...
  this.srcDir = path.join(
    this.rootDir,
    getNPMConfig(["projects", "chrome", "dir"])
  );
  ...
  this.braveGoogleApiKey =
    getNPMConfig(["brave_google_api_key"]) ||
    "AIzaSyAQfxPJiounkhOjODEO5ZieffeBv6yft2Q";
  this.googleApiKey =
    getNPMConfig(["google_api_key"]) ||
    "AIzaSyAH90V94EcZBP5oH7oc-mXQrSKgASVxER8";
  this.googleApiEndpoint =
    getNPMConfig(["brave_google_api_endpoint"]) ||
    "https://www.googleapis.com/geolocation/v1/geolocate?key=";
  this.safeBrowsingApiEndpoint =
    getNPMConfig(["safe_browsing_api_endpoint"]) || "safebrowsing.brave.com";
  this.buildProjects();
  const braveCoreDirPackage = path.join(
    this.projects["brave-core"].dir,
    "package"
  );
  this.braveCoreVersion =
    getNPMConfig(["brave_version"]) ||
    (fs.existsSync(braveCoreDirPackage + ".json") &&
      require(braveCoreDirPackage)["version"]) ||
    "";
...
const getProjectVersion = function (projectName) {
  return (
    getNPMConfig(["projects", projectName, "tag"]) ||
...
Config.prototype.buildProjects = function () {
  ...
  this.projectNames.forEach((projectName) => {
    this.projects[projectName] = {
      ...
      dir: path.join(
        this.rootDir,
        getNPMConfig(["projects", projectName, "dir"])
      ),
...
Config.prototype.update = function (options) {
  ...
  if (options.channel !== "release") this.channel = options.channel;
  ...
  if (options.gclient_verbose) this.gClientVerbose = options.gclient_verbose;
...
Object.defineProperty(Config.prototype, "defaultOptions", {
  get: function () {
    ...
    env = this.addPythonPathToEnv(
      env,
      path.join(this.srcDir, "brave", "chromium_src", "python_modules")
    );
    env = this.addPythonPathToEnv(
      env,
    ...
    env = this.addPythonPathToEnv(
      env,
      path.join(this.srcDir, "tools", "grit", "grit", "extern")
    );
    env = this.addPythonPathToEnv(
      env,
      path.join(this.srcDir, "brave", "vendor", "requests")
    );
...
Object.defineProperty(Config.prototype, "component", {
  get: function () {
    return (
      this.__component ||
      (this.buildConfig === "Release" ? "static_library" : "shared_library")
    );
  },
  set: function (component) {
    return (this.__component = component);
  },
});

Object.defineProperty(Config.prototype, "outputDir", {
  get: function () {
    if (this.__outputDir) return this.__outputDir;
