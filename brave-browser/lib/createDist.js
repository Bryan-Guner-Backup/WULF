    ...
    fs.readdir(config.outputDir, function (err, items) {
      for (var i = 0; i < items.length; i++) {
        ...
        if (new RegExp(/^brave-browser-[0-9.-]+.*rpm$/).test(items[i])) {
          newFileName =
            options.target_arch === "x86" ? I386_RPM_NAME : AMD64_RPM_NAME;
        } else if (new RegExp(/^brave-browser_[0-9.-]+.*deb$/).test(items[i])) {
          newFileName =
            options.target_arch === "x86" ? I386_DEB_NAME : AMD64_DEB_NAME;
        } else if (
          new RegExp(/^brave-browser-(unstable|beta).*(rpm|deb)$/).test(
            items[i]
          )
        ) {
        ...
        if (newFileName) {
          fs.renameSync(
            path.join(config.outputDir, items[i]),
