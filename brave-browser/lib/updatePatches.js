...
const updatePatches = (options) => {
  ...
  const modifiedFileList = modifiedDiff.stdout
    .toString()
    .split("\n")
    .filter(
      (s) =>
        s.length > 0 &&
        !s.startsWith("chrome/app/theme") &&
        !s.endsWith(".png") &&
        !s.endsWith(".xtb") &&
        !s.endsWith(".grd") &&
        !s.endsWith(".grdp") &&
  ...
  substitutedFileList = substitutedFileList.map(
  ...
  let existingFileList = existingFileOutput.stdout
    .toString()
    .split("\n")
    .filter((s) => s.length > 0);
  ...
  for (let i = 0; i < m; i++) {
    ...
    console.log(
      "updatePatches *REMOVED* " + (1 + i) + "/" + m + ": " + filename
    );
  }
};
