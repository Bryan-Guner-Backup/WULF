...
const util = {
  ...
  buildGClientConfig: () => {
    ...
    let solutions = config.projectNames
      .filter((projectName) => config.projects[projectName].ref)
      .map((projectName) => {
    ...
    const out =
      "solutions = " +
      JSON.stringify(solutions, replacer, 2)
        .replace(/"%None%"/g, "None")
  ...
  updateBranding: () => {
    ...
    const braveComponentsDir = path.join(
      config.projects["brave-core"].dir,
      "components"
    );
    const chromeAppDir = path.join(config.srcDir, "chrome", "app");
    const braveAppDir = path.join(config.projects["brave-core"].dir, "app");
    const chromeResourcesDir = path.join(
      config.srcDir,
      "chrome",
      "browser",
      "resources"
    );
    const braveResourcesDir = path.join(
      config.projects["brave-core"].dir,
      "browser",
      "resources"
    );
    const chromeBrowserDir = path.join(config.srcDir, "chrome", "browser");
    const braveBrowserDir = path.join(
      config.projects["brave-core"].dir,
      "browser"
    );
    const braveAppVectorIconsDir = path.join(
      config.projects["brave-core"].dir,
      "vector_icons",
      "chrome",
      "app"
    ...
    fs.copySync(
      path.join(braveAppDir, "generated_resources.grd"),
      path.join(chromeAppDir, "generated_resources.grd")
    );
    fs.copySync(
      path.join(braveAppDir, "bookmarks_strings.grdp"),
      path.join(chromeAppDir, "bookmarks_strings.grdp")
    );
    fs.copySync(
      path.join(braveAppDir, "media_router_strings.grdp"),
      path.join(chromeAppDir, "media_router_strings.grdp")
    );

    fs.copySync(
      path.join(braveAppDir, "settings_strings.grdp"),
      path.join(chromeAppDir, "settings_strings.grdp")
    );
    fs.copySync(
      path.join(braveAppDir, "md_extensions_strings.grdp"),
      path.join(chromeAppDir, "md_extensions_strings.grdp")
    );
    fs.copySync(
      path.join(braveResourcesDir, "md_extensions", "extensions_resources.grd"),
      path.join(chromeResourcesDir, "md_extensions", "extensions_resources.grd")
    );
    fs.copySync(
      path.join(braveResourcesDir, "settings", "settings_resources.grd"),
      path.join(chromeResourcesDir, "settings", "settings_resources.grd")
    );
    fs.copySync(
      path.join(braveBrowserDir, "browser_resources.grd"),
      path.join(chromeBrowserDir, "browser_resources.grd")
    );
    fs.copySync(
      path.join(braveAppDir, "components_brave_strings.grd"),
      path.join(config.srcDir, "components", "components_brave_strings.grd")
    );
    fs.copySync(
    ...
    fs.copySync(
      ...
      path.join(chromeAppDir, "vector_icons", "brave")
    );
    ...
    fs.copySync(
      path.join(braveAppDir, "strings"),
      path.join(chromeComponentsDir, "strings")
    );
    fs.copySync(
      path.join(braveResourcesDir, "settings", "brave_page_visibility.js"),
      path.join(chromeResourcesDir, "settings", "brave_page_visibility.js")
    );
  },
  ...
  submoduleSync: (options = {}) => {
    ...
    util.run(
      "git",
      ["-C", config.depotToolsDir, "reset", "--hard", "HEAD"],
      options
    );
  },
  ...
  gclientSync: (options = {}) => {
    runGClient(
      ["sync", "--force", "--nohooks", "--with_branch_heads"],
      options
    );
  },
  ...
  fetch: (options = {}) => {
    ...
    util.run(
      "git",
      ["-C", options.git_cwd, "fetch", "--all", "--tags"],
      options
    );
  },
  ...
  setVersion: (version, options = {}) => {
    ...
    util.run(
      "git",
      ["-C", options.git_cwd, "reset", "--hard", version],
      options
    );
  },
