(() => {
	...
	function onInBrowserReady() {
		inBrowserReady = true;
		for (
			let params = inBrowserQueue.shift();
			params;
			params = inBrowserQueue.shift()
		) {
	...
	function startServer() {
		...
		kernel.system(
			"/meme-service.js -bgdir=img -fontfile=font/impact.ttf",
			onExit,
			onStdout,
			onStderr
		);
	...
	function clicked() {
		...
		function completed(e) {
			if (e) {
				...
				console.log(
					"inBrowser call failed for unknown reason"
				);
			}
		}
