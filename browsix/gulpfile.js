...
function tsPipeline(src, dst) {
  return function () {
    var build = gulp.src(src).pipe(ts(project()));
...
function tsTask(subdir, options) {
  ...
  gulp.task("lint-" + subdir, function () {
    return gulp
      .src(["src/" + subdir + "/*.ts", "src/" + subdir + "/*/*.ts"])
      .pipe(lint())
...
gulp.task("copy-node-kernel", function () {
  return gulp
    .src(["node-modified/lib/binding/http_parser.js"])
...
gulp.task(
  "build-fs",
  ["dist-kernel", "dist-browser-node", "build-bin", "dist-syscall-api"],
  function () {
    var copyKernel = gulp
      .src("lib-dist/lib/kernel/kernel.js")
      .pipe(copy("./fs/boot/", { prefix: 3 }));

    var copyNode = gulp
      .src("lib-dist/lib/browser-node/browser-node.js")
      .pipe(
...
gulp.task("index-benchfs", [], function () {
  return run("./xhrfs-index benchfs")
    .exec()
    .pipe(
      rename(function (path) {
        path.basename = "index";
...
gulp.task("build-test", ["index-fs"], function () {
  return gulp.src("test/*.ts").pipe(ts(project())).js.pipe(gulp.dest("test"));
});
...
gulp.task("app:copy", ["index-fs"], function () {
  var app = gulp
    .src(["app/*", "!app/test", "!app/cache-config.json"], {
      dot: true,
    })
    .pipe(gulp.dest("dist"));
  ...
  var fs = gulp.src(["fs/**/*"]).pipe(gulp.dest("dist/fs"));
...
gulp.task("app:build", ["index-fs"], function (cb) {
  return gulp
    .src(["app/elements/**/*.ts"])
    .pipe(ts(project()))
    .js.pipe(gulp.dest("app/elements"));
});

// Watch files for changes & reload
gulp.task(
  "serve",
  ["app:build", "app:styles", "app:elements", "app:images"],
  function () {
