...
function SharedHandle(key, address, port, addressType, backlog, fd, flags) {
  ...
  else rval = net._createServerHandle(address, port, addressType, fd);
...
function RoundRobinHandle(key, address, port, addressType, backlog, fd) {
  ...
  if (fd >= 0) this.server.listen({ fd: fd });
  else if (port >= 0) this.server.listen(port, address);
...
RoundRobinHandle.prototype.handoff = function (worker) {
  ...
  sendHelper(worker.process, message, handle, function (reply) {
    ...
    else self.distribute(0, handle); // Worker is shutting down. Send to another.
    self.handoff(worker);
  });
};
...
function masterInit() {
  ...
  function onmessage(message, handle) {
    ...
    else if (message.act === "suicide") worker.suicide = true;
...
function workerInit() {
  ...
  cluster._setupWorker = function () {
    ...
    function onmessage(message, handle) {
      ...
      else if (message.act === "disconnect") worker.disconnect();
    }
  };
  ...
  cluster._getServer = function (obj, options, cb) {
    ...
    if (indexes[key] === undefined) indexes[key] = 0;
    else indexes[key]++;
    ...
    send(message, function (reply, handle) {
      ...
      if (handle) shared(reply, handle, cb);
      // Shared listen socket.
      else rr(reply, cb); // Round-robin.
    });
