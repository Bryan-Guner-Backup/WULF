...
fs.accessSync = function (path, mode) {
  ...
  if (mode === undefined) mode = fs.F_OK;
  else mode = mode | 0;
...
function readFileAfterRead(err, bytesRead) {
  ...
  if (context.size !== 0) {
    if (context.pos === context.size) context.close();
    else context.read();
  } else {
...
function readFileAfterClose(err) {
  ...
  if (context.err) return callback(context.err);
...
fs.symlink = function (destination, path, type_, callback_) {
  ...
  binding.symlink(
    preprocessSymlinkDestination(destination, type, path),
    pathModule._makeLong(path),
    type,
    req
  );
...
ReadStream.prototype._read = function (n) {
  ...
  function onread(er, bytesRead) {
    if (er) {
      ...
      var b = null;
      if (bytesRead > 0) b = thisPool.slice(start, start + bytesRead);
