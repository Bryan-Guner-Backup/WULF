...
exports.convertNPNProtocols = function convertNPNProtocols(NPNProtocols, out) {
  ...
  if (Array.isArray(NPNProtocols)) {
    var buff = new Buffer(
      NPNProtocols.reduce(function (p, c) {
...
exports.checkServerIdentity = function checkServerIdentity(host, cert) {
  ...
  if (net.isIP(host)) {
    ...
    if (!valid) {
      reason = util.format(
        "IP: %s is not in the cert's list: %s",
        host,
        ips.join(", ")
      );
    }
    ...
    if (!valid) {
      if (cert.subjectaltname) {
        reason = util.format(
          "Host: %s is not in the cert's altnames: %s",
          host,
          cert.subjectaltname
        );
      } else {
        reason = util.format(
          "Host: %s is not cert's CN: %s",
          host,
          cert.subject.CN
        );
      }
  ...
  if (!valid) {
    var err = new Error(
      util.format(
        "Hostname/IP doesn't match certificate's altnames: %j",
        reason
      )
    );
    err.reason = reason;
