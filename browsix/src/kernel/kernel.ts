...
if (
  ...
  typeof (<any>window).Worker === "undefined"
)
...
class Syscalls {
  ...
  execve(
    ...
    if (!senv["PATH"]) senv["PATH"] = "/usr/bin";

    ctx.task.exec(file, sargs, senv, (err: any, pid: number) => {
      let nerr = -EACCES;
      if (err && err.code === "ENOENT") nerr = -ENOENT;
  ...
  spawn(
    ...
    let args: string[] = iargs.map((x: Uint8Array | string): string =>
      toStr(x)
    );
  ...
  pwrite(ctx: SyscallContext, fd: number, buf: string | Uint8Array): void {
    ...
    if (!(buf instanceof Buffer) && buf instanceof Uint8Array) {
      ...
      // parent ArrayBuffer.
      let abuf = ubuf.buffer.slice(
        ubuf.byteOffset,
        ubuf.byteOffset + ubuf.byteLength
      );
      buf = new Buffer(abuf);
...
export class Task implements ITask {
  ...
  chdir(path: string, cb: Function): void {
    ...
    if (path[0] !== "/") path = join(this.cwd, path);
...
export function Boot(
  ...
  let asyncRoot = new (Function.prototype.bind.apply(
    rootConstructor,
    [null].concat(fsArgs)
  ))();
...
if (typeof window !== "undefined") (<any>window).Boot = Boot;
