...
function sys_getcwd(
  ...
  let done = function (p: string): void {
    for (let i = 0; i < p.length; i++) $getcwdArray[i] = p.charCodeAt(i);
    let nullPos = p.length;
    if (nullPos >= $getcwdArray.byteLength) nullPos = $getcwdArray.byteLength;
...
function sys_stat(cb: Function, trap: number, arg0: any, arg1: any): void {
  ...
  let done = function (err: any, stats: any): void {
    if (!err) $fstatArray.set(stats);
    cb([err ? -1 : 0, 0, err ? -1 : 0]);
  };
  let len = arg0.length;
  if (len && arg0[arg0.length - 1] === 0) len--;
...
function sys_readlinkat(
  ...
  let len = $path.length;
  if (len && arg1[$path.length - 1] === 0) len--;
...
function sys_openat(
  ...
  let len = arg1.length;
  if (len && arg1[arg1.length - 1] === 0) len--;
