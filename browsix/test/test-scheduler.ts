...
const IS_KARMA =
  ...
  typeof (<any>window).__karma__ !== "undefined";
...
describe("sched test", function (): void {
  ...
  it("should boot", function (done: MochaDone): void {
    Boot(
      ...
      function (err: any, freshKernel: Kernel): void {
  ...
  let count: number[] = [];
  for (let i = 0; i < 2; i++) count.push(i);
  ...
  count.map((i: number) => {
    it(
      "should run `priority-test /usr/bin/cpu-intensive-program`" + i,
      (done: MochaDone) => {
        kernel.system(
          "priority-test /usr/bin/cpu-intensive-program",
          cmdExited
        );
