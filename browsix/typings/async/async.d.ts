...
interface AsyncResultObjectCallback<T> {
  (err: Error, results: Dictionary<T>): void;
}
...
interface AsyncForEachOfIterator<T> {
  (item: T, key: number, callback: ErrorCallback): void;
}
interface AsyncResultIterator<T, R> {
  (item: T, callback: AsyncResultCallback<R>): void;
}
interface AsyncMemoIterator<T, R> {
  (memo: R, item: T, callback: AsyncResultCallback<R>): void;
}
interface AsyncBooleanIterator<T> {
  (item: T, callback: (truthValue: boolean) => void): void;
}

interface AsyncWorker<T> {
  (task: T, callback: ErrorCallback): void;
}
...
interface AsyncPriorityQueue<T> {
  ...
  push(
    task: T[],
    priority: number,
    callback?: AsyncResultArrayCallback<T>
  ): void;
  saturated: () => any;
  empty: () => any;
  drain: () => any;
...
interface Async {
  ...
  forEachOf<T>(
    obj: T[],
    iterator: AsyncForEachOfIterator<T>,
    callback?: ErrorCallback
  ): void;
  forEachOfSeries(
    obj: any,
  ...
  map<T, R>(
    arr: T[],
    iterator: AsyncResultIterator<T, R>,
    callback?: AsyncResultArrayCallback<R>
  ): any;
  mapSeries<T, R>(
    arr: T[],
    iterator: AsyncResultIterator<T, R>,
  ...
  reduce<T, R>(
    arr: T[],
    memo: R,
    iterator: AsyncMemoIterator<T, R>,
    callback?: AsyncResultCallback<R>
  ): any;
  inject<T, R>(
    arr: T[],
    memo: R,
  ...
  detect<T>(
    arr: T[],
    iterator: AsyncBooleanIterator<T>,
  ...
  sortBy<T, V>(
    arr: T[],
    iterator: AsyncResultIterator<T, V>,
    callback?: AsyncResultArrayCallback<T>
  ): any;
  some<T>(
    arr: T[],
  ...
  series<T>(
    tasks: Dictionary<AsyncFunction<T>>,
    callback?: AsyncResultObjectCallback<T>
  ): void;
  parallel<T>(
  ...
  during(
    test: (testCallback: (error: Error, truth: boolean) => void) => void,
    fn: AsyncVoidFunction,
    callback: (err: any) => void
  ): void;
  doDuring(
  ...
  forever(
    next: (errCallback: (err: Error) => void) => void,
  ...
  cargo(
    worker: (tasks: any[], callback: ErrorCallback) => void,
    payload?: number
  ): AsyncCargo;
  auto(tasks: any, callback?: (error: Error, results: any) => void): void;
  retry<T>(
    opts: number,
    task: (callback: AsyncResultCallback<T>, results: any) => void,
  ...
  times<T>(
    n: number,
    iterator: AsyncResultIterator<number, T>,
    callback: AsyncResultArrayCallback<T>
  ): void;
  timesSeries<T>(
    n: number,
