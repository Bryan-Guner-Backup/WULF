...
Interpreter.prototype._callbackStep = function (thisStep) {
  if (typeof this.stepListener === "function") {
    try {
      this.stepListener(
        thisStep,
        cloneDeep(this.stack, true),
        cloneDeep(this.altstack, true)
      );
    } catch (err) {
...
Interpreter.prototype.checkSequence = function (nSequence) {
  ...
  // transaction.
  var SEQUENCE_LOCKTIME_TYPE_FLAG_BN = new BN(
    Interpreter.SEQUENCE_LOCKTIME_TYPE_FLAG
  );
  ...
  if (fExec && opcodenum >= 0 && opcodenum <= Opcode.OP_PUSHDATA4) {
    ...
    switch (opcodenum) {
      ...
      case Opcode.OP_SPLIT:
        ...
        var position = BN.fromScriptNumBuffer(
          stacktop(-1),
          fRequireMinimal
        ).toNumber();
      ...
      case Opcode.OP_NUM2BIN:
        ...
        var size = BN.fromScriptNumBuffer(
          stacktop(-1),
          fRequireMinimal
        ).toNumber();
