...
Candy.Core = (function (self, Strophe, $) {
  ...
  self.registerEventHandlers = function () {
    ...
    self.addHandler(
      _connection.disco._onDiscoInfo.bind(_connection.disco),
      Strophe.NS.DISCO_INFO,
      "iq",
      "get"
    );
    self.addHandler(
      _connection.disco._onDiscoItems.bind(_connection.disco),
      Strophe.NS.DISCO_ITEMS,
      "iq",
      "get"
    );
    self.addHandler(
      _connection.caps._delegateCapabilities.bind(_connection.caps),
      Strophe.NS.CAPS
    );
  };
  ...
  self.connect = function (jidOrHost, password, nick) {
    ...
    _anonymousConnection = !_anonymousConnection
      ? jidOrHost && jidOrHost.indexOf("@") < 0
      : true;

    if (jidOrHost && password) {
      // authentication
      _connection.connect(
        _getEscapedJidFromJid(jidOrHost) + "/" + _options.resource,
        password,
