...
Candy.Core.Action = (function (self, Strophe, $) {
  ...
  self.Jabber = {
    ...
    Presence: function (attr, el) {
      ...
      var pres = $pres(attr)
        .c("priority")
        .t(Candy.Core.getOptions().presencePriority.toString())
        .up()
    ...
    Autojoin: function () {
      ...
      if (Candy.Core.getOptions().autojoin === true) {
        ...
        var pubsubBookmarkRequest =
          Candy.Core.getConnection().getUniqueId("pubsub");
        Candy.Core.addHandler(
          Candy.Core.Event.Jabber.Bookmarks,
          Strophe.NS.PUBSUB,
          "iq",
          "result",
    ...
    GetIgnoreList: function () {
      ...
      Candy.Core.addHandler(
        Candy.Core.Event.Jabber.PrivacyList,
        null,
        "iq",
        null,
        iqId
    ...
    Room: {
      ...
      Join: function (roomJid, password) {
        ...
        var conn = Candy.Core.getConnection(),
          ...
          pres = $pres({ to: roomNick, id: "pres:" + conn.getUniqueId() }).c(
            "x",
