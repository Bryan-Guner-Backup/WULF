  ...
  self.Jabber = {
      ...
      if (msg.attr("type") === "result") {
        $('list[name="ignore"] item', msg).each(function () {
          ...
          if (item.attr("action") === "deny") {
            currentUser.addToOrRemoveFromPrivacyList(
              "ignore",
              item.attr("value")
            );
          }
    ...
    Room: {
      ...
      Disco: function (msg) {
        ...
        var roomJid = Strophe.getBareJidFromJid(
          Candy.Util.unescapeJid(msg.attr("from"))
        );
      ...
      Presence: function (msg) {
        ...
        if (presenceType !== "unavailable") {
          if (roster.get(from)) {
            ...
            if (
              room.getUser() === null &&
              (Candy.Core.getUser().getNick() === nick || nickAssign)
            ) {
              room.setUser(user);
              currentUser = user;
            }
      ...
      _selfLeave: function (msg, from, roomJid, roomName, action) {
        ...
        var user = new Candy.Core.ChatUser(
          from,
      ...
      Message: function (msg) {
        ...
        if (
          msg.children("subject").length > 0 &&
          msg.children("subject").text().length > 0 &&
          msg.attr("type") === "groupchat"
        ) {
          roomJid = Candy.Util.unescapeJid(
            Strophe.getBareJidFromJid(msg.attr("from"))
          );
          message = {
            from: roomJid,
          ...
          var xhtmlChild = msg.children(
            'html[xmlns="' + Strophe.NS.XHTML_IM + '"]'
          );
          if (xhtmlChild.length > 0) {
            var xhtmlMessage = $(
              $("<div>")
                .append(
                  xhtmlChild
                    .children('body[xmlns="' + Strophe.NS.XHTML + '"]')
                    .first()
                    .contents()
                )
                .html()
        ...
        var timestamp =
          delay.length > 0 ? delay.attr("stamp") : new Date().toISOString();
      ...
      _checkForChatStateNotification: function (msg, roomJid, name) {
        var chatStateElements = msg.children(
