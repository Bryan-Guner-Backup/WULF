    ...
    if (isNaN(timestamp)) {
      ...
      if (struct) {
        ...
        timestamp = Date.parse(
          date.replace(/^(\d{4})(\d{2})(\d{2})/, "$1-$2-$3") + "Z"
        );
      }
    }
  ...
  self.Parser = {
    ...
    emotify: function (text) {
      ...
      for (i = this.emoticons.length - 1; i >= 0; i--) {
        text = text.replace(
          this.emoticons[i].regex,
          '$2<img class="emoticon" alt="$1" src="' +
            this._emoticonPath +
            this.emoticons[i].image +
            '" />$3'
        );
      }
      return text;
      ...
      if (Strophe.XHTML.validTag(tag)) {
        try {
          ...
          for (i = 0; i < elem.childNodes.length; i++) {
            el.append(
              self.createHtml(elem.childNodes[i], maxLength, currentLength)
            );
          }
        } catch (e) {
          // invalid elements
