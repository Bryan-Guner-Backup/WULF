...
Candy.View = (function (self, $) {
  ...
  var _current = { container: null, roomJid: null },
    ...
    _registerWindowHandlers = function () {
      if (Candy.Util.getIeVersion() < 9) {
        $(document)
          .focusin(Candy.View.Pane.Window.onFocus)
          .focusout(Candy.View.Pane.Window.onBlur);
      } else {
        $(window)
          .focus(Candy.View.Pane.Window.onFocus)
          .blur(Candy.View.Pane.Window.onBlur);
      }
    ...
    _delegateTooltips = function () {
      $("body").delegate(
        "li[data-tooltip]",
        "mouseenter",
        Candy.View.Pane.Chat.Tooltip.show
      );
    };
  ...
  self.init = function (container, options) {
    ...
    Candy.Util.Parser.setEmoticonPath(
      this.getOptions().assets + "img/emoticons/"
    );
