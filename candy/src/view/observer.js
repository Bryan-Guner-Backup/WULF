...
Candy.View.Observer = (function (self, $) {
  ...
  self.Presence = {
    ...
    update: function (event, args) {
      ...
      if (args.type === "leave") {
        ...
        switch (args.type) {
          case "kick":
            actionLabel = $.i18n._(
              actorName ? "youHaveBeenKickedBy" : "youHaveBeenKicked",
              translationParams
            );
            break;
          case "ban":
            actionLabel = $.i18n._(
              actorName ? "youHaveBeenBannedBy" : "youHaveBeenBanned",
              translationParams
            );
            break;
        }
        ...
        if (!Candy.View.Pane.Chat.rooms[args.roomJid]) {
          if (
            Candy.View.Pane.Room.init(args.roomJid, args.roomName) === false
          ) {
            return false;
          }
        ...
        // user jid
        if (
          Candy.View.Pane.Chat.rooms[args.user.getJid()] &&
          args.action !== "nickchange"
        ) {
          Candy.View.Pane.Roster.update(
            args.user.getJid(),
            args.user,
    ...
    notifyPrivateChats: function (user, type) {
      ...
      for (roomJid in Candy.View.Pane.Chat.rooms) {
        if (
          Candy.View.Pane.Chat.rooms.hasOwnProperty(roomJid) &&
  ...
  self.PresenceError = function (obj, args) {
    switch (args.type) {
      ...
      case "registration-required":
        Candy.View.Pane.Chat.Modal.showError("errorMembersOnly", [
          args.roomName,
        ]);
        break;
      case "service-unavailable":
        Candy.View.Pane.Chat.Modal.showError("errorMaxOccupantsReached", [
          args.roomName,
        ]);
        break;
    }
  ...
  self.Message = function (event, args) {
    if (args.message.type === "subject") {
      ...
      if (
        args.message.type === "chat" &&
        !Candy.View.Pane.Chat.rooms[args.roomJid]
      ) {
