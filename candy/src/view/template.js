...
Candy.View.Template = (function (self) {
  ...
  self.Chat = {
    ...
    adminMessage:
      '<li><small data-timestamp="{{timestamp}}">{{time}}</small><div class="adminmessage">' +
      '<span class="label">{{sender}}</span>' +
      '<span class="spacer">▸</span>{{subject}} {{message}}</div></li>',
    infoMessage:
      '<li><small data-timestamp="{{timestamp}}">{{time}}</small><div class="infomessage">' +
      '<span class="spacer">•</span>{{subject}} {{message}}</div></li>',
    toolbar:
      '<ul id="chat-toolbar">' +
    ...
    soundcontrol:
      '<script type="text/javascript">var audioplayerListener = new Object();' +
    ...
    Context: {
      menu:
        '<div id="context-menu"><i class="arrow arrow-top"></i>' +
        '<ul></ul><i class="arrow arrow-bottom"></i></div>',
      menulinks:
        '<li class="{{class}}" id="context-menu-{{id}}">{{label}}</li>',
      contextModalForm:
      ...
      adminMessageReason:
        '<a id="admin-message-cancel" class="close" href="#">×</a>' +
        "<p>{{_action}}</p>{{#reason}}<p>{{_reason}}</p>{{/reason}}",
    },
    tooltip:
      '<div id="tooltip"><i class="arrow arrow-top"></i>' +
      '<div></div><i class="arrow arrow-bottom"></i></div>',
  };

  self.Room = {
    pane:
      '<div class="room-pane roomtype-{{roomType}}" id="chat-room-{{roomId}}" data-roomjid="{{roomJid}}" data-roomtype="{{roomType}}">' +
      "{{> roster}}{{> messages}}{{> form}}</div>",
    subject:
      '<li><small data-timestamp="{{timestamp}}">{{time}}</small><div class="subject">' +
  ...
  self.Message = {
    ...
    item:
      '<li><small data-timestamp="{{timestamp}}">{{time}}</small><div>' +
