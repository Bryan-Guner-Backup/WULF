...
define([
  ...
  bdd.describe("Candy.Core.Action", function () {
    ...
    bdd.describe("requesting the roster", function () {
      ...
      bdd.describe(
        "if roster versioning is supported server side",
        function () {
          bdd.beforeEach(function () {
            Candy.Core.getConnection().features =
              Strophe.xmlGenerator().createElement("stream:features");
          ...
          bdd.it(
            "bootstraps the roster from the initial items provided",
            function () {
              Candy.Core.Action.Jabber.Roster();
          ...
          bdd.it(
            "includes the cached version in the roster request",
            function () {
              var request = sinon.spy(Candy.Core.getConnection(), "send");
          ...
          bdd.it(
            "emits an event indicating that the roster was loaded from cache",
            function () {
              var foo;
              $(Candy).on("candy:core:roster:loaded", function (ev, params) {
                foo = params;
              });
              Candy.Core.Action.Jabber.Roster();
      ...
      bdd.describe("once the roster is received", function () {
        ...
        bdd.it(
          "makes the returned items available in the main roster",
          function () {
            receiveResponse();
            expect(Candy.Core.getRoster().getAll()).to.have.keys([
              "foo@bar.com",
              "doo@dah.com",
            ]);
            expect(
              Candy.Core.getRoster().get("foo@bar.com")
            ).to.be.an.instanceof(Candy.Core.Contact);
            expect(
              Candy.Core.getRoster().get("doo@dah.com")
            ).to.be.an.instanceof(Candy.Core.Contact);
          }
        );
        ...
        bdd.it(
          "emits an event indicating that the roster was fetched",
          function () {
            var foo;
            $(Candy).on("candy:core:roster:fetched", function (ev, params) {
              foo = params;
            });
            receiveResponse();
            expect(foo).to.be.eql({ roster: Candy.Core.getRoster() });
        ...
        bdd.describe("updating roster items from pushes", function () {
          ...
          bdd.describe("modifying a user", function () {
            ...
            bdd.it(
              "emits an event indicating that the roster was updated",
              function () {
                var eventParams = null;
                $(Candy).on("candy:core:roster:updated", function (ev, params) {
                  eventParams = params;
                });
                receivePush();
          ...
          bdd.describe("removing a user", function () {
            ...
            bdd.it(
              ...
              function () {
                ...
                $(Candy).on("candy:core:roster:removed", function (ev, params) {
                  eventParams = params;
                });
                receivePush();
          ...
          bdd.describe("adding a user", function () {
            ...
            bdd.it(
              "makes the new item available in the main roster",
              function () {
                receivePush();
                expect(
                  Candy.Core.getConnection().roster.findItem("new@guy.com").name
                ).to.eql("Foo Bar");
                expect(
                  Candy.Core.getRoster().get("new@guy.com")
                ).to.be.an.instanceof(Candy.Core.Contact);
              }
            );
            ...
            bdd.it("records groups", function () {
              receivePush();
              var rosterItem = Candy.Core.getRoster().get("new@guy.com");
              expect(rosterItem.getGroups()).to.eql([
                "Friends",
                "Close Friends",
              ]);
            });
            ...
            bdd.it(
              "emits an event indicating that the roster item was added",
              function () {
                var eventParams = null;
                $(Candy).on("candy:core:roster:added", function (ev, params) {
                  eventParams = params;
                });
                receivePush();
