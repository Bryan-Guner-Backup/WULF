...
define([
  ...
  bdd.describe("Candy.Core.ChatUser", function () {
    ...
    bdd.beforeEach(function () {
      chatUser = new Candy.Core.ChatUser(
        "foo bar@conference.baz.com/SomeNick",
        "SomeNick",
        "admin",
        "member",
        "foo@bar.com/somewhere"
      );
    });
    ...
    bdd.it("reveals its escaped JID", function () {
      expect(chatUser.getEscapedJid()).to.equal(
        "foo\\20bar@conference.baz.com/SomeNick"
      );
    });
    ...
    bdd.describe("privacy lists", function () {
      bdd.it("has only empty lists to start", function () {
        ...
        expect(
          chatUser.isInPrivacyList("somelist", "some@jid.com")
        ).to.be.false;
      });
      ...
      bdd.it("can have existing lists set", function () {
        ...
        expect(
          chatUser.isInPrivacyList("otherlist", "some@jid.com")
        ).to.be.false;
      });
