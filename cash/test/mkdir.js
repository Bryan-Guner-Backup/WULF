...
describe("mkdir", function () {
  ...
  it("should error on an existing dir", function () {
    ...
    cash
      .mkdir("testabot")
      .should.equal("mkdir: cannot create directory testabot: File exists\n");
  });
  ...
  it("should error on a child dir that doesn't exist", function () {
    ...
    cash
      .mkdir("testabot/foo")
      .should.equal(
        "mkdir: cannot create directory testabot/foo: No such file or directory\n"
      );
  });
  ...
  describe("-v", function () {
    it("should print a message on a single directory created", function () {
      ...
      $.test("-e", "./testabot").should.equal(false);
      cash
        .mkdir("testabot", { verbose: true })
        .should.equal("mkdir: created directory testabot\n");
      $.test("-e", "./testabot").should.equal(true);
    });
    ...
    it("should print a message on a -p directory created", function () {
      ...
      $.test("-e", "./testabot").should.equal(false);
      cash
        .mkdir("testabot/foo", { parents: true, verbose: true })
        .should.equal(
          "mkdir: created directory testabot\nmkdir: created directory testabot/foo\n"
        );
      $.test("-e", "./testabot/foo").should.equal(true);
    });
  });
