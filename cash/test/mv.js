...
describe("mv", function () {
  ...
  it("should only accept directories on multiple files", function () {
    cash
      .mv("mv-a mv-b mvtemp/mv-cows")
      .should.equal("mv: target mvtemp/mv-cows is not a directory\n");
  });
  ...
  it("should skip and give message on non-existing src files", function () {
    ...
    cash.mv("mv-a mv-b mv-vorpal mv-c mv-howdy mvtemp").should.equal(msg);
    $.test("-e", "./mvtemp/mv-a").should.equal(true);
    $.test("-e", "./mvtemp/mv-b").should.equal(true);
  ...
  describe("-v", function () {
    it("should print verbose messages", function () {
      cash
        .mv("mv-a mv-b mvtemp", { verbose: true })
        .should.equal("mv-a -> mvtemp/mv-a\nmv-b -> mvtemp/mv-b\n");
      $.test("-e", "./mvtemp/mv-a").should.equal(true);
      $.test("-e", "./mvtemp/mv-b").should.equal(true);
    });
  });
