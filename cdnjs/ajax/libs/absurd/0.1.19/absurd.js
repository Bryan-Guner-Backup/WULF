...
var Absurd = (function (w) {
  ...
  lib.api.add = function (API) {
      ...
      for (var prop in props) {
        if (typeof props[prop] === "object") {
          ...
          if (prop.charAt(0) === ":") {
            ...
            checkAndExecutePlugin(selector, prop, props[prop], stylesheet) ===
            false
          ) {
            addRule(selector + " " + prop, props[prop], stylesheet);
          }
    ...
    var addRule = function (selector, props, stylesheet) {
      ...
      if (
        typeof API.getRules(stylesheet || "mainstream")[selector] == "object"
      ) {
    ...
    var add = function (rules, stylesheet) {
      ...
      for (var selector in rules) {
        if (rules[selector] instanceof Array) {
          for (
            var i = 0;
            i < rules[selector].length, (r = rules[selector][i]);
            i++
          ) {
    ...
    for (var selector in rules) {
      ...
      if (selector.indexOf("____raw") === 0) {
        ...
        for (var prop in rules[selector]) {
          ...
          entity +=
            "  " +
            transformUppercase(prop, options) +
            ": " +
            value +
            ";" +
            newline;
        ...
        case "_attrs":
          for (var attrName in value) {
            if (typeof value[attrName] === "function") {
              attrs +=
                " " +
                transformUppercase(attrName, passedOptions) +
                '="' +
                value[attrName]() +
                '"';
            } else {
  ...
  var packTag = function (tagName, attrs, childs) {
    ...
    if (childs !== "") {
      html +=
        "<" +
        transformUppercase(tagName, passedOptions) +
        attrs +
        ">" +
        newline +
        childs +
