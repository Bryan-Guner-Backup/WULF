      ...
      dispatch: function (eventName, data, scope) {
        ...
        if (listeners[eventName]) {
          for (var i = 0; i < listeners[eventName].length; i++) {
            ...
            callback.apply(scope || listeners[eventName][i].scope || {}, [
              data,
            ]);
          }
        }
    ...
    var handleAsyncFunctions = function (next) {
      if (HTMLElement) {
        ...
        if (
          ...
          HTMLElement.hasAttribute("data-absurd-async")
        ) {
          funcs.push(HTMLElement);
        } else {
          var els = HTMLElement.querySelectorAll
            ? HTMLElement.querySelectorAll("[data-absurd-async]")
            : [];
      ...
      if (HTMLElement) {
        ...
        var registerEvent = function (el) {
          ...
          if (attrValue.length >= 2) {
            if (
              !cache.events[attrValue[0]] ||
              cache.events[attrValue[0]].indexOf(el) < 0
            ) {
        ...
        if (
          HTMLElement.hasAttribute &&
          HTMLElement.hasAttribute("data-absurd-event")
        ) {
          registerEvent(HTMLElement);
        }
        var els = HTMLElement.querySelectorAll
          ? HTMLElement.querySelectorAll("[data-absurd-event]")
          : [];
        for (var i = 0; i < els.length; i++) {
          registerEvent(els[i]);
        }
  ...
  var components = function (absurd) {
    ...
    api.get = function (name) {
      if (comps[name]) {
        ...
        throw new Error("There is no component with name '" + name + "'.");
      }
    };
      ...
      if (selector.indexOf("____raw") === 0) {
        ...
        for (var prop in rules[selector]) {
          ...
          if (options && options.keepCamelCase === true) {
            ...
            entity +=
              "  " + transformUppercase(prop) + ": " + value + ";" + newline;
          }
        }
        entity += "}" + newline;
        css += entity;
    ...
    var processor = function (rules, callback, options) {
      ...
      for (var stylesheet in rules) {
        ...
        if (stylesheet === "mainstream") {
          ...
          css +=
            stylesheet + " {" + newline + toCSS(r, options) + "}" + newline;
        }
      }
