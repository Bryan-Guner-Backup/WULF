...
var Absurd = (function (w) {
  ...
  var qs = function (selector, parent) {
    if (parent === false) {
      ...
      parent = parent || this.el || document;
    }
  ...
  var Component = function (componentName, absurd, eventBus, cls) {
        ...
        var registerEvent = function (el) {
          ...
          var processAttributes = function (attrValue) {
            ...
            if (attrValue.length >= 2) {
              ...
              var args = attrValue;
              if (
                !cache.events[eventType] ||
                cache.events[eventType].indexOf(el) < 0
              ) {
    ...
    api.onAnimationEnd = function (el, func) {
      ...
      if (!eventName) {
        func.apply(this, [{ error: "Animations not supported." }]);
        return;
      }
          ...
          (function callFuncs() {
            if (funcs.length === 0) {
              ...
              if (typeof self[async.funcs[value].name] === "function") {
                self[async.funcs[value].name].apply(
                  self,
                  [replaceNodes].concat(async.funcs[value].args)
                );
              } else if (typeof async.funcs[value].func === "function") {
                async.funcs[value].func.apply(
                  self,
                  [replaceNodes].concat(async.funcs[value].args)
                );
              }
  ...
  var injecting = function (absurd) {
    ...
    var mq = function (query, callback, usePolyfill) {
      ...
      var isMatchMediaSupported =
        !!(window && window.matchMedia) && !usePolyfill;
      ...
      _api.handlecss = function (parsed, path) {
        ...
        if (
          ...
          for (var i = 0; (rule = rules[i]); i++) {
            switch (rule.type) {
              case "rule":
                _api.handlecssrule(rule);
                break;
              case "import":
                _api.handlecssimport(rule, path);
                break;
              default:
      ...
      _api.components = (function (api) {
        ...
        return (exports = {
          ...
          broadcast: function (event, data) {
            for (
              var i = 0;
              i < instances.length, (instance = instances[i]);
              i++
            ) {
        ...
        if (typeof o == "object") {
          for (var key in o) {
            if (typeof o[key] == "function") {
              ...
              o[key].length == 2 &&
              typeof o[key][0] == "string" &&
              typeof o[key][1] == "function"
            ) {
  ...
  lib.api.add = function (API) {
    ...
    var checkAndExecutePlugin = function (
      selector,
      prop,
      value,
      stylesheet,
    ...
    var addRule = function (selector, props, stylesheet, parentSelector) {
      ...
      // check for plugin
      if (
        checkAndExecutePlugin(null, selector, props, stylesheet, parentSelector)
      ) {
        return;
      }
      ...
      for (var prop in props) {
        ...
        if (
          props[prop] &&
          typeof props[prop].classify != "undefined" &&
          props[prop].classify === true
        ) {
          props[prop] =
            typeof props[prop].toJSON != "undefined"
              ? props[prop].toJSON()
              : props[prop].toString();
        }
        ...
        if (
          ...
          if (
            ...
            if (_selector.indexOf("^") === 0) {
              _selector =
                _selector.substr(1, _selector.length - 1) +
      ...
      for (var prop in _objects) {
        ...
        if (prop.charAt(0) === ":") {
          ...
          prop.indexOf("@media") === 0 ||
          prop.indexOf("@supports") === 0
        ) {
    ...
    var add = function (rules, stylesheet, opts) {
      ...
      try {
        ...
        if (typeof stylesheet === "object" && typeof opts === "undefined") {
          options = {
            combineSelectors:
              typeof stylesheet.combineSelectors != "undefined"
                ? stylesheet.combineSelectors
                : options.combineSelectors,
            preventCombining: options.preventCombining.concat(
              stylesheet.preventCombining || []
            ),
          };
          stylesheet = null;
        }
        ...
        for (var i = 0; i < toRegister.length; i++) {
          ...
          var pc =
            options && options.preventCombining
              ? "|" + options.preventCombining.join("|")
              : "";
          var uid =
            pc.indexOf("|" + selector) >= 0
              ? "~~" + API.numOfAddedRules + "~~"
              : "";
        ...
        throw new Error(
          "Error adding: " +
            JSON.stringify({ rules: rules, error: err.toString() })
        );
      }
    };
    return add;
  ...
  lib.api.express = function (api) {
    return function (options) {
      if (options === "addImport") return addImport;
      else {
        switch (options.morph) {
          ...
          case "jsonify":
            ...
            return __json;
          default:
            if ("string" === typeof options) cssoptions = { src: options };
            return __css;
        }
      }
    };
      ...
      if (/\.json$/.test(pathname)) {
        ...
        if (Array.prototype.isPrototypeOf(extpref)) {
          for (var iter = 0; iter < extpref.length; iter++) {
            absurdPath = path.join(
              absurdBasePath,
              pathname.replace(".json", extpref[iter])
            );
          ...
          for (var iter = 0; iter < defextpref.length; iter++) {
            absurdPath = path.join(
              absurdBasePath,
              pathname.replace(".json", defextpref[iter])
            );
            if (fs.existsSync(absurdPath)) {
              break;
            }
        ...
        if (!imports[absurdPath]) {
          return compile();
        }

        if (once && imports[absurdPath]) {
          return next();
        }
      ...
      if (/\.css$/.test(pathname)) {
        ...
        if (Array.prototype.isPrototypeOf(extpref)) {
          for (var iter = 0; iter < extpref.length; iter++) {
            absurdPath = path.join(
              absurdBasePath,
              pathname.replace(".css", extpref[iter])
            );
            if (fs.existsSync(absurdPath)) {
              break;
            }
          ...
          for (var iter = 0; iter < defextpref.length; iter++) {
            absurdPath = path.join(
              absurdBasePath,
              pathname.replace(".css", defextpref[iter])
            );
            if (fs.existsSync(absurdPath)) {
              break;
            }
      ...
      if (cssoptions.notstrimports) {
        if (tempimports.length > 0) {
          for (var iter = 0; iter < tempimports.length; iter++) {
            if (
              !(resultArray.indexOf(tempimports[iter]) > -1) &&
              tempimports[iter] !== pathname
            ) {
              ...
              if (rule.type === "import") {
                importPath = path.resolve(
                  path.dirname(pathname) +
                    "/" +
                    rule.import.replace(/"/g, "").replace(/'/g, "")
                );
  ...
  var metamorphosis = {
    html: function (api) {
      api.defaultProcessor = require(__dirname +
        "/../processors/html/HTML.js")();
    ...
    component: function (api) {
      api.defaultProcessor = require(__dirname +
        "/../processors/component/Component.js")();
    ...
    var storage = function (name, value) {
      if (typeof value !== "undefined") {
        ...
        if (_s[name]) {
          ...
          throw new Error(
            "There is no data in the storage associated with '" + name + "'"
          );
        }
      }
      return API;
  ...
  var toCSS = function (rules, options, indent) {
    ...
    for (var selector in rules) {
      ...
      if (selector.indexOf("____raw") === 0) {
        ...
        var entityStyle =
          indent[0] + selector.replace(/~~(.+)~~/, "") + " {" + newline;
        var entity = "";
  ...
  lib.processors.css.CSS = function () {
    var processor = function (rules, callback, options) {
      ...
      for (var stylesheet in rules) {
        ...
        r = options.combineSelectors
          ? combineSelectors(r, options.preventCombining)
          : r;
        if (stylesheet === "mainstream") {
          css += toCSS(r, options);
        } else {
  ...
  lib.processors.css.plugins.charset = function () {
    return function (api, charsetValue) {
      if (typeof charsetValue === "string") {
        ...
        charsetValue = charsetValue.charset
          .replace(/:/g, "")
          .replace(/'/g, "")
          .replace(/"/g, "")
          .replace(/ /g, "");
        ...
        case "_attrs":
          for (var attrName in value) {
            if (typeof value[attrName] === "function") {
              attrs +=
                " " +
                prepareProperty(attrName, passedOptions) +
                '="' +
                value[attrName]() +
                '"';
            } else {
  ...
  var packTag = function (tagName, attrs, childs) {
    ...
    if (childs !== null) {
      html +=
        "<" +
        prepareProperty(tagName, passedOptions) +
        attrs +
        ">" +
        newline +
        childs +
  ...
  var prepareHTML = function (html) {
    html = require("./helpers/TemplateEngine")(
      html.replace(/[\r\t\n]/g, ""),
      passedOptions
    );
