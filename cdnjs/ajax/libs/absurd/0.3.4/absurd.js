...
var Absurd = (function (w) {
  ...
  var require = function (v) {
    ...
    if (v.indexOf("css/CSS.js") > 0 || v == "/../CSS.js") {
      ...
      v == "./helpers/TemplateEngine" ||
      v == "../html/helpers/TemplateEngine"
    ) {
  ...
  var prefixExtract = function (prop) {
    var result, match;
    if (
      (match = prop.match(/^\-(w|m|s|o)+\-/) || prop.charAt(0) === "-") &&
      !prop.match(/^\-(webkit|moz|ms)+\-/)
    ) {
    ...
    for (var selector in rules) {
      ...
      for (var prop in props) {
        map.push({
          ...
          value: props[prop],
          combine:
            preventCombining.indexOf("|" + prop) < 0 &&
      ...
      if (map[i].combine === true && map[i].selector !== false) {
        for (var j = i + 1; j < map.length; j++) {
          if (map[i].prop === map[j].prop && map[i].value === map[j].value) {
            map[i].selector +=
              ", " +
              map[j].selector.replace(/~~(.+)~~/, "").replace(/^%.*?%/, "");
    ...
    var addEventAttribute = function (obj) {
      ...
      if (addition.length > 0) {
        if (/data-absurd-event/g.test(attrs)) {
          attrs.replace(
            /data-absurd-event="(.*)"/,
            'data-absurd-event="$1,' + addition.join(",") + '"'
          );
        } else {
