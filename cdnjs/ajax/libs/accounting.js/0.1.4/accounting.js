  ...
  function unformat(number, decimal) {
    ...
    var regex = new RegExp("[^0-9-" + decimal + "]", ["g"]),
      unformatted = parseFloat(
        ("" + number).replace(regex, "").replace(decimal, ".")
      );
  ...
  function formatNumber(number, precision, thousand, decimal) {
    ...
    if (typeof number === "object") {
      ...
      for (
        var i = 0, values = [];
        i < number.length;
        values.push(formatNumber(number[i], precision, thousand, decimal)) &&
        i++
      );
  ...
  function formatMoney(number, symbol, precision, thousand, decimal, format) {
    ...
    if (typeof number === "object") {
      ...
      for (
        ...
        i < number.length;
        values.push(
          formatMoney(number[i], symbol, precision, thousand, decimal, format)
        ) && i++
      );
      // We're done, return it:
      return values;
    ...
    return opts.format
      ...
      .replace(
        "%v",
        formatNumber(number, opts.precision, opts.thousand, opts.decimal)
      );
  }
  ...
  function formatColumn(list, symbol, precision, thousand, decimal) {
    ...
    for (i = 0; i < list.length; i++) {
      if (typeof list[i] === "object") {
        ...
        formatted.push(
          formatColumn(list[i], symbol, precision, thousand, decimal)
        );
      } else {
        // Format this number, push into formatted list and save the length:
        formatted.push(
          formatMoney(list[i], symbol, precision, thousand, decimal)
        );
