...
var accounting = (function () {
  ...
  function f(r) {
    ...
    if (d(r) && r.match("%v")) {
      return { pos: r, neg: r.replace("-", "").replace("%v", "-%v"), zero: r };
    } else {
  ...
  function n(t, u, r, w, q, v) {
    if (l(t)) {
      return a(t, function (x) {
        return n(x, u, r, w, q, v);
      });
    }
    var s = i(
      p(u)
        ? u
        : { symbol: u, precision: r, thousand: w, decimal: q, format: v },
      h.currency
    );
    t = j(t);
    s.precision = m(s.precision);
    s.format = f(s.format);
    s.format = t > 0 ? s.format.pos : t < 0 ? s.format.neg : s.format.zero;
    return s.format
      .replace("%s", s.symbol)
      .replace("%v", e(Math.abs(t), s.precision, s.thousand, s.decimal));
  }
  function b(x, s, v, z, u, y) {
    if (!x) {
      return [];
    }
    var q = i(
      p(s)
        ? s
        : { symbol: s, precision: v, thousand: z, decimal: u, format: y },
      h.currency
    );
    q.format = f(q.format);
    q.precision = m(q.precision);
    var r = 0,
      w =
        q.format.pos.indexOf("%s") < q.format.pos.indexOf("%v") ? true : false,
      t = a(x, function (D, A) {
        if (l(D)) {
          return b(D, q);
        } else {
          D = j(D);
          var C = D > 0 ? q.format.pos : D < 0 ? q.format.neg : q.format.zero,
            B = C.replace("%s", q.symbol).replace(
              "%v",
              e(Math.abs(D), q.precision, q.thousand, q.decimal)
            );
          if (B.length > r) {
            r = B.length;
          }
    ...
    return a(t, function (B, A) {
      if (d(B) && B.length < r) {
        return w
          ? B.replace(
              q.symbol,
              q.symbol + new Array(r - B.length + 1).join(" ")
            )
          : new Array(r - B.length + 1).join(" ") + B;
      }
      return B;
    });
  }
