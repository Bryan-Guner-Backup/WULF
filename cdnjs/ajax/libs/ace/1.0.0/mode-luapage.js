define(
  ...
  function (a, b, c) {
    var d = a("../lib/oop"),
      ...
      h = a("./luapage_highlight_rules").LuaPageHighlightRules,
      i = function () {
        var a = new h();
        (this.$tokenizer = new g(new h().getRules())),
          (this.$embeds = a.getEmbeds()),
          this.createModeDelegates({ "lua-": f });
      };
    d.inherits(i, e), (b.Mode = i);
  }
),
  define(
    "ace/mode/html",
  ...
  define(
    ...
    function (a, b, c) {
      var d = a("../lib/oop"),
        ...
        f = a("./lua_highlight_rules").LuaHighlightRules,
        g = function () {
          this.$rules = new e().getRules();
          for (var a in this.$rules)
            this.$rules[a].unshift(
              { token: "keyword", regex: "<\\%\\=?", next: "lua-start" },
              { token: "keyword", regex: "<\\?lua\\=?", next: "lua-start" }
            );
          this.embedRules(f, "lua-", [
            { token: "keyword", regex: "\\%>", next: "start" },
            { token: "keyword", regex: "\\?>", next: "start" },
          ]);
        };
      d.inherits(g, e), (b.LuaPageHighlightRules = g);
    }
  );
