...
new (function () {
  ...
  function destroyAPI(SYS) {
    if (!isDOM05) {
      for (var i = SYS.length; --i >= 0; window[SYS[i]] = null) {}
    } else {
  ...
  function ensureFailSafe(owner, isOn) {
    isOn = typeof isOn != "undefined" ? isOn : !(owner == false);
    var $onload = (owner = owner || window || this).onload;
    function runFailSafes(isRestoring) {
      if (isRestoring == true) {
        return $onload || null;
      }
      ...
      if (
        isFunction($onload) &&
        runFailSafes.toString() != $onload.toString()
      ) {
        $onload(isRestoring);
      }
      return $onload;
    }
    if (isFunction($onload) && runFailSafes.toString() == $onload.toString()) {
      if (!isOn) {
        owner.onload = $onload(true);
      }
      return;
    }
    owner.onload = runFailSafes;
  }
  ...
  function getMETAInfo(path) {
    ...
    var iExt = EXTENSION ? path.lastIndexOf(EXTENSION) : path.lastIndexOf(".");
  ...
  function getNamespaceInfo(_namespace, $notation) {
    ...
    for (
      ...
      for (var notation in lookups) {
        ...
        $notation = position == path.lastIndexOf(lookup) + 1 ? notation : null;
  ...
  function getNamespaceLookups(_namespace, notation) {
    ...
    for (var i = notations.length; --i >= 0; ) {
      ...
      lookups[notation][2] = lookups[notation][2] + ".";
    }
    return lookups;
  }
  function getOptions() {
  ...
  function getSeparator(mustRedefine) {
    ...
    var iBSlash = path.lastIndexOf("\\") + 1;
    var iFSlash = path.lastIndexOf("/") + 1;
    SEPARATOR = iBSlash > iFSlash ? "\\" : "/";
    return SEPARATOR;
  }
  ...
  function handleImport$formatURL(shortName, fullName, url, notation, version) {
    ...
    if (notation == null || typeof notation == "undefined") {
      notation = nsInfo
        ? typeof nsInfo.notation == "undefined"
          ? INFO.notation
          : nsInfo.notation
        : INFO.notation;
    }
  ...
  function handleImported$Wild(
    ...
    if (!isInclude) {
      ...
      for (var member in module) {
        ...
        if (
          nsInfoMap[$fullName] ||
          hasNamingConflict(member, $fullName, owner)
        ) {
          continue;
        }
  ...
  function ImportAs(shortName, fullName, url, notation, owner) {
    ...
    var _parentNsID = "";
    for (
      var nsParts = fullName.split("."), i = 0, j = nsParts.length;
      i < j;
      i++
    ) {
  ...
  function Load(url, container, code, defer, title, type, language) {
    ...
    if (
      typeof script.canHaveChildren == "undefined" ||
  ...
  function log(message, _caller, showLog) {
    ...
    if (typeof message != "undefined") {
      ...
      LOG =
        ...
        message +
        "\r\n\r\n";
      if ("undefined" != typeof console && isFunction(console.info)) {
        console.info(LOG);
      }
      if ("undefined" != typeof YAHOO && isFunction(YAHOO.log)) {
        YAHOO.log(LOG);
      }
      if ("undefined" != typeof MochiKit && isFunction(MochiKit.log)) {
        MochiKit.log(LOG);
      }
      LOG += _LOG;
    }
  ...
  function NamespaceInfo(
    ...
    function update(
      ...
      this.notation = isString(notation)
        ? notation
        : this.notation ||
          (INFO && isString(INFO.notation) ? INFO.notation : ".");
  ...
  function setAttribution(meta) {
    ...
    ATTRIB = QNAME.split(".").reverse().join(".");
  ...
  function SetLegacy(turnOn) {
    ...
    if (turnOn) {
      THIS.DIR_NAMESPACE = THIS.USE_PATH = "/";
      THIS.DOT_NAMESPACE = THIS.USE_NAME = ".";
