...
(function (/*com.iskitz.ajile*/ VERSION) {
  ...
  if (typeof $create != "undefined" && $create() > 1) return;
  ...
  var ALIAS = "Ajile",
    ...
    SEPARATOR,
    TYPES = [ALIAS, "Import", "ImportAs", "Include", "Load", "Namespace"],
    TYPES_OLD = [
      "JSBasePath",
      "JSImport",
      "JSPackage",
  ...
  function $create(exists) {
    ...
    if ((exists = exists || isIncompatible())) return 3;

    INFO = new NamespaceInfo(getNamespaceInfo(QNAME));

    if (!OVERRIDE && (exists = exists || (THIS = GetModule(QNAME)))) return 2;
  ...
  function AddImportListener(moduleName, listener) {
    ...
    else if (moduleName && !isString(moduleName)) return false;

    if (moduleName == INTERNAL && this == window[ALIAS]) return false;

    if (!moduleName && this != window[ALIAS]) moduleName = INTERNAL;
  ...
  function addUsage(fullName) {
    ...
    if (!usesOf) usage.add(fullName, (usesOf = new SimpleSet()));
  ...
  function cloakModule(moduleName, container) {
    ...
    if (module) cloakObject(isFunction(module) ? module : module.constructor);
    ...
    for (
      var id, src, sys, loaders = getLoaders(), i = loaders.length;
      --i >= 0;

    ) {
      if (!loaders[i]) continue;
      if ((id = loaders[i].title) && moduleName && id != moduleName) continue;
    ...
    if (isDOM1)
      cloakNode = function cloakDOM1(element, container) {
        if ((container = container || element.parentNode))
          if (container.removeChild) container.removeChild(element);
      };
    else if (isDOM05)
      cloakNode = new Function(
        "element",
        "container", //)
        /*{*/
        "if(container)" +
  ...
  function cloakObject(obj) {
    if (!obj || typeof obj.toString == "undefined") return;

    var name = /(function\s*.*\s*\(.*\))/.exec(obj.toString()) || [""],
      funcName = name.length > 1 ? name[1] : name[0];

    if (typeof obj.prototype == "undefined") return;
  ...
  function destroyEmptyNamespace(fullName) {
    ...
    for (var i = 1; typeof nsParts[i] != "undefined"; i++) {
      if (typeof ns[nsParts[i]] == "undefined") break;
  ...
  function detectCurrentModule(_namespace) {
    ...
    for (var moduleName, i = 0, j = pending.length; i < j; ++i) {
      if (processed.has((moduleName = pending[i][0]))) continue;

      if ("*" != pending[i][1])
        parentNamespace = RE_PARENT_NAMESPACE.exec(moduleName);

      if (!(parentNamespace && _namespace == parentNamespace[1])) continue;
  ...
  function ensureFailSafe(owner, isOn) {
    ...
    if (isFunction(onLoad) && executeFailSafe === onLoad) {
      if (!isOn) owner.onload = onLoad(true);
      return;
    }

    if (onLoad != executeFailSafe.onLoad) executeFailSafe.onLoad = onLoad;
  ...
  function executeFailSafe(isRestoring) {
    ...
    if (!onLoad || isRestoring == true) return onLoad || null;

    AddImportListener(cloakModule);
    completeImports();
    cloakModule();

    if (isFunction(onLoad)) onLoad(isRestoring);
  ...
  function getContainer(element) {
    ...
    if (typeof element.write == "undefined")
      if (typeof element.document != "undefined") element = element.document;
  ...
  function getImportInfo(shortName, fullName) {
    ...
    for (
      var isInclude = shortName == fullName, i = 0, j = parts.length;
      i < j;
      i++
    ) {
  ...
  function getLoaders(container) {
    if (!(container = getContainer(container))) return undefined;

    var loaders =
      typeof container.scripts != "undefined" &&
  ...
  function getMainLoader(container) {
    if (container)
      if ((!LOADER || isDOM1) && !isDOM05) {
        if (
          (LOADER && isDOM2 && container != LOADER.ownerDocument) ||
          !LOADER ||
          (!isDOM2 && isDOM1)
        )
  ...
  function getMETAInfo(path) {
    ...
    if (iExt < path.length && iExt >= 0) {
      ...
      if (version && isNaN(version.charAt(0))) version = "";
    }
  ...
  function GetNamespaceInfo(moduleOrName) {
    if (!moduleOrName) return new NamespaceInfo(INFO);
    ...
    for (var moduleName in nsInfoMap)
      if ("undefined" == typeof Object.prototype[moduleName])
        if (
          (isModuleName && moduleOrName == moduleName) ||
  ...
  function getNamespaceInfo(_namespace, $notation) {
    ...
    if (_namespace == QNAME && INFO && INFO.path) return INFO;
    ...
    if (!(loaders && lookups)) return undefined;

    var $path;

    for (
      var found = false, path, paths, i = 0, j = loaders.length;
      i < j;
      i++
    ) {
      ...
      for (var notation in lookups) {
        if (typeof Object.prototype[notation] != "undefined") continue;
        ...
        for (var lc = lookupList.length; --lc >= 0; ) {
          ...
          if (position <= 0 || position == positions[0]) continue;
        ...
        $notation =
          position == path.lastIndexOf(lookup) + 1 ? notation : undefined;
  ...
  function getNamespaceInfoCached(_namespace, lookups) {
    ...
    paths: for (var path in paths) {
      if (typeof Object.prototype[path] != "undefined") continue;
      ...
      for (var notation in lookups) {
        ...
        for (
          var lookupList = lookups[notation], i = lookupList.length;
          --i >= 0;
  ...
  function handleImport(
    ...
    if (isLoading) new ImportThread(fullName).start();
  ...
  function handleImport$formatURL(shortName, fullName, url, notation, version) {
    ...
    if (LEGACY)
      if (notation == false)
        //Backwards compatibility for JSPackaging & Ajile 0.5.7-.
        notation = "\x2f";
      else if (notation == true) notation = "\x2e";

    if (notation == null || undefined == notation)
      notation = nsInfo
        ? "undefined" == typeof nsInfo.notation
          ? INFO.notation
          : nsInfo.notation
        : INFO.notation;
    ...
    if (nsInfo && nsInfo.hasOption("refresh")) url = setRefresher(url);
  ...
  function handleImported(shortName, fullName, module, owner) {
    ...
    if (
      shortName &&
      shortName != LOADED &&
      (!pendingName || (pendingName != "*" && pendingName != LOADED))
    ) {
      if (isInclude) logMsg[0] = 'SUCCESS :: Include ("' + fullName + '")';
      else
        (owner[shortName] = module),
          (logMsg[0] =
            'SUCCESS :: ImportAs ("' + shortName + '", "' + fullName + '")');
      ...
      pendingName != "*" &&
      (pendingName == LOADED || shortName == LOADED)
    ) {
      logMsg[0] =
        "SUCCESS :: " +
    ...
    if (logMsg.length > 0) log(logMsg.join("\r\n"), arguments);

    notifyImportListeners(fullName);
    return module;
  }

  function handleImported$Wild(
    shortName,
    fullName,
    module,
    owner,
    ...
    if (!isInclude) {
      ...
      for (var member in module) {
        if (typeof Object.prototype[member] != "undefined") continue;
        ...
        logMsg[logMsg.length] =
          'SUCCESS :: ImportAs ("' + member + '", "' + $fullName + '")';
      }
    }

    pendingImports.remove(fullName);

    if (shortName != LOADED) pendingImports.add(fullName, LOADED);
  }
  ...
  function ImportAs(shortName, fullName, url, notation, owner) {
    ...
    if (shortName == "*") fullName = RE_PARENT_NAMESPACE.exec(fullName)[1];
    else if (typeof owner[shortName] != "undefined" && shortName != fullName)
      for (
        var sys = LEGACY ? TYPES.concat(TYPES_OLD) : TYPES, t = sys.length;
        --t >= 0;
    ...
    for (
      var nsParts = fullName.split("\x2e"), i = 0, j = nsParts.length;
      i < j;
      i++
    )
  ...
  function ImportThread(fullName, ttl, maxCheckCount) {
    ...
    function start() {
      ...
      if (GetModule(fullName)) completeImports();
      else threadID = window.setTimeout(start, 0);
    }

    function stop() {
      if (undefined != threadID) window.clearTimeout(threadID);

      if (undefined != terminatorID) window.clearInterval(terminatorID);
    }

    if (this.constructor != ImportThread)
      if (
        !this.constructor ||
        this.constructor.toString() != ImportThread.toString()
      )
  ...
  function Include(fullName, url, notation, owner) {
    if (fullName) fullName = fullName.split(".*").join("");
  ...
  function isIncompatible() {
    ...
    isDOM05 =
      ...
      "undefined" !=
        typeof (HEAD = document.getElementsByTagName("head")[0]).appendChild &&
  ...
  function isSupported(fullName, shortName) {
    ...
    for (var supporter in supporters)
      if ("undefined" == typeof Object.prototype[supporter])
        if (!GetModule(supporters[supporter])) return false;
  ...
  function Load(url, container, code, defer, title, type, language) {
    ...
    if (isDOM && !isICab) script = container.createElement("script");
    ...
    if (url) {
      ...
      if (isWebKit || !(isIE || isOpera)) script.src = url;
    ...
    else if (!script.canHaveChildren) script.text = code;
  ...
  function loadController() {
    ...
    if ("" != (name = name.substring(0, iEnd))) CONTROLLER = name;
  ...
  function loadOptions(path) {
    ...
    if ((option = RE_OPT_CLOAK.exec(path))) CLOAK = option[1] == "cloak";

    if ((option = RE_OPT_DEBUG.exec(path))) DEBUG = option[1] == "debug";

    if ((option = RE_OPT_LEGACY.exec(path))) SetLegacy(option[1] == "legacy");

    if ((option = RE_OPT_MVC.exec(path))) MVC = option[1] == "mvc";
    ...
    if ((option = RE_OPT_REFRESH.exec(path))) REFRESH = option[1] == "refresh";
  }

  function LoadSimple(src, container, code, defer, title, type, language) {
    if (!(container = getContainer(container || window || this))) return;
    ...
    if (src) log("DONE\t:: LoadSimple [ " + src + " ]", arguments);
    ...
    if (message != undefined) {
      ...
      !log.is
        ? (log.is = {
            ...
            MochiKit:
              typeof MochiKit != "undefined" && isFunction(MochiKit.log),
      ...
      log.is.MochiKit &&
        (state == "info"
          ? MochiKit.log(LOG)
          : state == "error"
          ? MochiKit.logError(LOG)
          : state == "warn"
          ? MochiKit.logWarning(LOG)
          : 0);
  ...
  function Namespace(_namespace, path, notation, owner) {
    ...
    for (var i = 0, j = nsParts.length; i < j; i++)
      script =
        typeof script[nsParts[i]] != "undefined"
          ? script[nsParts[i]]
          : (script[nsParts[i]] = {});
    ...
    if (!path) nsInfo = getNamespaceInfo(_namespace, notation);

    if (path || !nsInfo) nsInfo = new NamespaceInfo(path, notation, _namespace);

    if (nsInfo && !nsInfoMap[_namespace]) nsInfoMap[_namespace] = nsInfo;
  ...
  function NamespaceInfo(
    ...
    function handleArguments() {
      if (!(path && path.constructor == NamespaceInfo)) return;
    ...
    function hasOption(optionName) {
      ...
      return (
        option && typeof option[1] != "undefined" && option[1] == optionName
      );
    }
    ...
    function update(
      ...
      this.notation = isString(notation)
        ? notation
        : this.notation ||
          (INFO && isString(INFO.notation) ? INFO.notation : "\x2e");

      this.options = isString(options) ? options : this.options || getOptions();

      this.path = isString(path)
        ? path
        : this.path || (INFO && isString(INFO.path) ? INFO.path : "");
    ...
    if (this.constructor != NamespaceInfo)
      if (
        !this.constructor ||
        this.constructor.toString() != NamespaceInfo.toString()
      )
  ...
  function notifyImportListeners(fullName) {
    ...
    if (!listenerList[0] && !listenerList[1] && !listenerList[2]) return;

    var hasListeners =
      (listenerList[0] && listenerList[0].getSize() > 0) ||
    ...
    for (var listeners, i = listenerList.length; --i >= 0; ) {
      ...
      for (var id in listeners)
        if ("undefined" == typeof Object.prototype[id])
          if (isFunction(listeners[id])) listeners[id](fullName);
    }
  ...
  function publishAPI() {
    ...
    THIS.GetVersion.toString = THIS.GetVersion.prototype.toString =
      THIS.GetVersion;
  ...
  function RemoveImportListener(moduleName, listener) {
    ...
    if (!listenerList[0] && !listenerList[1] && !listenerList[2]) return false;
  ...
  function SetLegacy(turnOn) {
    ...
    if (turnOn) {
      ...
      cloakObject(
        (window.JSPacked = function (n) {
          INFO.notation = n;
        })
      );
      cloakObject(
        (window.NamespaceException = window.PackageException =
          DEPRECATED$NamespaceException)
      );
    }

    if (turnOn || typeof window["JSPackaging"] == "undefined") return;
  ...
  function setRefresher(url) {
    if (/ajile.refresh/g.test(url)) return url;

    return (
      url + (/\?/g.test(url) ? "&" : "?") + "ajile.refresh=" + Math.random()
    );
  }
  ...
  function ShowLog() {
    ...
    if (!DEBUG)
      LOG =
        ...
        ALIAS +
        ".EnableDebug()" +
        "</b> from within any of your scripts or use " +
        ALIAS +
        "'s " +
  ...
  function SimpleSet() {
    ...
    function clear() {
      for (var key in members)
        if ("undefined" == typeof Object.prototype[key]) delete members[key];
    ...
    function has(key) {
      return (
        typeof Object.prototype[key] == "undefined" &&
        typeof members[key] != "undefined"
      );
    }
    ...
    if (this.constructor != SimpleSet)
      if (
        !this.constructor ||
        this.constructor.toString() != SimpleSet.toString()
      )
  ...
  var currentModuleName = QNAME,
    ...
    isICab =
      typeof ScriptEngine != "undefined" && /InScript/.test(ScriptEngine()),
