...
(function (AQ, Y, b) {
  ...
  function AA(BF, BG) {
    ...
    var BJ = AZ(BF);
    if (BJ) {
      c(Ax(BJ) ? BJ : BJ.constructor);
    }
    if (!B) {
      return;
    }
    var BM = As(BF);
    var BI = BM && BM.hasOption("cloak");
    for (var BH, BE, BN, BL = Ao(), BK = BL.length; --BK >= 0; ) {
      if (!BL[BK]) {
        continue;
      }
      if ((BH = BL[BK].title) && BF && BH != BF) {
        continue;
      }
      BE = BL[BK].src;
      BN = (BE && BE.indexOf(A3) >= 0) || (BH && BH.indexOf(A3) == 0);
      if (BN || (!BE && BH) || BI || BB) {
        Am(BL[BK], BG);
      }
    }
  ...
  function c(BG) {
    if (!BG || typeof BG.toString == "undefined") {
      return;
    }
    var BE = /(function\s*.*\s*\(.*\))/.exec(BG.toString()) || [""],
      BF = BE.length > 1 ? BE[1] : BE[0];
    if (typeof BG.prototype == "undefined") {
      return;
    }
    BG.prototype.constructor.toString = function (BH) {
      return Ah && !BB ? BF : BG.prototype.toString();
    };
  }
  function AO(BF, BE) {
    return BF - BE;
