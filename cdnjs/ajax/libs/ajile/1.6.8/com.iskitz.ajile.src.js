...
(function (/*com.iskitz.ajile*/ VERSION, global, undefined) {
  ...
  function AddImportListener(moduleName, listener) {
    ...
    switch (true) {
      ...
      case !!moduleName && !isString(moduleName):
        ...
        groupListener = AddImportListener$handleGroup(
          moduleName,
          listener,
          notified
        );
    ...
    moduleName &&
      (processed.has(moduleName) || GetModule(moduleName)) &&
      (notified[notified.length] = notifyImportListener(
        listener.notify,
        moduleName
      ));
      ...
      for (var id in listeners)
        if ("undefined" == typeof Object.prototype[id]) {
          ...
          isFunction(listener.notify) &&
            (listener.notified[listener.notified.length] = notifyImportListener(
              listener.notify,
              fullName
            ));
        }
  ...
  function RemoveImportListener(moduleName, listener) {
    ...
    for (var listeners, notified, i = listenerList.length; --i >= 0; ) {
      ...
      for (var id in listeners)
        if ("undefined" == typeof Object.prototype[id]) {
          if (!listeners[id] || listeners[id].notify != listener) continue;
