...
(function (/*com.iskitz.ajile*/ VERSION, global, undefined) {
    ...
    if (moduleName) {
      ...
      for (var importee in processed.getAll())
        if ("undefined" == typeof Object.prototype[importee]) {
          ...
          item &&
            notifyImportListener(importee, item, listener.notify, notified);
        }
  ...
  function AddImportListener$handleGroup(names, notify, notified) {
    function isGroupImported(listener) {
      ...
      !notified.length &&
        notifyImportListener(names, items, notify, notified, isGroupImported);
    }
  ...
  function cloakModule(moduleName) {
    for (
      ...
      if (
        module &&
        (sys || (nsInfo && isCloaked) || CLOAK || !loaders[i].src)
      ) {
