...
(function (/*com.iskitz.ajile*/ VERSION, global, undefined) {
  ...
  function addDependence(fullName, shortName) {
    ...
    supporters.add(fullName, shortName) &&
      DEBUG &&
      log(
        currentModuleName + " :: USES :: " + (shortName || fullName),
        arguments
      );
  }

  function AddImportListener(moduleName, listener, notifyOriginal) {
    // API: ajile 1.6.1...1.7.3
    ensureFailSafe();
    ...
    switch (true) {
      ...
      case !!moduleName && !isString(moduleName):
        ...
        listener = AddImportListener$handleGroup(
          moduleName,
          listener,
          notified
        );
    ...
    if (moduleName == INTERNAL && this == global[ALIAS]) return false;
    ...
    !listeners &&
      (listeners = new SimpleSet()) &&
      importListeners.add(moduleName, listeners);
    ...
    moduleName &&
      moduleName != INTERNAL &&
      new ImportThread(moduleName).start();
    return true;
  }
  ...
  function AddImportListener$handleGroup(names, notify, notified) {
    function isGroupImported(listener) {
      ...
      !notified.length &&
        notifyOne(names, items, notify, notified, isGroupImported);
    }
  ...
  function cloakModule(moduleName) {
    for (
      ...
      if (
        module &&
        ((sys && debugOff) || (nsInfo && isCloaked) || CLOAK || !loaders[i].src)
      ) {
  ...
  function ensureFailSafe(isOn, owner) {
    ...
    if (onLoad && onLoad != executeFailSafe.onLoad)
      //HACK: Need a QueueSet, could have multiple unique onload listeners.
  ...
  function handleImported(shortName, fullName, module, owner) {
    ...
    if (
      ...
      if (isInclude) logMsg[0] = 'Include  ("' + fullName + '")...SUCCESS!';
      else {
        owner[shortName] = module;
        logMsg[0] =
          'ImportAs ("' + shortName + '", "' + fullName + '")...SUCCESS!';
      }
  ...
  function handleImported$Wild(
    ...
    if (!isInclude) {
      ...
      for (var member in module) {
        ...
        logMsg[logMsg.length] =
          'ImportAs ("' + member + '", "' + $fullName + '")...SUCCESS!';
      }
  ...
  function handleImports(fullName) {
    ...
    for (var module, shortName, i = modules.length; --i >= 0; ) {
      ...
      if (!module || !isSupported(fullName)) continue;
  ...
  function ImportAs(shortName, fullName, url, notation, owner) {
    ...
    for (
      var name, nsParts = fullName.split("\x2e"), i = 0, j = nsParts.length;
      i < j;
      i++
    ) {
      name = nsParts[i];
  ...
  function isArray(array) {
    ...
    return (
      ...
      (Array == array.constructor ||
        Array.toString() == array.constructor.toString())
    );
  }
  ...
  function isIncompatible() {
    ...
    isDOM2 = isDOM1 && !!document.ownerDocument;
  ...
  function isString(obj) {
    return (
      obj != undefined && (typeof obj == "string" || String == obj.constructor)
    );
  }
  ...
  function Listener(map) {
    if (!map || "function" != typeof map.notify) return;
    this.error = undefined;
    this.name = map.name || GENERIC;
    this.item = map.item;
    this.notify = map.notify;
  }
  Listener.prototype.toString = Listener.prototype.valueOf =
  ...
  function log(message, _caller, showLog) {
    ...
    var name = !_caller
        ? [""]
        : /function\s*([^(]*)\s*\(/.exec(String(_caller.callee)),
  ...
  function notifyOne(name, item, notify, notified, internalNotify) {
    function notifyOne() {
      if (notified && !notified.length) {
        DEBUG &&
          log(
            name + (isDOM05 ? "" : " :: synchronously") + "...SKIP",
            arguments
          );
      ...
      DEBUG &&
        log(name + (isDOM05 ? "" : " :: synchronously") + "...DONE", arguments);
    }

    DEBUG &&
      log(name + (isDOM05 ? "" : " :: synchronously") + "...", arguments);
  ...
  function notifyAll(fullName, item) {
    ...
    var hasListeners =
      !!(listenerList[0] && listenerList[0].getSize()) ||
  ...
  function RemoveImportListener(moduleName, listener) {
    ...
    switch (true) {
      ...
      case !!moduleName && !isString(moduleName):
        if ("object" == typeof moduleName && moduleName.name != undefined) {
          // API: ajile 1.7.3-
        ...
        for (var m = 0, ml = moduleName.length; m < ml; m++)
          wasRemoved =
            wasRemoved && RemoveImportListener(moduleName[m], listener);
        return wasRemoved;
    }
    ...
    for (
      var callback = listener, listeners, i = listenerList.length;
      --i >= 0;
      ...
      for (var id in listeners)
        if ("undefined" == typeof Object.prototype[id]) {
          ...
          if (
            !listener ||
            (callback != listener.notify && callback != listener.notifyOriginal)
          )
          ...
          for (
            var notified = listener.notified, n = 0, nl = notified.length;
            n < nl;
            n++
          ) {
  ...
  function SetLegacy(turnOn) {
    ...
    if (turnOn) {
      ...
      cloakObject((global.JSPackage = global.Package = Namespace));
      cloakObject(
        (global.JSPacked = function (n) {
          INFO.notation = n;
        })
      );
      cloakObject(
        (global.NamespaceException = global.PackageException =
          DEPRECATED$NamespaceException)
      );
    }

    if (turnOn || typeof global["JSPackaging"] == "undefined") return;
