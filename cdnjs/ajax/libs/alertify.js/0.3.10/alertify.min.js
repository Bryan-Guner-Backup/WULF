      ...
      (p = {
        ...
        build: function (a) {
          ...
          switch (
            ...
            "prompt" === c && (b += '<form id="alertify-form">'),
            (b += '<article class="alertify-inner">'),
            (b += q.message.replace("{{message}}", d)),
            "prompt" === c && (b += q.input),
            (b += q.buttons.holder),
            (b += "</article>"),
            "prompt" === c && (b += "</form>"),
            ...
            case "confirm":
              (b = b.replace(
                "{{buttons}}",
                this.appendButtons(q.buttons.cancel, q.buttons.ok)
              )),
                (b = b
            ...
            case "prompt":
              (b = b.replace(
                "{{buttons}}",
                this.appendButtons(q.buttons.cancel, q.buttons.submit)
              )),
                (b = b
                  .replace("{{ok}}", this.labels.ok)
                  .replace("{{cancel}}", this.labels.cancel));
          ...
          return (
            (k.className = "alertify alertify-" + c + " " + e),
            (j.className = "alertify-cover"),
            b
          );
        },
        close: function (a, b) {
          ...
          this.bind(a, "click", function () {
            ...
            (d = function (a) {
              a.stopPropagation(),
                ...
                l.removeChild(this),
                l.hasChildNodes() || (l.className += " alertify-logs-hidden");
            }),
            (c = function (a) {
              "undefined" != typeof a &&
                a.parentNode === l &&
                (f.transition.supported
                  ...
                  : (l.removeChild(a),
                    l.hasChildNodes() ||
                      (l.className += " alertify-logs-hidden")));
            }),
            0 !== b &&
              setTimeout(function () {
                c(a);
        ...
        dialog: function (a, b, c, e, f) {
          ...
          return (
            "function" == typeof this.init && (this.init(), g()),
            t.push({
              type: b,
              message: a,
              callback: c,
          ...
          t.splice(0, 1),
            t.length > 0
              ...
              : ((r = !1),
                (a = function (c) {
                  c.stopPropagation(),
                    (k.className += " alertify-isHidden"),
                    b.unbind(k, b.transition.type, a);
                }),
                this.transition.supported
                  ? (this.bind(k, this.transition.type, a),
                    (k.className = "alertify alertify-hide alertify-hidden"))
                  : (k.className =
                      "alertify alertify-hide alertify-hidden alertify-isHidden"),
                (j.className = "alertify-cover alertify-cover-hidden"),
                i.focus());
        },
        init: function () {
          d.createElement("nav"),
            ...
            j.setAttribute("id", "alertify-cover"),
            (j.className = "alertify-cover alertify-cover-hidden"),
            d.body.appendChild(j),
            (k = d.createElement("section")),
            k.setAttribute("id", "alertify"),
            (k.className = "alertify alertify-hidden"),
            d.body.appendChild(k),
            (l = d.createElement("section")),
            l.setAttribute("id", "alertify-logs"),
            (l.className = "alertify-logs alertify-logs-hidden"),
