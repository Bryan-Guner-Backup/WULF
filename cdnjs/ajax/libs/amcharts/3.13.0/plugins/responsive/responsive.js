...
AmCharts.addInitHandler(
  function (chart) {
    ...
    if (undefined === chart.responsive) return;

    // check if it's already initialized
    if (chart.responsive.ready) return;

    // a small variable for easy reference
    var r = chart.responsive;

    // init
    (r.ready = true), (r.currentRules = {});
    r.overridden = [];
    r.original = {};

    // check if responsive is enabled
    if (true !== r.enabled) return;
    ...
    function checkRules() {
      ...
      for (var x in r.rules) {
        ...
        if (
          ...
          (undefined == rule.rotate ||
            (true === rule.rotate && true === chart.rotate) ||
            (false === rule.rotate &&
              (undefined === chart.rotate || false === chart.rotate))) &&
          (undefined == rule.legendPosition ||
            (undefined !== chart.legend &&
              undefined !== chart.legend.position &&
    ...
    function findArrayObject(node, id) {
      if (node instanceof Array) {
        for (var x in node) {
          if ("object" === typeof node[x] && node[x].id == id) return node[x];
        }
      }
      return false;
    }
    ...
    function redrawChart() {
      ...
      if ("xy" !== chart.type) chart.marginsUpdated = false;
        ...
        if (undefined === original[key]) {
          ...
          // an array of objects
            ...
            for (var x in override[key]) {
              ...
              if (
                undefined === override[key][x].id &&
                undefined != original[key][x]
              )
                originalNode = original[key][x];
              else if (undefined !== override[key][x].id)
                originalNode = findArrayObject(
                  original[key],
                  override[key][x].id
                );
              if (originalNode) {
    ...
    function setOriginalProperty(object, prop, value) {
      if (undefined === object["_r_" + prop]) object["_r_" + prop] = value;
    ...
    function restoreOriginals() {
      ...
      while ((p = r.overridden.pop())) {
        if ("_r_none" === p.o["_r_" + p.p]) delete p.o[p.p];
        else p.o[p.p] = p.o["_r_" + p.p];
      }
    }
