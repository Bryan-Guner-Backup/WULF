AmCharts.AmExport = AmCharts.Class({
  ...
  init: function () {
    ...
    if (
      ...
      typeof window.RGBColor != "undefined"
    ) {
    ...
    if (cfg) {
      cfg.menuItemOutput = AmCharts.extend(
        _this.cfg.menuItemOutput,
        cfg.menuItemOutput || {}
      );
      cfg.menuItemStyle = AmCharts.extend(
        _this.cfg.menuItemStyle,
        cfg.menuItemStyle || {}
      );
  ...
  output: function (cfg, externalCallback) {
    var _this = this;
    cfg = AmCharts.extend(
      AmCharts.extend({}, _this.cfg.menuItemOutput),
      cfg || {}
    );
    ...
    function internalCallback() {
      ...
      if (cfg.format == "image/svg+xml" || cfg.format == "svg") {
        ...
        if (cfg.output == "save") {
          ...
          cfg.output == "datastring" ||
          cfg.output == "datauristring" ||
          cfg.output == "dataurlstring"
        ) {
        ...
        if (externalCallback) externalCallback.apply(_this, [blob]);
      ...
      while (i--) {
        if (_this.cfg.removeImagery) {
          ...
          try {
            ...
            _this.log(
              "Tainted canvas, reached browser CORS security; origin from imagery must be equal to the server!"
            );
            throw new Error(err);
          }
  ...
  generateOutput: function (cfg, callback) {
    ...
    if (_this.chart.periodSelector) {
      if (
        ["left", "right"].indexOf(_this.chart.periodSelector.position) != -1
      ) {
    ...
    if (_this.chart.dataSetSelector) {
      if (
        ["left", "right"].indexOf(_this.chart.dataSetSelector.position) != -1
      ) {
        if (!adapted.width) {
    ...
    function drawItWhenItsLoaded() {
      ...
      if (
        _this.processing.buffer.length == _this.processing.drawn ||
        cfg.format == "svg"
      ) {
        if (_this.DEBUG == 10) {
        ...
        if (_this.DEBUG == 10) {
          _this.log(
            "DRAW",
            _this.processing.drawn + 1,
            "OF",
            _this.processing.buffer.length
          );
        } // DEBUG
        ...
        if (cfg.render == "browser") {
          ...
          if (
            img.complete ||
            typeof img.complete == "undefined" ||
            img.complete === undefined
          ) {
            if (_this.DEBUG == 10) {
  ...
  generateButtons: function () {
    ...
    function createList(items) {
      ...
      for (var i = 0; i < items.length; i++) {
        var li = document.createElement("li"),
          ...
          item = items[i],
          children = null,
          itemStyle = AmCharts.extend(
            AmCharts.extend({}, _this.cfg.menuItemStyle),
            items[i]
          );

        // MERGE CFG
        item = AmCharts.extend(
          AmCharts.extend({}, _this.cfg.menuItemOutput),
          item
        );
        ...
        if (item["icon"]) {
          ...
          img.setAttribute(
            "style",
            "margin: 0 auto;border: none;outline: none"
          );
          if (item["iconTitle"]) {
    ...
    div.setAttribute(
      "style",
      "width:39px; height:28px; position: absolute;top:" +
        _this.cfg.menuTop +
        ";right:" +
        _this.cfg.menuRight +
        ";bottom:" +
        _this.cfg.menuBottom +
        ";left:" +
        _this.cfg.menuLeft +
