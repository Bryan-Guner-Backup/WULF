 ...
 */
angular
  .module("autofields.bootstrap", ["autofields.standard", "ui.bootstrap"])
  .config([
    "$autofieldsProvider",
    function ($autofieldsProvider) {
      ...
      $autofieldsProvider.registerHandler(
        "checkbox",
        function (directive, field, index) {
          var fieldElements = $autofieldsProvider.field(
            directive,
            field,
            "<input/>"
          );
          ...
          if (fieldElements.label)
            fieldElements.label.prepend(fieldElements.input);
      ...
      $autofieldsProvider.settings.scope.openCalendar = function (
        $scope,
        property,
        e
      ) {
      ...
      $autofieldsProvider.registerHandler(
        "date",
        function (directive, field, index) {
          var showWeeks = field.showWeeks
            ? field.showWeeks
            : directive.options.dateSettings.showWeeks;
          var datepickerPopup = field.datepickerPopup
            ? field.datepickerPopup
            : directive.options.dateSettings.datepickerPopup;
      ...
      $autofieldsProvider.settings.classes.row =
        $autofieldsProvider.settings.classes.row || [];
      $autofieldsProvider.settings.classes.row.push("row");
      $autofieldsProvider.settings.classes.col =
        $autofieldsProvider.settings.classes.col || [];
      $autofieldsProvider.settings.classes.col.push("col-sm-$size");
      $autofieldsProvider.settings.classes.colOffset =
        $autofieldsProvider.settings.classes.colOffset || [];
      $autofieldsProvider.settings.classes.colOffset.push(
        "col-sm-offset-$size"
      );
      $autofieldsProvider.registerHandler(
        "multiple",
        function (directive, field, index) {
          ...
          angular.forEach(field.fields, function (cell, cellIndex) {
            ...
            var cellSize =
              cell.type != "multiple"
                ? cell.columns || field.columns
                : field.columns;
            cellContainer.addClass(
              directive.options.classes.col
                .join(" ")
                .replace(/\$size/g, cellSize)
            );

            cellContainer.append(
              $autofieldsProvider.createField(directive, cell, cellIndex)
            );
      ...
      $autofieldsProvider.settings.classes.helpBlock =
        $autofieldsProvider.settings.classes.helpBlock || [];
      $autofieldsProvider.settings.classes.helpBlock.push("help-block");
      $autofieldsProvider.registerMutator(
        "helpBlock",
        function (directive, field, fieldElements) {
          if (!field.help) return fieldElements;

          fieldElements.helpBlock = angular.element("<p/>");
          fieldElements.helpBlock.addClass(
            directive.options.classes.helpBlock.join(" ")
          );
      ...
      $autofieldsProvider.settings.classes.inputGroupAddon = [
        "input-group-addon",
      ];
      $autofieldsProvider.settings.classes.inputGroupAddonButton = [
        "input-group-btn",
      ];
      ...
      $autofieldsProvider.registerMutator(
        ...
        function (directive, field, fieldElements) {
          ...
          fieldElements.inputGroup.addClass(
            $autofieldsProvider.settings.classes.inputGroup.join(" ")
          );
          ...
          angular.forEach(field.$addons || field.addons, function (addon) {
            var inputGroupAddon = angular.element("<span/>"),
              button = null;
            inputGroupAddon.addClass(
              $autofieldsProvider.settings.classes.inputGroupAddon.join(" ")
            );

            if (addon.button) {
              inputGroupAddon.attr(
                "class",
                $autofieldsProvider.settings.classes.inputGroupAddonButton.join(
                  " "
                )
              );
              button = angular.element('<button type="button"/>');
              button.addClass(
                $autofieldsProvider.settings.classes.button.join(" ")
              );
            ...
            if (addon.content != null)
              (button || inputGroupAddon).html(addon.content);
            if (addon.attr)
              $autofieldsProvider.setAttributes(
                directive,
                field,
          ...
          angular.forEach(toAppend, function (el) {
            fieldElements.inputGroup.append(el);
          });
      ...
      $autofieldsProvider.registerMutator(
        ...
        function (directive, field, fieldElements) {
          if (
            !(
              directive.options.layout &&
              directive.options.layout.type == "horizontal"
            )
          ) {
          ...
          var labelSize = field.labelSize
            ? field.labelSize
            : directive.options.layout.labelSize;
          var inputSize = field.inputSize
            ? field.inputSize
            : directive.options.layout.inputSize;
          ...
          if (field.type == "checkbox") {
            ...
            inputContainer.append(
              fieldElements.inputGroup || fieldElements.input
            );
          }
 ...
 */
angular
  .module("autofields.bootstrap.validation", ["autofields.validation"])
  .config([
    "$tooltipProvider",
  ...
  .config([
    ...
    function ($autofieldsProvider) {
      ...
      $autofieldsProvider.settings.attributes.container.ngClass =
        "{'has-error':" +
        $autofieldsProvider.settings.validation.invalid +
        ", 'has-success':" +
        $autofieldsProvider.settings.validation.valid +
        "}";
      $autofieldsProvider.settings.attributes.input.popover =
        "{{(" +
        $autofieldsProvider.settings.validation.valid +
        ") ? '$validMsg' : ($errorMsgs)}}";
      ...
      $autofieldsProvider.registerMutator(
        ...
        function (directive, field, fieldElements) {
          ...
          if (
            !fieldElements.validation ||
            $autofieldsProvider.settings.noPopover.indexOf(field.type) != -1
          ) {
          ...
          if (fieldElements.msgs.length) {
            var popoverAttr = fieldElements.input
              .attr("popover")
