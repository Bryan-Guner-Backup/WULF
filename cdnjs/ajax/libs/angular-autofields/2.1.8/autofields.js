...
angular
  ...
  .provider("$autofields", function () {
    ...
    var setAttributes = (autofields.setAttributes = function (
      directive,
      field,
      el,
      attrs
    ...
    var getFieldContainer = function (directive, field, attrs) {
      ...
      attrs = angular.extend(
        {},
        autofields.settings.attributes.container,
        attrs
      );
      setAttributes(directive, field, fieldContainer, attrs);
      fieldContainer.addClass(
        (directive.options || autofields.settings).classes.container.join(" ")
      );
    ...
    var getLabel = function (directive, field, attrs) {
      ...
      label.addClass(
        (directive.options || autofields.settings).classes.label.join(" ")
      );
    ...
    var getInput = function (directive, field, html, attrs) {
      ...
      attrs = angular.extend(
        {},
        autofields.settings.attributes.input,
        attrs,
        field.attr
      );
      setAttributes(directive, field, input, attrs);
      input.addClass(
        (directive.options || autofields.settings).classes.input.join(" ")
      );
    ...
    autofields.field = function (directive, field, html, attrs) {
      ...
      fieldElements.fieldContainer
        .append(fieldElements.label)
        .append(fieldElements.input);
      ...
      if (!mutators[key] || options.override) {
        mutators[key] = function (
          ...
          if (
            ...
            mutatorsRun.indexOf(options.require) == -1
          ) {
            fieldElements = mutators[options.require];
          }
          if (mutatorsRun.indexOf(key) == -1)
            return fn(directive, field, fieldElements);
        };
      }
    };
    ...
    function ($autofieldsProvider) {
      ...
      $autofieldsProvider.registerHandler(
        ...
        function (directive, field, index) {
          ...
          if (field.type == "url" && fixUrl)
            fieldElements.input.attr("fix-url", "");
      ...
      $autofieldsProvider.registerHandler(
        "select",
        function (directive, field, index) {
          var defaultOption = field.defaultOption
            ? field.defaultOption
            : directive.options.defaultOption;

          var inputHtml =
            '<select><option value="">' + defaultOption + "</option></select>";
          var inputAttrs = {
      ...
      $autofieldsProvider.settings.displayAttributes = (
        $autofieldsProvider.settings.displayAttributes || []
        ...
        link: function (scope, element, attr, ngModel) {
          ...
          var formatUrl = function (value) {
            ...
            if (test) {
              ...
              var reformatted =
                matches[1] != null && matches[1] != "" ? matches[1] : "http://";
    ...
    $autofieldsProvider.registerMutator(
      ...
      function (directive, field, fieldElements) {
        ...
        fieldElements.validation =
          directive.options.validation.enabled && field.validate !== false;
        ...
        angular.forEach(
          angular.extend(
            {},
            directive.options.validation.defaultMsgs,
            field.msgs
          ),
        ...
        // Get Valid Message
        fieldElements.validMsg =
          field.msgs && field.msgs.valid
            ? field.msgs.valid
            : directive.options.validation.defaultMsgs.valid;
