angular.module("azure-mobile-service.module", []).service("Azureservice", [
  ...
  function Azureservice($window, $q, AzureMobileServiceClient) {
    ...
    var initMSClient = function () {
      if (
        isNullOrUndefined(AzureMobileServiceClient.API_URL) ||
        isNullOrUndefined(AzureMobileServiceClient.API_KEY)
      ) {
      ...
      client = new MobileServiceClient(
        AzureMobileServiceClient.API_URL,
        AzureMobileServiceClient.API_KEY
      );
    };
    var setStorage = function () {
      if (
        typeof AzureMobileServiceClient.STORAGE === "string" &&
        AzureMobileServiceClient.STORAGE.toLowerCase() === "local"
      ) {
    ...
    return {
      query: function (tableName, obj, withFilterFn) {
        ...
        return wrapAzurePromiseWithAngularPromise(
          data.includeTotalCount().read()
        );
      },
      getById: function (tableName, id, withFilterFn) {
        ...
        return wrapAzurePromiseWithAngularPromise(
          getTable(tableName, withFilterFn).lookup(id)
        );
      },
      ...
      read: function (tableName, parameters, withFilterFn) {
        return wrapAzurePromiseWithAngularPromise(
          getTable(tableName, withFilterFn).read(parameters)
        );
      },
      insert: function (tableName, obj, withFilterFn) {
        ...
        return wrapAzurePromiseWithAngularPromise(
          getTable(tableName, withFilterFn).insert(obj)
        );
      },
      update: function (tableName, obj, withFilterFn) {
        ...
        return wrapAzurePromiseWithAngularPromise(
          getTable(tableName, withFilterFn).update(obj)
        );
      },
      del: function (tableName, obj, withFilterFn) {
        ...
        return wrapAzurePromiseWithAngularPromise(
          getTable(tableName, withFilterFn).del(obj)
        );
      },
      login: function (oauthProvider) {
        if (
          !angular.isDefined(oauthProvider) ||
          VAILD_OAUTH_PROVIDERS.indexOf(oauthProvider) === -1
        ) {
          throw new Error(
      ...
      isLoggedIn: function () {
        return (
          isNotNullOrUndefined(client.currentUser) &&
          isNotNullOrUndefined(storage.loggedInUser)
        );
      },
      invokeApi: function (name, options) {
        ...
        client.invokeApi(name, options).done(
          function (results) {
