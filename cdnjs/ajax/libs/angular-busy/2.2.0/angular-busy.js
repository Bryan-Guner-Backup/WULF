    ...
    return {
      ...
      link: function (scope, element, attrs, fn) {
        ...
        if (
          ...
          angular.isUndefined(options.tracker)
        ) {
          throw new Error(
            "Options for cgBusy directive must be provided (tracker option is required)."
          );
        }

        options.tracker = angular.isArray(options.tracker)
          ? options.tracker
          : [options.tracker];
        ...
        var indicatorTemplateName = options.template
          ? options.template
          : cgBusyTemplateName;

        $http
          .get(indicatorTemplateName, { cache: $templateCache })
          .success(function (indicatorTemplate) {
            options.backdrop =
              typeof options.backdrop === "undefined" ? true : options.backdrop;
            var backdrop = options.backdrop
              ? '<div class="cg-busy cg-busy-backdrop"></div>'
              : "";

            var template =
              '<div class="cg-busy cg-busy-animation ng-hide" ng-show="isActive()">' +
              backdrop +
              indicatorTemplate +
              "</div>";
            ...
            angular
              .element(templateElement.children()[options.backdrop ? 1 : 0])
