  ...
  .directive("rnCarouselInfinite", [
    ...
    function ($parse, $compile) {
      return {
        ...
        scope: true,
        template:
          "<ul rn-carousel rn-carousel-buffered><li ng-transclude></li></ul>",
          ...
          return function (scope, iElement, iAttrs) {
            ...
            scope.$watchCollection(
              ...
              function (newValue) {
                ...
                $parse(iAttrs.rnCarouselCurrent).assign(
                  scope.$parent,
      ...
      */
          ...
          if (!repeatAttribute) {
            ...
            if (liChilds.length < 2) {
              throw new Error(
                "carousel: cannot find the ngRepeat attribute OR no childNodes detected"
              );
            }
            ...
            repeatAttribute.value =
              originalItem + " in carouselCollection.cards " + trackProperty;
          }
          return function (scope, iElement, iAttrs, controller) {
            ...
            var carousel = iElement.wrap(
                "<div id='" +
                  carouselId +
                  "' class='rn-carousel-container'></div>"
              ),
            ...
            function transitionEndCallback(event) {
              ...
              if (
                ...
                carousel.css(
                  translateSlideProperty(
                    getTransformCoordinates(carousel[0]),
                    true
                  )
                );
              }
            ...
            function checkEdges() {
              ...
              if (
                position === lastIndex &&
                angular.isDefined(iAttrs.rnCarouselNext)
              ) {
                slides = $parse(iAttrs.rnCarouselNext)(scope, {
                  item: scope.carouselCollection.cards[
                    scope.carouselCollection.cards.length - 1
                  ],
                });
            ...
            scope.carouselCollection =
              CollectionManager.create(collectionParams);

            scope.$watch(
              "carouselCollection.updated",
            ...
            if (angular.isDefined(iAttrs.rnCarouselIndicator)) {
              var indicator = $compile(
                "<div id='" +
                  carouselId +
                  "-indicator' index='carouselCollection.index' items='carouselCollection.items' data-rn-carousel-indicators class='rn-carousel-indicator'></div>"
              )(scope);
            ...
            function updateSlidePosition(forceSkipAnimation) {
              ...
              if (containerWidth === 0) updateContainerWidth();
              offset =
                scope.carouselCollection.getRelativeIndex() * -containerWidth;
              if (skipAnimation === true) {
                ...
                carousel
                  .removeClass("rn-carousel-noanimate")
            ...
            function swipeEnd(coords) {
              ...
              if (swiping > 1) {
                var lastIndex = scope.carouselCollection.getLastIndex(),
                  ...
                  tmpSlideIndex = Math.min(
                    Math.max(0, position + slideOffset),
                    lastIndex
                  );
            ...
            var lastMove = null,
              ...
              moveDelay =
                $window.jasmine || $window.navigator.platform == "iPad"
                  ? 0
                  : 50;
            ...
            $swipe.bind(carousel, {
              ...
              move: function (coords) {
                ...
                if (swiping === 1 && deltaX !== 0) {
                  ...
                  var ratio = 1;
                  if (
                    (position === 0 && coords.x > startX) ||
                    (position === lastIndex && coords.x < startX)
                  )
                    ratio = 3;
  ...
  .service("CollectionManager", [
    function () {
      ...
      CollectionManager.prototype.goTo = function (position, delayedUpdate) {
        ...
        var realIndex =
          (this.position - this.cycleOffset + this.length()) % this.length();
      ...
      CollectionManager.prototype.prev = function () {
        ...
        if (this.cycle) {
          ...
          var prevIndex =
            this.length() > 0
              ? Math.max(0, (this.position - 1) % this.length())
              : 0;
          this.goTo(prevIndex);
        }
      };
      ...
      CollectionManager.prototype.getRelativeIndex = function () {
        var relativeIndex = Math.max(
          0,
          Math.min(this.getLastIndex(), this.position - this.bufferStart)
        );
      ...
      CollectionManager.prototype.adjustBuffer = function () {
        ...
        var maxBufferStart =
          (this.getLastIndex() + 1 - this.bufferSize) % this.length();
        this.log("maxBufferStart", maxBufferStart);
        this.bufferStart = Math.max(
          0,
          Math.min(maxBufferStart, this.position - 1)
        );
        this.cards = this.items.slice(
          this.bufferStart,
