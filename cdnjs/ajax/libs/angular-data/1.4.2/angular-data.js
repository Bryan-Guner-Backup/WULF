      ...
      function (require, module, exports) {
        ...
        function _injectRelations(definition, injected, options) {
          ...
          DS.utils.forEach(definition.relationList, function (def) {
            ...
            if (injected[def.localField]) {
              ...
              try {
                injected[def.localField] = DS.inject(
                  relationName,
                  injected[def.localField],
                  options
                );
              } catch (err) {
                DS.$log.error(
        ...
        function _inject(definition, resource, attrs, options) {
          ...
          function _react(added, removed, changed, oldValueFn, firstTime) {
            ...
            if (
              !DS.utils.isEmpty(added) ||
              !DS.utils.isEmpty(removed) ||
              !DS.utils.isEmpty(changed) ||
              firstTime
            ) {
              item = DS.get(definition.name, innerId);
              resource.modified[innerId] = DS.utils.updateTimestamp(
          ...
          if (DS.utils.isArray(attrs)) {
            ...
            if (!(idA in attrs)) {
              ...
              try {
                ...
                resource.saved[id] = DS.utils.updateTimestamp(
                  resource.saved[id]
                );
                resource.modified[id] =
                  initialLastModified &&
                  resource.modified[id] === initialLastModified
                    ? DS.utils.updateTimestamp(resource.modified[id])
                    : resource.modified[id];
