            ...
            element
              .bind("drop", function (event) {
                ...
                scope.$emit(
                  "file:add",
                  dataTransfer.files,
                  scope.$eval(attributes.ngFileDrop)
                );
              })
      ...
      link: function (scope, element, attributes) {
        ...
        element.bind("change", function () {
          scope.$emit(
            ...
            scope.$eval(attributes.ngFileSelect)
          );
    ...
    function ($compile, $rootScope, $http) {
      ...
      Uploader.prototype = {
        ...
        addToQueue: function (items, options) {
          ...
          angular.forEach(
            "length" in items ? items : [items],
            function (item) {
              var isValid = !this.filters.length
                ? true
                : this.filters.every(function (filter) {
        ...
        removeFromQueue: function (value) {
          var index = angular.isObject(value)
            ? this.getIndexOfItem(value)
            : value;
        ...
        _xhrTransport: function (item) {
          ...
          xhr.addEventListener(
            ...
            function () {
              ...
              xhr.status !== 200 &&
                that.trigger("in:error", xhr, item, response);
      ...
      function Item(params) {
        ...
        if (angular.isElement(params.file)) {
          ...
          var iframe = angular.element(
            '<iframe name="iframeTransport' + +new Date() + '">'
          );
          var value = input.val();
