...
(function (root, factory) {
  ...
  angular.module("formly").directive("formlyCustomValidation", [
    "formlyUtil",
    function (formlyUtil) {
      "use strict";
      ...
      return {
        ...
        link: function (scope, el, attrs, ctrl) {
          ...
          angular.forEach(validators, function (validator, name) {
            if (hasValidators) {
              var validatorCollection = validator.isAsync
                ? "$asyncValidators"
                : "$validators";
      ...
      return {
        ...
        controller: function fieldController($scope, $interval) {
          ...
          function runExpressions() {
            ...
            angular.forEach(
              ...
              function runExpression(expression, prop) {
                if (prop !== "data") {
                  field[prop] = formlyUtil.formlyEval(
                    $scope,
                    expression,
                    currentValue
                  );
                  ...
                  angular.forEach(
                    field.expressionProperties.data,
                    function runExpression(dataExpression, dataProp) {
                      field.data[dataProp] = formlyUtil.formlyEval(
                        $scope,
                        dataExpression,
                        currentValue
                      );
          ...
          function setFormControl() {
            ...
            var interval = $interval(function () {
              iterations++;
              if (
                !angular.isDefined($scope.options.key) &&
        ...
        link: function fieldLink($scope, $element) {
          ...
          if (templateOptions === 0) {
            ...
            formlyUtil.throwErrorWithField(
          ...
          var template =
            ...
            formlyConfig.getTemplate($scope.options.type);
          if (template) {
            setElementTemplate(template);
          } else {
            var templateUrl =
              $scope.options.templateUrl ||
              formlyConfig.getTemplateUrl($scope.options.type);
            if (templateUrl) {
              $http
                .get(templateUrl, {
                  cache: $templateCache,
  ...
  angular.module("formly").factory("formlyUtil", [
    "formlyConfig",
