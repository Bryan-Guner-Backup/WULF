    ...
    return {
      ...
      controller: [
        ...
        function ($scope, $element, $timeout) {
          ...
          if ($scope.viewScaleFactor === undefined)
            $scope.viewScaleFactor = 0.1;
          ...
          $scope.$watch(
            "viewScale+viewScaleFactor+firstDayOfWeek",
          ...
          $scope.$watch(
            "weekendDays+showWeekends+workHours+showNonWorkHours",
            function (newValue, oldValue) {
              if (!angular.equals(newValue, oldValue)) {
          ...
          $scope.updateTaskBounds = function (task) {
            ...
            task.width =
              Math.round((calculateTaskPos(cTo[0], task) - task.left) * 10) /
              10;
          ...
          var calculateTaskPos = function (column, task) {
            return (
              Math.round(
                (column.left +
                  column.width *
                    getTaskPrecisionFactor(task.from, $scope.taskPrecision)) *
                  10
              ) / 10
            );
          };
          ...
          var getTaskPrecisionFactor = function (date, precision) {
            return $scope.viewScale === "hour"
              ? Math.round((date.getMinutes() / 60) * precision) / precision
              : Math.round((date.getHours() / 24) * precision) / precision;
          };
          ...
          $scope.updateColumnBounds = function () {
            ...
            for (i = 0, l = $scope.gantt.columns.hours.length; i < l; i++) {
              ...
              currentDay = setColumnBound(
                $scope.gantt.columns.days,
                currentDay,
                hourColumn.width,
                currentDay > -1
                  ? $scope.gantt.columns.days[currentDay].date.getDate() !==
                      hourColumn.date.getDate()
                  : true
              );

              // Set month column left and width (if not already)
              currentWeek = setColumnBound(
                $scope.gantt.columns.weeks,
                currentWeek,
                hourColumn.width,
                currentWeek > -1
                  ? $scope.gantt.columns.weeks[currentWeek].week !==
                      df.getWeek(hourColumn.date)
                  : true
              );

              // Set month column left and width (if not already)
              currentMonth = setColumnBound(
                $scope.gantt.columns.months,
                currentMonth,
                hourColumn.width,
                currentMonth > -1
                  ? $scope.gantt.columns.months[
                      currentMonth
                    ].date.getMonth() !== hourColumn.date.getMonth()
                  : true
              );
            }
          };
          ...
          var setColumnBound = function (cols, curIndex, width, goToNextCol) {
            ...
            if (col === undefined || goToNextCol) {
              ...
              col.left =
                Math.round(
                  (curIndex > 0
                    ? cols[curIndex - 1].left + cols[curIndex - 1].width
                    : 0) * 10
                ) / 10;
              col.width = width;
            } else {
          ...
          $scope.autoExpandColumns = function (el, date, direction) {
            ...
            var oldScrollLeft =
              el.scrollLeft === 0
                ? ((to - from) * el.scrollWidth) /
                  ($scope.gantt.columns.getLast().date -
                    $scope.gantt.columns.getFirst().date)
                : el.scrollLeft;
          ...
          $scope.raiseRowClicked = function (e, row) {
            var emPxFactor =
              $scope.ganttScroll.children()[0].offsetWidth /
              $scope.ganttInnerWidth;
            var clickedColumn = $scope.calcClosestColumns(
              $scope.getOffset(e).x / emPxFactor,
              function (c) {
                return c.left;
              }
            )[0];
            $scope.onRowClicked({
              event: {
                row: row.clone(),
                column: clickedColumn.clone(),
                date: df.clone(clickedColumn.date),
              },
            });
            ...
            if (direction !== null) {
              ...
              $timeout(
                function () {
                  ...
                  $scope.onScroll({
                    event: { date: date, direction: direction },
                  });
                },
                500,
                true
          ...
          $scope.raiseTaskClicked = function (e, row, task) {
            ...
            $scope.onTaskClicked({
              event: { row: rClone, task: rClone.tasksMap[task.id] },
            });
          ...
          $scope.setData = function (data) {
            var el = $scope.ganttScroll[0];
            var oldDateRange =
              $scope.gantt.columns.hours.length > 0
                ? $scope.gantt.columns.getLast().date -
                  $scope.gantt.columns.getFirst().date
                : 1;
            ...
            if ($scope.fromDate || $scope.toDate) {
              var firstDate = $scope.fromDate
                ? $scope.fromDate
                : $scope.gantt.columns.getFirst().date;
              var lastDate = $scope.toDate
                ? $scope.toDate
                : $scope.gantt.columns.getLast().date;
            ...
            el.scrollLeft =
              el.scrollLeft === 0 && $scope.gantt.columns.hours.length > 0
                ? (($scope.gantt.columns.getLast().date -
                    $scope.gantt.columns.getFirst().date) *
                    oldWidth) /
                    oldDateRange -
    ...
    var Gantt = function (
      ...
      var EmptyColumns = function () {
        ...
        self.prepend = function (columns) {
          ...
          if (
            columns.weeks.length > 0 &&
            self.weeks[0].week === columns.weeks[columns.weeks.length - 1].week
          ) {
            columns.weeks.splice(columns.weeks.length - 1, 1);
          }
          if (
            columns.months.length > 0 &&
            self.months[0].date.getMonth() ===
              columns.months[columns.months.length - 1].date.getMonth()
          ) {
      ...
      self.expandColumns = function (from, to) {
        ...
        if (self.columns.hours.length === 0) {
          ...
          if (from < first.date) {
            generateColumns(
              from,
              df.addDays(df.setTimeZero(first.date, true), -1)
            );
      ...
      var generateColumns = function (from, to) {
        ...
        if (
          self.columns.hours.length === 0 ||
          from > self.columns.getFirst().date
        ) {
        ...
        while (to - date >= 0) {
          ...
          for (var i = 0; i < 24; i++) {
            ...
            if (
              ((isWeekend && self.showWeekends) || !isWeekend) &&
              ((!isWorkHour && self.showNonWorkHours) || isWorkHour)
            ) {
          ...
          if (hourAdded) {
            ...
            if (
              days.length === 0 ||
              days[days.length - 1].date.getDate() !== date.getDate()
            ) {
              days.push(
                new Column.Day(
                  new Date(
            ...
            if (
              weeks.length === 0 ||
              weeks[weeks.length - 1].week !== currentWeek
            ) {
              weeks.push(
                new Column.Week(
                  new Date(
            ...
            if (
              months.length === 0 ||
              months[months.length - 1].date.getMonth() !== date.getMonth()
            ) {
              months.push(
                new Column.Month(
                  new Date(
...
gantt.directive("ganttSortable", [
  "$document",
  "sortableState",
  function ($document, sortableState) {
        ...
        function ($scope, $element) {
          ...
          var isInDragMode = function () {
            return (
              ...
              !angular.equals($scope.row, sortableState.startRow)
            );
          };
...
gantt.directive("ganttTooltip", [
  ...
  "$timeout",
  "$document",
  function (dateFilter, $timeout, $document) {
          ...
          $scope.mouseEnter = function (e) {
            ...
            $timeout(
              function () {
                ...
                if (
                  e.clientX + elTip[0].offsetWidth >
                  $scope.getViewPortWidth()
                ) {
                  $scope.css.left =
                    e.clientX + 20 - elTip[0].offsetWidth + "px";
