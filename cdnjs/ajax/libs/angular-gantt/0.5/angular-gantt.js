      ...
      controller: [
        ...
        function ($scope, $element) {
          ...
          $scope.raiseRowClickedEvent = function (row, column, date) {
            $scope.onRowClicked({
              event: { row: row, column: column.clone(), date: date },
            });
          };
  ...
  function (Row, ColumnGenerator, HeaderGenerator, df, bs) {
    ...
    var Gantt = function (
      ...
      self.getColumnByDate = function (date) {
        return bs.get(self.columns, date, function (c) {
          return c.date;
        })[0];
      };
...
gantt.directive("ganttLabelResizable", [
  ...
  function ($document, debounce, mouseOffset) {
    return {
      restrict: "A",
      scope: { width: "=ganttLabelResizable", minWidth: "=resizeMin" },
      controller: [
        "$scope",
        "$element",
        function ($scope, $element) {
...
gantt.directive("ganttTaskMoveable", [
  "$document",
  "$timeout",
  "debounce",
  "dateFunctions",
  "mouseOffset",
  function ($document, $timeout, debounce, df, mouseOffset) {
    return {
      restrict: "A",
      controller: [
        "$scope",
        ...
        function ($scope, $element) {
          ...
          var enableMoveMode = function (mode, e) {
            ...
            moveStartX = mouseOffset.getOffsetForElement(
              ganttBodyElement[0],
              e
            ).x;
...
gantt.directive("ganttTooltip", [
  ...
  function ($timeout, $document, debounce, smartEvent) {
    ...
    return {
      restrict: "E",
      template:
        "<div ng-mouseenter='mouseEnter($event)' ng-mouseleave='mouseLeave($event)'>" +
