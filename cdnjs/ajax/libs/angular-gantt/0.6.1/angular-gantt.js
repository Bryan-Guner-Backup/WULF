    ...
    return {
      ...
      controller: [
        ...
        function ($scope, $element) {
          ...
          $scope.$watch(
            "viewScale+columnWidth+columnSubScale+firstDayOfWeek+weekendDays+showWeekends+workHours+showNonWorkHours",
            function (newValue, oldValue) {
              if (!angular.equals(newValue, oldValue)) {
                ...
                if (!$scope.gantt.reGenerateColumns()) {
                  ...
                  $scope.gantt.expandDefaultDateRange(
                    $scope.fromDate,
                    $scope.toDate
                  );
                }
  ...
  function (Column, df) {
    ...
    var MonthColumnGenerator = function (columnWidth, columnSubScale) {
      ...
      this.generate = function (from, to) {
        ...
        while ((excludeTo && to - date > 0) || (!excludeTo && to - date >= 0)) {
          generatedCols.push(
            new Column.Month(df.clone(date), left, columnWidth, columnSubScale)
          );
          left += columnWidth;
...
gantt.factory("Gantt", [
  ...
  "dateFunctions",
  "binarySearch",
  function (Row, ColumnGenerator, HeaderGenerator, df, bs) {
...
gantt.directive("ganttHorizontalScrollReceiver", [
  "scrollManager",
...
gantt.directive("ganttScrollSender", [
  "scrollManager",
  "$timeout",
  function (scrollManager, $timeout) {
...
gantt.directive("ganttVerticalScrollReceiver", [
  "scrollManager",
  function (scrollManager) {
    // The element with this attribute will scroll at the same time as the scrollSender element
