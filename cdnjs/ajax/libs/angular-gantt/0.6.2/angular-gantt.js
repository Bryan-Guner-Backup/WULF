    ...
    return {
      ...
      controller: [
        ...
        function ($scope, $element) {
          ...
          $scope.$watch(
            "viewScale+autoExpand+taskOutOfRange+width+labelsWidth+columnWidth+columnSubScale+firstDayOfWeek+weekendDays+showWeekends+workHours+showNonWorkHours",
            function (newValue, oldValue) {
              if (!angular.equals(newValue, oldValue)) {
                $scope.gantt.setViewScale(
                  $scope.viewScale,
                  $scope.autoExpand,
                  $scope.taskOutOfRange,
                  $scope.width - $scope.labelsWidth / $scope.getPxToEmFactor(),
                  $scope.columnWidth,
                  $scope.columnSubScale,
                  $scope.firstDayOfWeek,
                  $scope.weekendDays,
                ...
                if (!$scope.gantt.reGenerateColumns()) {
                  ...
                  $scope.gantt.setDefaultDateRange(
                    $scope.fromDate,
                    $scope.toDate
                  );
                }
...
gantt.factory("Gantt", [
  ...
  "dateFunctions",
  "binarySearch",
  function (Row, Timespan, ColumnGenerator, HeaderGenerator, df, bs) {
  ...
  function (df) {
    var Timespan = function (
      ...
      self.updatePosAndSize = function () {
        ...
        self.width =
          Math.round((self.gantt.getPositionByDate(self.to) - self.left) * 10) /
          10;
        ...
        if (self.est !== undefined && self.lct !== undefined) {
          ...
          self.bounds.width =
            Math.round(
              (self.gantt.getPositionByDate(self.lct) - self.bounds.left) * 10
            ) / 10;
        }
      };
        ...
        function ($scope, $element) {
          ...
          var getRowByY = function (y) {
            var rowHeight =
              ...
              $scope.task.row.gantt.rows.length;
