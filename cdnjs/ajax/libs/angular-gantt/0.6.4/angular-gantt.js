...
gantt.directive("gantt", [
  ...
  "debounce",
  "keepScrollPos",
  function (Gantt, df, mouseOffset, debounce, keepScrollPos) {
    return {
      restrict: "EA",
      replace: true,
      ...
      controller: [
        ...
        function ($scope, $element) {
          ...
          $scope.getPxToEmFactor = function () {
            return (
              $scope.ganttScroll.children()[0].offsetWidth / $scope.gantt.width
            );
          };
          ...
          $scope.scrollToDate = function (date) {
            ...
            if (column !== undefined) {
              var x =
                (column.left + column.width / 2) * $scope.getPxToEmFactor();
              $scope.ganttScroll[0].scrollLeft =
                x - $scope.ganttScroll[0].offsetWidth / 2;
            }
          };

          $scope.autoExpandColumns = keepScrollPos(
            $scope,
            function (el, date, direction) {
              if (
              ...
              if (direction === "left") {
                from =
                  $scope.viewScale === "hour"
                    ? df.addDays(date, -expandHour, true)
                    : df.addDays(date, -expandDay, true);
                to = date;
              } else {
                from = date;
                to =
          ...
          $scope.raiseColumnDateDblClickedEvent = function (evt, column) {
            $scope.onColumnDateDblClicked({
              event: { evt: evt, column: column },
            });
          };

          $scope.raiseColumnDateContextMenuEvent = function (evt, column) {
            $scope.onColumnDataContextClicked({
              event: { evt: evt, column: column },
            });
          };
          ...
          $scope.raiseRowAddedEvent = function (row, userTriggered) {
            $scope.onRowAdded({
              event: { row: row, userTriggered: userTriggered },
            });
          };
          ...
          $scope.raiseTaskUpdatedEvent = function (task, userTriggered) {
            $scope.onTaskUpdated({
              event: { task: task, userTriggered: userTriggered },
            });
          };

          $scope.raiseTaskMoveStartEvent = function (task) {
            $scope.onTaskMoveBegin({
              event: { task: task, userTriggered: true },
            });
          };
          ...
          $scope.raiseTaskResizeStartEvent = function (task) {
            $scope.onTaskResizeBegin({
              event: { task: task, userTriggered: true },
            });
          };

          $scope.raiseTaskResizeEndEvent = function (task) {
            $scope.onTaskResizeEnd({
              event: { task: task, userTriggered: true },
            });
          };
          ...
          $scope.raiseTaskClickedEvent = function (evt, task) {
            var x = mouseOffset.getOffset(evt).x,
              ...
              clickedColumn = $scope.gantt.getColumnByPosition(
                xInEm + task.left
              ),
          ...
          $scope.raiseTaskDblClickedEvent = function (evt, task) {
            $scope.onTaskDblClicked({
              event: { evt: evt, task: task, userTriggered: true },
            });
          };

          $scope.raiseTaskContextMenuEvent = function (evt, task) {
            $scope.onTaskContextClicked({
              event: { evt: evt, task: task, userTriggered: true },
            });
          };
          ...
          $scope.raiseTimespanAddedEvent = function (timespan, userTriggered) {
            $scope.onTimespanAdded({
              event: { timespan: timespan, userTriggered: userTriggered },
            });
          };

          $scope.raiseTimespanUpdatedEvent = function (
            timespan,
            userTriggered
          ) {
            $scope.onTimespanUpdated({
              event: { timespan: timespan, userTriggered: userTriggered },
            });
          };
...
gantt.factory("Column", [
  ...
  function (df) {
    ...
    var calcDbyP = function (column, maxDateValue, currentPosition) {
      return (
        Math.round(
          ((maxDateValue / column.width) * currentPosition) /
            (maxDateValue / column.subScale)
        ) *
        (maxDateValue / column.subScale)
      );
    };
    ...
    var calcPbyD = function (column, date, maxDateValue, currentDateValue) {
      ...
      if (date - column.date > 0 && !column.containsDate(date)) {
        ...
        factor =
          Math.round((currentDateValue / maxDateValue) * column.subScale) /
          column.subScale;
      }
    ...
    var WeekColumn = function (date, left, width, subScale, firstDayOfWeek) {
      ...
      var firstDayIs0 = function (day) {
        return (
          (column.daysInWeek - column.firstDayOfWeek + day) % column.daysInWeek
        );
      };
      ...
      column.getDateByPosition = function (position) {
        ...
        df.setToDayOfWeek(
          res,
          day !== 7 ? firstDayIsSunday(day) : firstDayIsSunday(day) + 7,
          false,
          direction
        );
        return res;
    ...
    var DayColumn = function (
      date,
      left,
      width,
      subScale,
      isWeekend,
      daysToNextWorkingDay,
      daysToPrevWorkingDay,
      workHours,
      showNonWorkHours
    ) {
      var column = new Column(date, left, width, subScale);
    ...
    var HourColumn = function (
      ...
      isWeekend,
      isWorkHour,
      hoursToNextWorkingDay,
      hoursToPrevWorkingDay
    ) {
      var column = new Column(date, left, width, subScale);
      column.isWeekend = isWeekend;
      ...
      column.containsDate = function (date) {
        return (
          date.getDate() === column.date.getDate() &&
          date.getHours() === column.date.getHours()
        );
      };
...
gantt.factory("ColumnGenerator", [
  ...
  function (Column, df) {
    ...
    var HourColumnGenerator = function (
      width,
      columnWidth,
      columnSubScale,
      weekendDays,
      ...
      this.generate = function (from, to, maximumWidth, leftOffset, reverse) {
        ...
        while (true) {
          if (
            maximumWidth &&
            Math.abs(left) > maximumWidth + columnWidth * 24
          ) {
            break;
          }
          ...
          for (var i = 0; i < 24; i++) {
            ...
            if (
              ((isWeekend && showWeekends) || !isWeekend) &&
              ((!isWorkHour && showNonWorkHours) || isWorkHour)
            ) {
              var hoursToNextWorkingDay = 1;
              var hoursToPrevWorkingDay = 1;
              if (!showNonWorkHours) {
                //hours to next/prev working day is only relevant if non-work hours are hidden
                hoursToNextWorkingDay = getHoursToNextWorkingDay(
                  workHoursMap,
                  cDate.getHours()
                );
                hoursToPrevWorkingDay = getHoursToPreviousWorkingDay(
                  workHoursMap,
                  cDate.getHours()
                );
              }
      ...
      this.columnExpandNecessary = function (
        firstColDate,
        lastColDate,
        newFromDate,
        newToDate
        ...
        return (
          firstColDate > newFromDate ||
          lastColDate < df.setTimeZero(newToDate, true)
        );
      };
    ...
    var DayColumnGenerator = function (
      width,
      columnWidth,
      columnSubScale,
      weekendDays,
      ...
      this.generate = function (from, to, maximumWidth, leftOffset, reverse) {
        ...
        while (true) {
          ...
          if ((isWeekend && showWeekends) || !isWeekend) {
            ...
            if (!showWeekends) {
              //days to next/prev working day is only relevant if weekends are hidden
              daysToNextWorkingDay = getDaysToNextWorkingDay(
                weekendDaysMap,
                date.getDay()
              );
              daysToPreviousWorkingDay = getDaysToPrevWorkingDay(
                weekendDaysMap,
                date.getDay()
              );
            }
    ...
    var WeekColumnGenerator = function (
      width,
      columnWidth,
      columnSubScale,
      firstDayOfWeek
      ...
      this.generate = function (from, to, maximumWidth, leftOffset, reverse) {
        ...
        var excludeTo = false;
        from = df.setToDayOfWeek(
          df.setTimeZero(from, true),
          firstDayOfWeek,
          false
        );
        if (to) {
          excludeTo = isToDateToExclude(to);
          to = df.setToDayOfWeek(
            df.setTimeZero(to, true),
            firstDayOfWeek,
            false
          );
        }
      ...
      this.columnExpandNecessary = function (
        ...
        return (
          firstColDate > newFromDate ||
          lastColDate <
            df.setToDayOfWeek(df.setTimeZero(newToDate, true), firstDayOfWeek)
        );
      };
    ...
    var MonthColumnGenerator = function (width, columnWidth, columnSubScale) {
      ...
      this.columnExpandNecessary = function (
        ...
        return (
          firstColDate > newFromDate ||
          lastColDate < df.setToFirstDayOfMonth(df.setTimeZero(newToDate, true))
        );
      };
...
gantt.factory("Gantt", [
  ...
  "dateFunctions",
  "binarySearch",
  function ($filter, Row, Timespan, ColumnGenerator, HeaderGenerator, df, bs) {
    ...
    var Gantt = function ($scope) {
      ...
      $scope.$watch(
        "viewScale+width+labelsWidth+columnWidth+columnSubScale+firstDayOfWeek+weekendDays+showWeekends+workHours+showNonWorkHours",
      ...
      self.buildGenerators = function () {
        switch ($scope.viewScale) {
          case "hour":
            self.columnGenerator = new ColumnGenerator.HourGenerator(
              $scope.width,
              $scope.columnWidth,
              $scope.columnSubScale,
              $scope.weekendDays,
              $scope.showWeekends,
              $scope.workHours,
              $scope.showNonWorkHours
            );
            break;
          case "day":
            self.columnGenerator = new ColumnGenerator.DayGenerator(
              $scope.width,
              $scope.columnWidth,
              $scope.columnSubScale,
              $scope.weekendDays,
            ...
            break;
          case "week":
            self.columnGenerator = new ColumnGenerator.WeekGenerator(
              $scope.width,
              $scope.columnWidth,
              $scope.columnSubScale,
            ...
            break;
          case "month":
            self.columnGenerator = new ColumnGenerator.MonthGenerator(
              $scope.width,
              $scope.columnWidth,
              $scope.columnSubScale
            );
      ...
      var expandColumns = function () {
        ...
        if (self.columns.length === 0) {
          ...
          self.columnGenerator.columnExpandNecessary(
            self.getFirstColumn().date,
            self.getLastColumn().date,
            dateRange.from,
            dateRange.to
          )
        ) {
          var minFrom =
            self.getFirstColumn().date > dateRange.from
              ? dateRange.from
              : self.getFirstColumn().date;
          var maxTo =
            self.getLastColumn().date < dateRange.to
              ? dateRange.to
              : self.getLastColumn().date;
  ...
  function (Column, df) {
    ...
    var generateDayHeader = function (columns) {
      ...
      for (var i = 0, l = columns.length; i < l; i++) {
        ...
        if (i === 0 || columns[i - 1].date.getDay() !== col.date.getDay()) {
          header = new Column.Day(
            df.clone(col.date),
            col.left,
            col.width,
            col.isWeekend,
            col.daysToNextWorkingDay,
            col.daysToPrevWorkingDay
          );
          generatedHeaders.push(header);
        } else {
          header.width += col.width;
    ...
    var generateWeekHeader = function (columns) {
      ...
      for (var i = 0, l = columns.length; i < l; i++) {
        ...
        if (
          i === 0 ||
          df.getWeek(columns[i - 1].date) !== df.getWeek(col.date)
        ) {
          header = new Column.Week(
            df.clone(col.date),
            col.left,
            col.width,
            df.getWeek(col.date)
    ...
    return {
      instance: function ($scope) {
        this.generate = function (columns) {
          var headers = {};
          if (
            $scope.headerShowHour &&
            ["hour"].indexOf($scope.viewScale) > -1
          ) {
            headers.hour = generateHourHeader(columns);
          }
          if (
            $scope.headerShowDay &&
            ["hour", "day"].indexOf($scope.viewScale) > -1
          ) {
            headers.day = generateDayHeader(columns);
          }
          if (
            $scope.headerShowWeek &&
            ["week"].indexOf($scope.viewScale) > -1
          ) {
            headers.week = generateWeekHeader(columns);
          }
          if (
            $scope.headerShowMonth &&
            ["week", "month"].indexOf($scope.viewScale) > -1
          ) {
        ...
        if (taskId in self.tasksMap) {
          ...
          for (var i = 0, l = self.tasks.length; i < l; i++) {
            ...
            if (task.id === taskId) {
              ...
              if (
                self.minFromDate - task.from === 0 ||
                self.maxToDate - task.to === 0
              ) {
                self.setTasksMinMaxDate();
              }
  ...
  function (df) {
    var Timespan = function (
      ...
      self.copy = function (timespan) {
        ...
        self.est =
          timespan.est !== undefined ? df.clone(timespan.est) : undefined;
        self.lct =
          timespan.lct !== undefined ? df.clone(timespan.lct) : undefined;
    ...
    return function (input, scroll_left, scroll_width, $scope) {
      ...
      for (var i = 0, l = input.length; i < l; i++) {
        ...
        if (task.isMoving) {
          ...
          if (
            task.to > $scope.$parent.gantt.getFirstColumn().date &&
            task.from < $scope.$parent.gantt.getLastColumn().getEndDate()
          ) {
            // If task has a visible part on the screen
            if (
              (task.left >= scroll_left &&
                task.left <= scroll_left + scroll_width) ||
              (task.left + task.width >= scroll_left &&
                task.left + task.width <= scroll_left + scroll_width) ||
              (task.left < scroll_left &&
                task.left + task.width > scroll_left + scroll_width)
            ) {
              res.push(task);
...
gantt.directive("ganttLabelsResize", [
  ...
  "debounce",
  "mouseOffset",
  function ($document, debounce, mouseOffset) {
    return {
      restrict: "A",
      scope: {
        ...
        function ($scope, $element) {
          ...
          $scope.getCss = function () {
            if ($scope.task.hasBounds()) {
              ...
              if ($scope.task.isMilestone === true || $scope.task.width === 0)
                css.left =
                  $scope.task.bounds.left - ($scope.task.left - 0.3) + "em";
          ...
          $scope.getClass = function () {
            if ($scope.task.est === undefined || $scope.task.lct === undefined)
              ...
              return "gantt-task-bounds-out";
            else return "gantt-task-bounds-in";
          };
        ...
        function ($scope, $element) {
          ...
          var getRowByY = function (y) {
            if (
              ...
              var rowHeight =
                ganttBodyElement[0].offsetHeight /
                $scope.task.row.gantt.visibleRows.length;
          ...
          var enableMoveMode = function (mode, x) {
            ...
            if (!$scope.task.isMoving) {
              if (mode === "M") $scope.raiseTaskMoveStartEvent($scope.task);
              else $scope.raiseTaskResizeStartEvent($scope.task);
            }
          ...
          var disableMoveMode = function () {
            ...
            else $scope.raiseTaskResizeEndEvent($scope.task);
      ...
      return function () {
        if ($scope.ganttScroll) {
          ...
          left =
            left === undefined
              ? 0
              : $scope.gantt.getColumnByDate(left.date).left * pxToEmFactor;
