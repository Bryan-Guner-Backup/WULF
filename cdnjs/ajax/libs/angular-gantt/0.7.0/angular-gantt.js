      ...
      controller: [
        ...
        function ($scope, $element) {
          ...
          $scope.getHeaderFormat = function (unit) {
            ...
            if (format === undefined) {
              if (
                ["millisecond", "second", "minute", "hour"].indexOf(
                  $scope.viewScale
                ) > -1
              ) {
                format = defaultDayHeadersFormats[unit];
              } else if (
                ["month", "quarter", "year"].indexOf($scope.viewScale) > -1
              ) {
          ...
          $scope.scrollToDate = function (date) {
            ...
            if (position !== undefined) {
              $scope.template.scrollable.$element[0].scrollLeft =
                position -
                $scope.template.scrollable.$element[0].offsetWidth / 2;
            }
          };
          ...
          $scope.autoExpandColumns = function (el, date, direction) {
            ...
            if (direction === "left") {
              from =
                $scope.viewScale === "hour"
                  ? moment(date).add(-expandHour, "day")
                  : moment(date).add(-expandDay, "day");
              to = date;
            } else {
              from = date;
              to =
  ...
  function ($filter) {
    ...
    var Calendar = function () {
          ...
          if (dateFrame !== undefined) {
            angular.forEach(dateFrame.targets, function (timeFrameMappingName) {
              var timeFrameMapping =
                self.timeFrameMappings[timeFrameMappingName];
      ...
      self.solve = function (timeFrames, startDate, endDate) {
        ...
        var orderedTimeFrames = $filter("orderBy")(
          timeFrames,
          function (timeFrame) {
        ...
        angular.forEach(orderedTimeFrames, function (timeFrame) {
          ...
          angular.forEach(solvedTimeFrames, function (solvedTimeFrame) {
            if (!treated) {
              if (
                timeFrame.end > solvedTimeFrame.start &&
                timeFrame.start < solvedTimeFrame.end
              ) {
        ...
        solvedTimeFrames = $filter("filter")(
          solvedTimeFrames,
          function (timeFrame) {
            return (
  ...
  function (moment) {
    ...
    var Column = function (
      ...
      if (
        ...
        (self.timeFramesNonWorkingMode !== "hidden" ||
          self.timeFramesWorkingMode !== "hidden")
      ) {
        ...
        while (cDate < self.endDate) {
          ...
          angular.forEach(
            timeFrames,
            buildPushTimeFrames(cTimeFrames, cDate, nextCDate)
          );
        ...
        angular.forEach(self.timeFrames, function (timeFrame) {
          var positionDuration = timeFrame.start.diff(
            self.date,
            "milliseconds"
          );
          var position = (positionDuration / self.duration) * self.width;

          var timeFrameDuration = timeFrame.end.diff(
            timeFrame.start,
            "milliseconds"
          );
          var timeFramePosition =
            (timeFrameDuration / self.duration) * self.width;
          ...
          if (timeFrame.working && self.timeFramesWorkingMode !== "visible") {
            ...
            !timeFrame.working &&
            self.timeFramesNonWorkingMode !== "visible"
          ) {
            hidden = true;
          }
        ...
        if (
          self.timeFramesNonWorkingMode === "cropped" ||
          self.timeFramesWorkingMode === "cropped"
        ) {
          ...
          if (timeFramesWidth !== self.width) {
            ...
            angular.forEach(self.timeFrames, function (timeFrame) {
              if (
                ...
                timeFrame.originalSize.left =
                  (timeFrame.originalSize.left - originalCroppedWidth) *
                  croppedRatio;
                timeFrame.originalSize.width =
                  timeFrame.originalSize.width * croppedRatio;
      ...
      self.getMagnetDate = function (date) {
        if (self.columnMagnetValue > 0 && self.columnMagnetUnit !== undefined) {
          ...
          var magnetValue =
            Math.round(value / self.columnMagnetValue) * self.columnMagnetValue;
      ...
      var getDateByPositionUsingTimeFrames = function (timeFrames, position) {
        for (var i = 0; i < timeFrames.length; i++) {
          ...
          if (
            !timeFrame.cropped &&
            position >= timeFrame.left &&
            position <= timeFrame.left + timeFrame.width
          ) {
            var positionDuration =
              (timeFrame.getDuration() / timeFrame.width) *
              (position - timeFrame.left);
            var date = moment(timeFrame.start).add(
              positionDuration,
              "milliseconds"
            );
            return date;
          ...
          for (var i = 0; i < timeFrames.length; i++) {
            ...
            if (
              ...
              if (timeFrame.cropped) {
                ...
                positionDuration = croppedDate.diff(
                  timeFrame.start,
                  "milliseconds"
                );
                position =
                  (positionDuration / timeFrame.getDuration()) *
                  timeFrame.width;
...
gantt.factory("GanttColumnGenerator", [
  "GanttColumn",
  "moment",
    ...
    var ColumnGenerator = function ($scope) {
      ...
      if ($scope.columnMagnet) {
        ...
        if (splittedColumnMagnet.length > 1) {
          ...
          columnMagnetUnit =
            splittedColumnMagnet[splittedColumnMagnet.length - 1];
        }
      }
...
gantt.factory("GanttColumnHeader", [
  "moment",
  "GanttColumn",
...
gantt.factory("Gantt", [
  ...
  function (
    ...
    var Gantt = function ($scope, $element) {
      ...
      $scope.$watch(
        "viewScale+width+labelsWidth+columnWidth+timeFramesWorkingMode+timeFramesNonWorkingMode+columnMagnet",
      ...
      $scope.$watch(
        "fromDate+toDate+autoExpand+taskOutOfRange",
        function (newValue, oldValue) {
          if (!angular.equals(newValue, oldValue)) {
            self.updateColumns();
          }
      ...
      $scope.$watch(
        "ganttElementWidth+labelsWidth+showLabelsColumn+maxHeight",
        function (newValue, oldValue) {
          if (!angular.equals(newValue, oldValue)) {
            updateColumnsMeta();
          }
      ...
      var updateVisibleColumns = function () {
        self.visibleColumns = $filter("ganttColumnLimit")(
          self.columns,
          $scope.scrollLeft,
          $scope.scrollWidth
        );
        ...
        angular.forEach(self.headers, function (headers, key) {
          if (self.headers.hasOwnProperty(key)) {
            self.visibleHeaders[key] = $filter("ganttColumnLimit")(
              headers,
              $scope.scrollLeft,
              $scope.scrollWidth
            );
      ...
      var updateVisibleRows = function () {
        ...
        if ($scope.filterRow) {
          self.filteredRows = $filter("filter")(
            self.rows,
            $scope.filterRow,
            $scope.filterRowComparator
          );
        } else {
      ...
      var setScrollAnchor = function () {
        if (
          $scope.template.scrollable &&
          $scope.template.scrollable.$element &&
      ...
      var updateColumnsMeta = function () {
        ...
        self.originalWidth =
          lastColumn !== undefined
            ? lastColumn.originalSize.left + lastColumn.originalSize.width
            : 0;

        if ($scope.columnWidth === undefined) {
          var newWidth =
            $scope.ganttElementWidth -
            ($scope.showLabelsColumn ? $scope.labelsWidth : 0);
        ...
        self.width =
          lastColumn !== undefined ? lastColumn.left + lastColumn.width : 0;

        if (self._currentDate !== undefined) {
          self.setCurrentDate(self._currentDate);
        }
        $scope.currentDatePosition = self.getPositionByDate(
          $scope.currentDateValue
        );
      ...
      var expandExtendedColumnsForPosition = function (x) {
        if (x < 0) {
          ...
          if (!firstExtendedColumn || firstExtendedColumn.left > x) {
            self.previousColumns = self.columnGenerator.generate(
              from,
              undefined,
              -x,
              0,
          ...
          if (
            !lastExtendedColumn ||
            lastExtendedColumn.left + lastExtendedColumn.width < x
          ) {
            self.nextColumns = self.columnGenerator.generate(
              endDate,
              undefined,
              x - self.width,
              self.width,
      ...
      self.getColumnByDate = function (date) {
        ...
        var extendedColumns = self.previousColumns.concat(
          self.columns,
          self.nextColumns
        );
      ...
      var addTimespan = function (timespanData) {
        ...
        if (timespanData.id in self.timespansMap) {
          ...
          timespan = new Timespan(
            ...
            timespanData.name,
            timespanData.color,
            timespanData.classes,
            timespanData.priority,
            timespanData.from,
            timespanData.to,
            timespanData.data
          );
...
gantt.factory("GanttHeaderGenerator", [
  "GanttColumnHeader",
      ...
      for (var i = 0, l = columns.length; i < l; i++) {
        ...
        if (i === 0 || columns[i - 1].date.get(unit) !== col.date.get(unit)) {
          ...
          header = new ColumnHeader(
            col.date,
            unit,
            col.originalSize.left,
            col.originalSize.width,
            label
          );
...
gantt.factory("GanttRow", [
  "GanttTask",
  "moment",
  "$filter",
  "GANTT_EVENTS",
  function (Task, moment, $filter, GANTT_EVENTS) {
    var Row = function (id, gantt, name, order, data) {
      ...
      self.moveTaskToRow = function (task) {
        ...
        self.gantt.$scope.$emit(GANTT_EVENTS.TASK_MOVED, {
          oldRow: oldRow,
          task: task,
        });
      };
      ...
      self.updateVisibleTasks = function () {
        if (gantt.$scope.filterTask) {
          self.filteredTasks = $filter("filter")(
            self.tasks,
            gantt.$scope.filterTask,
            gantt.$scope.filterTaskComparator
          );
        } else {
          self.filteredTasks = self.tasks;
        }
        self.visibleTasks = $filter("ganttTaskLimit")(
          self.filteredTasks,
          self.gantt
        );
      };
      ...
      self.removeTask = function (taskId, disableEmit) {
        if (taskId in self.tasksMap) {
          ...
          if (!disableEmit) {
            self.gantt.$scope.$emit(GANTT_EVENTS.TASK_REMOVED, {
              task: removedTask,
            });
          }
...
gantt.factory("GanttTask", [
  "moment",
  "GanttTaskProgress",
  function (moment, TaskProgress) {
    var Task = function (
      id,
      row,
      name,
      ...
      if (progress !== undefined) {
        if (typeof progress === "object") {
          self.progress = new TaskProgress(
            self,
            progress.percent,
            progress.color,
            progress.classes
      ...
      self.getFromLabel = function () {
        if (self._fromLabel === undefined) {
          self._fromLabel = self.from.format(
            self.gantt.$scope.tooltipDateFormat
          );
        }
      ...
      self.updatePosAndSize = function () {
        ...
        self.modelWidth =
          self.gantt.getPositionByDate(self.to) - self.modelLeft;

        self.outOfRange =
          self.modelLeft + self.modelWidth < 0 ||
          self.modelLeft > self.gantt.width;
        ...
        if (self.est !== undefined && self.lct !== undefined) {
          ...
          self.bounds.width =
            self.gantt.getPositionByDate(self.lct) - self.bounds.left;
        }
      };
      ...
      self.moveTo = function (x) {
        ...
        self.to = self.gantt.getDateByPosition(
          newTaskLeft + self.modelWidth,
          true
        );
        self._toLabel = undefined;
        self.row.setFromToByTask(self);
...
gantt.factory("GanttTaskProgress", [
  function () {
...
gantt.factory("GanttBodyColumns", [
  function () {
...
gantt.factory("GanttHeaderColumns", [
  function () {
  ...
  function (moment) {
    var Timespan = function (
      ...
      self.copy = function (timespan) {
        ...
        self.est =
          timespan.est !== undefined ? moment(timespan.est) : undefined;
        self.lct =
          timespan.lct !== undefined ? moment(timespan.lct) : undefined;
...
gantt.service("ganttBinarySearch", [
  function () {
...
gantt.directive("ganttLimitUpdater", [
  "$timeout",
  "ganttDebounce",
    ...
    return function (input, gantt) {
      ...
      for (var i = 0, l = input.length; i < l; i++) {
        ...
        if (task.isMoving) {
          ...
          if (
            task.to > gantt.getFirstColumn().date &&
            task.from < gantt.getLastColumn().endDate
          ) {
            ...
            if (
              (task.left >= scrollLeft &&
                task.left <= scrollLeft + scrollWidth) ||
              (task.left + task.width >= scrollLeft &&
                task.left + task.width <= scrollLeft + scrollWidth) ||
              (task.left < scrollLeft &&
                task.left + task.width > scrollLeft + scrollWidth)
            ) {
              res.push(task);
...
gantt.directive("ganttLabelsResize", [
  "$document",
  "ganttDebounce",
  "ganttMouseOffset",
  "GANTT_EVENTS",
  function ($document, debounce, mouseOffset, GANTT_EVENTS) {
    return {
      restrict: "A",
      scope: {
        enabled: "=ganttLabelsResize",
      ...
      controller: [
        ...
        function ($scope, $element) {
          ...
          var disableResizeMode = function () {
            ...
            $scope.$emit(GANTT_EVENTS.ROW_LABELS_RESIZED, {
              width: $scope.width,
            });
          };
        },
...
gantt.directive("ganttRightClick", [
  "$parse",
  function ($parse) {
    return {
      restrict: "A",
...
gantt.directive("ganttRow", [
  "GanttEvents",
  "GANTT_EVENTS",
  function (Events, GANTT_EVENTS) {
    return {
      restrict: "E",
      require: "^ganttBody",
      transclude: true,
      ...
      controller: [
        ...
        function ($scope, $element) {
          ...
          $element.bind("mousedown", function (evt) {
            $scope.$emit(
              GANTT_EVENTS.ROW_MOUSEDOWN,
              Events.buildRowEventData(evt, $element, $scope.row, $scope.gantt)
            );
          });

          $element.bind("mouseup", function (evt) {
            $scope.$emit(
              GANTT_EVENTS.ROW_MOUSEUP,
              Events.buildRowEventData(evt, $element, $scope.row, $scope.gantt)
            );
          });
...
gantt.directive("ganttRowHeader", [
  "GanttEvents",
  "GANTT_EVENTS",
  function (Events, GANTT_EVENTS) {
    return {
    ...
    return {
      ...
      controller: [
        ...
        function ($scope, $element) {
          $element.bind("mousedown", function (evt) {
            $scope.$emit(
              GANTT_EVENTS.ROW_LABEL_MOUSEDOWN,
              Events.buildRowEventData(evt, $element, $scope.row, $scope.gantt)
            );
          });
          ...
          $element.bind("mouseup", function (evt) {
            $scope.$emit(
              GANTT_EVENTS.ROW_LABEL_MOUSEUP,
              Events.buildRowEventData(evt, $element, $scope.row, $scope.gantt)
            );
          });
          ...
          $element.bind("click", function (evt) {
            $scope.$emit(
              GANTT_EVENTS.ROW_LABEL_CLICKED,
              Events.buildRowEventData(evt, $element, $scope.row, $scope.gantt)
            );
          });
          ...
          $element.bind("dblclick", function (evt) {
            $scope.$emit(
              GANTT_EVENTS.ROW_LABEL_DBL_CLICKED,
              Events.buildRowEventData(evt, $element, $scope.row, $scope.gantt)
            );
          });
          ...
          $element.bind("contextmenu", function (evt) {
            $scope.$emit(
              GANTT_EVENTS.ROW_LABEL_CONTEXTMENU,
              Events.buildRowEventData(evt, $element, $scope.row, $scope.gantt)
            );
          });
        },
...
gantt.directive("ganttScrollSender", [
  "$timeout",
  "ganttDebounce",
  "GANTT_EVENTS",
  function ($timeout, debounce, GANTT_EVENTS) {
      ...
      controller: [
        ...
        function ($scope, $element) {
          ...
          var updateListeners = function () {
            ...
            for (
              i = 0, l = $scope.scrollManager.horizontal.length;
              i < l;
              i++
            ) {
...
gantt.directive("ganttScrollable", [
  "GanttScrollable",
  "ganttDebounce",
  "ganttLayout",
  "GANTT_EVENTS",
  function (Scrollable, debounce, layout, GANTT_EVENTS) {
    return {
      restrict: "E",
      transclude: true,
      replace: true,
        ...
        function ($scope, $element) {
          ...
          $element.bind(
            ...
            debounce(function () {
              ...
              if (el.scrollLeft < lastScrollLeft && el.scrollLeft === 0) {
                ...
                el.offsetWidth + el.scrollLeft >= el.scrollWidth - 1
              ) {
                direction = "right";
                date = $scope.gantt.to;
              }
              ...
              if (date !== undefined) {
                ...
                $scope.$emit(GANTT_EVENTS.SCROLL, {
                  left: el.scrollLeft,
                  date: date,
                  direction: direction,
                });
          ...
          $scope.$watch("gantt.columns.length", function (newValue, oldValue) {
            if (
              ...
              newValue > 0 &&
              $scope.gantt.scrollAnchor !== undefined
            ) {
          ...
          $scope.getScrollableCss = function () {
            ...
            if (
              $scope.ganttElementWidth -
                ($scope.showLabelsColumn ? $scope.labelsWidth : 0) >
              $scope.gantt.width + scrollBarWidth
            ) {
            ...
            if ($scope.maxHeight > 0) {
              css["max-height"] =
                $scope.maxHeight - $scope.template.header.getHeight() + "px";
              css["overflow-y"] = "auto";
            } else {
...
gantt.directive("ganttElementWidthListener", [
  function () {
    // Updates the limit filters if the user scrolls the gantt chart
...
gantt.service("ganttSortManager", [
  function () {
...
gantt.directive("ganttSortable", [
  "$document",
  "ganttSortManager",
  function ($document, sortManager) {
    ...
    return {
      ...
      controller: [
        ...
        function ($scope, $element) {
          ...
          $element.bind("mousemove", function (e) {
            if (isInDragMode()) {
              var elementBelowMouse = angular.element(
                $document[0].elementFromPoint(e.clientX, e.clientY)
              );
              var targetRow =
                elementBelowMouse.controller("ngModel").$modelValue;
          ...
          var isInDragMode = function () {
            return (
              ...
              !angular.equals($scope.row, sortManager.startRow)
            );
          };
      ...
      controller: [
        ...
        function ($scope) {
          ...
          $scope.getCss = function () {
            if ($scope.task.hasBounds()) {
              ...
              if ($scope.task.isMilestone === true || $scope.task.width === 0) {
                css.left =
                  $scope.task.bounds.left - ($scope.task.left - 0.3) + "px";
              } else {
          ...
          $scope.getClass = function () {
            if (
              $scope.task.est === undefined ||
              $scope.task.lct === undefined
            ) {
          ...
          $scope.$watch("task.isMouseOver", function () {
            if ($scope.task.hasBounds() && !$scope.task.isMoving) {
              $scope.visible = !(
                $scope.task.isMouseOver === undefined ||
...
gantt.directive("ganttTaskProgress", [
  function () {
    return {
      restrict: "E",
      templateUrl: function (tElement, tAttrs) {
      ...
      controller: [
        ...
        function ($scope, $element) {
          ...
          $element.bind("mousedown", function (evt) {
            $scope.$apply(function () {
              ...
              if (mode !== "" && mouseButton.getButton(evt) === 1) {
                var offsetX = mouseOffset.getOffsetForElement(
                  ganttBodyElement[0],
                  evt
                ).x;
          ...
          $element.bind("click", function (evt) {
            $scope.$apply(function () {
              ...
              if (!taskHasBeenChanged) {
                $scope.$emit(
                  GANTT_EVENTS.TASK_CLICKED,
                  Events.buildTaskEventData(
                    evt,
                    $element,
                    $scope.task,
                    $scope.gantt
          ...
          var getRowByY = function (y) {
            if (
              y >= ganttRowElement[0].offsetTop &&
              y <=
                ganttRowElement[0].offsetTop + ganttRowElement[0].offsetHeight
            ) {
              ...
              var rowHeight =
                ganttBodyElement[0].offsetHeight / visibleRows.length;
          ...
          var getMoveMode = function (e) {
            ...
            if ($scope.allowTaskResizing) {
              distance =
                $element[0].offsetWidth < 10
                  ? resizeAreaWidthSmall
                  : resizeAreaWidthBig;
            }

            if (
              $scope.allowTaskResizing &&
              x > $element[0].offsetWidth - distance
            ) {
              return "E";
            } else if ($scope.allowTaskResizing && x < distance) {
              return "W";
            } else if (
              ($scope.allowTaskMoving || $scope.allowTaskRowSwitching) &&
              x >= distance &&
              x <= $element[0].offsetWidth - distance
            ) {
              return "M";
          ...
          var enableMoveMode = function (mode, x, evt) {
            ...
            if (!$scope.task.isMoving) {
              if (mode === "M") {
                ...
                $scope.$emit(
                  GANTT_EVENTS.TASK_RESIZE_BEGIN,
                  Events.buildTaskEventData(
                    evt,
                    $element,
                    $scope.task,
...
gantt.directive("ganttTooltip", [
  "$timeout",
  "$document",
  "ganttDebounce",
  "ganttSmartEvent",
  function ($timeout, $document, debounce, smartEvent) {
...
gantt.directive("ganttBodyColumns", [
  function () {
    return {
      restrict: "E",
      require: "^ganttBody",
      ...
      controller: [
        ...
        "$element",
        "GanttBodyColumns",
        function ($scope, $element, BodyColumns) {
  ...
  function () {
    return {
      ...
      controller: [
        ...
        "$element",
        "GanttBodyRows",
        function ($scope, $element, BodyRows) {
...
gantt.directive("ganttColumnHeader", [
  "GanttEvents",
  "GANTT_EVENTS",
  function (Events, GANTT_EVENTS) {
    return {
      ...
      controller: [
        ...
        function ($scope, $element) {
          $element.bind("click", function (evt) {
            $scope.$emit(
              GANTT_EVENTS.COLUMN_CLICKED,
              Events.buildColumnEventData(evt, $element, $scope.column)
            );
          });

          $element.bind("dblclick", function (evt) {
            $scope.$emit(
              GANTT_EVENTS.COLUMN_DBL_CLICKED,
              Events.buildColumnEventData(evt, $element, $scope.column)
            );
          });
          ...
          $element.bind("contextmenu", function (evt) {
            $scope.$emit(
              GANTT_EVENTS.COLUMN_CONTEXTMENU,
              Events.buildColumnEventData(evt, $element, $scope.column)
            );
          });
        },
...
gantt.directive("ganttHeaderColumns", [
  function () {
    return {
      restrict: "E",
      require: "^ganttHeader",
      ...
      controller: [
        ...
        "$element",
        "GanttHeaderColumns",
        function ($scope, $element, HeaderColumns) {
...
gantt.directive("ganttTimeFrame", [
  function () {
    return {
      restrict: "E",
      require: "^ganttColumn",
    ...
    function debounce(fn, timeout, invokeApply) {
      ...
      return function () {
        ...
        return $timeout(
          later,
          timeout,
          invokeApply === undefined ? true : invokeApply
        );
      };
    }
...
gantt.service("ganttEnableNgAnimate", [
  "$injector",
  function ($injector) {
    var ngAnimate;
    try {
...
gantt.service("ganttMouseButton", [
  function () {
...
gantt.service("ganttMouseOffset", [
  function () {
...
angular.module("ganttTemplates", []).run([
  "$templateCache",
