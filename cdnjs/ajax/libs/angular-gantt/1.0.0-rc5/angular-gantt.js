    ...
    function (moment) {
      return {
        initialize: function (options) {
          ...
          options.filterTaskComparator =
            options.filterTaskComparator || undefined;

          options.filterRow = options.filterRow || undefined;
          options.filterRowComparator =
            options.filterRowComparator || undefined;
          ...
          options.allowLabelsResizing =
            options.allowLabelsResizing !== undefined
              ? !!options.allowLabelsResizing
              : true;
          ...
          options.showLabelsColumn =
            options.showLabelsColumn !== undefined
              ? !!options.showLabelsColumn
              : true;
          options.showTooltips =
            options.showTooltips !== undefined ? !!options.showTooltips : true;
          ...
          options.timeFramesWorkingMode =
            options.timeFramesWorkingMode || "hidden";
          options.timeFramesNonWorkingMode =
            options.timeFramesNonWorkingMode || "visible";
      ...
      GanttCurrentDateManager.prototype.setCurrentDate = function (
        ...
        if (this.currentDateColumn !== undefined) {
          if (this.currentDateColumn.$element !== undefined) {
            this.currentDateColumn.$element.removeClass(
        ...
        if (this.date !== undefined) {
          ...
          if (column !== undefined) {
            ...
            if (
              this.gantt.$scope.currentDate === "column" &&
              this.currentDateColumn.$element !== undefined
            ) {
              this.currentDateColumn.$element.addClass(
                "gantt-foreground-col-current-date"
              );
            }
          }
      ...
      Column.prototype.getMagnetDate = function (
        ...
        if (magnetValue > 0 && magnetUnit !== undefined) {
          ...
          if (magnetValue < 1) {
            ...
            duration = moment
              .duration(magnetValue, magnetUnit)
              .asMilliseconds();
          ...
          for (var i = 0, l = this.timeFrames.length; i < l; i++) {
            ...
            if (
              !timeFrame.cropped &&
              (previousCropped ||
                (date >= timeFrame.start && date <= timeFrame.end))
            ) {
              if (previousCropped) {
              ...
              if (
                initialDate.diff(timeFrame.start, "milliseconds") < magnetDiff
              ) {
                date = timeFrame.start;
              } else if (
                timeFrame.end.diff(initialDate, "milliseconds") < magnetDiff
              ) {
                date = timeFrame.end;
              }
  ...
  angular.module("gantt").factory("GanttColumnGenerator", [
    ...
    function (Column, moment) {
      var ColumnGenerator = function (columnsManager) {
        ...
        var timeFramesWorkingMode =
          columnsManager.gantt.$scope.timeFramesWorkingMode;
        var timeFramesNonWorkingMode =
          columnsManager.gantt.$scope.timeFramesNonWorkingMode;
...
(function () {
  ...
  angular.module("gantt").factory("GanttColumnsManager", [
    ...
    "ganttLayout",
    "ganttBinarySearch",
    function (ColumnGenerator, HeaderGenerator, $filter, $timeout, layout, bs) {
      var ColumnsManager = function (gantt) {
        ...
        this.gantt.$scope.$watchGroup(
          ["bodyRowsWidth", "labelsWidth", "showLabelsColumn", "maxHeight"],
          function (oldValues, newValues) {
            if (oldValues !== newValues && self.gantt.rendered) {
      ...
      ColumnsManager.prototype.updateColumnsMeta = function () {
        ...
        if (autoFitWidthEnabled) {
          var newWidth =
            this.gantt.getElementWidth() -
            (this.gantt.$scope.showLabelsColumn
              ? this.gantt.labels.getWidth()
              : 0);
        ...
        this.gantt.currentDateManager.setCurrentDate(
          this.gantt.$scope.currentDateValue
        );
      };
        ...
        if (format === undefined) {
          if (
            ["millisecond", "second", "minute", "hour"].indexOf(
              this.gantt.$scope.viewScale
            ) > -1
          ) {
            format = defaultDayHeadersFormats[unit];
          } else if (
            ["month", "quarter", "year"].indexOf(this.gantt.$scope.viewScale) >
            -1
          ) {
      ...
      return function (columnsManager) {
        this.generate = function (columns) {
          ...
          if (columnsManager.gantt.$scope.headers === undefined) {
            units = [];
            if (
              ["year", "quarter", "month"].indexOf(
                columnsManager.gantt.$scope.viewScale
              ) > -1
            ) {
              units.push("year");
            }
            if (
              ["quarter"].indexOf(columnsManager.gantt.$scope.viewScale) > -1
            ) {
              units.push("quarter");
            }
            if (
              ["day", "week", "month"].indexOf(
                columnsManager.gantt.$scope.viewScale
              ) > -1
            ) {
              units.push("month");
            ...
            if (
              ["day", "week"].indexOf(columnsManager.gantt.$scope.viewScale) >
              -1
            ) {
              units.push("week");
            }
            if (
              ["hour", "day"].indexOf(columnsManager.gantt.$scope.viewScale) >
              -1
            ) {
              units.push("day");
            }
            if (
              ["hour", "minute", "second"].indexOf(
                columnsManager.gantt.$scope.viewScale
              ) > -1
            ) {
              units.push("hour");
            ...
            if (
              ["second"].indexOf(columnsManager.gantt.$scope.viewScale) > -1
            ) {
              units.push("second");
            }
            if (units.length === 0) {
    ...
    function (Task, moment, $filter) {
      ...
      Row.prototype.updateVisibleTasks = function () {
        if (this.rowsManager.gantt.$scope.filterTask) {
          ...
          var filterTaskComparator =
            this.rowsManager.gantt.$scope.filterTaskComparator;
          if (typeof filterTaskComparator === "function") {
            filterTaskComparator = function (actual, expected) {
              return this.rowsManager.gantt.$scope.filterRowComparator(
                actual.model,
                expected.model
              );
            };
...
(function () {
  ...
  angular.module("gantt").factory("GanttRowsManager", [
    ...
    "$filter",
    "moment",
    function (Row, arrays, $filter, moment) {
        ...
        if (this.gantt.$scope.filterRow) {
          ...
          if (typeof filterRowComparator === "function") {
            filterRowComparator = function (actual, expected) {
              return this.gantt.$scope.filterRowComparator(
                actual.model,
                expected.model
              );
            };
      ...
      RowsManager.prototype.getExpandedTo = function (to) {
        ...
        if (
          maxRowTo &&
          (!this.gantt.$scope.toDate || maxRowTo > this.gantt.$scope.toDate)
        ) {
          return maxRowTo;
        }
        return to;
    ...
    function () {
      ...
      Task.prototype.updatePosAndSize = function () {
        ...
        if (this.modelLeft < 0) {
          ...
          this.modelWidth + this.modelLeft >
          this.rowsManager.gantt.width
        ) {
          this.truncatedRight = true;
          this.truncatedLeft = false;
      ...
      Task.prototype.moveTo = function (x) {
        this.model.from = this.rowsManager.gantt.getDateByPosition(x, true);
        var newTaskLeft = this.rowsManager.gantt.getPositionByDate(
          this.model.from
        );
        this.model.to = this.rowsManager.gantt.getDateByPosition(
          newTaskLeft + this.modelWidth,
          true
        );
        this.row.setFromToByTask(this);
        this.updatePosAndSize();
...
(function () {
  "use strict";
  angular.module("gantt").factory("GanttLabels", [
    function () {
          ...
          function setWidth(width) {
            ...
            if (oldWidth !== width) {
              ...
              if ($attrs.resizerWidth) {
                $scope.$eval($attrs.resizerWidth + " =  $$xValue", {
                  $$xValue: width,
                });
...
(function () {
  ...
  angular.module("gantt").directive("ganttScrollable", [
    ...
    "ganttLayout",
    "moment",
    function (Builder, debounce, layout, moment) {
      ...
      builder.controller = function ($scope, $element) {
        ...
        $element.bind(
          ...
          debounce(function () {
            ...
            if (date !== undefined) {
              ...
              $scope.gantt.api.scroll.raise.scroll(
                el.scrollLeft,
                date,
                direction
              );
        ...
        $scope.getScrollableCss = function () {
          ...
          if (
            $scope.gantt.width > 0 &&
            $scope.gantt.width -
              ($scope.showLabelsColumn ? $scope.gantt.labels.getWidth() : 0) >
          ...
          if ($scope.maxHeight > 0) {
            css["max-height"] =
              $scope.maxHeight - $scope.gantt.header.getHeight() + "px";
            css["overflow-y"] = "auto";
          } else {
...
(function () {
  "use strict";
  angular.module("gantt").directive("ganttLabels", [
    "GanttDirectiveBuilder",
    function (Builder) {
...
(function () {
  "use strict";
  angular.module("gantt").directive("ganttLabelsBody", [
    "GanttDirectiveBuilder",
    function (Builder) {
...
(function () {
  "use strict";
  angular.module("gantt").directive("ganttLabelsHeader", [
    "GanttDirectiveBuilder",
    function (Builder) {
...
(function () {
  "use strict";
  angular.module("gantt").directive("ganttRowHeader", [
    "GanttDirectiveBuilder",
    function (Builder) {
