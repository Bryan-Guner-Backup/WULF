...
(function () {
  ...
  angular.module("gantt.drawtask", ["gantt"]).directive("ganttDrawTask", [
    ...
    "ganttMouseOffset",
    "moment",
    function (document, mouseOffset, moment) {
      return {
        restrict: "E",
        require: "^gantt",
        scope: {
...
(function () {
  "use strict";
  angular
    .module("gantt.groups", ["gantt", "gantt.groups.templates"])
    .directive("ganttGroups", [
      "ganttUtils",
      "GanttHierarchy",
      "$compile",
      "$document",
      function (utils, Hierarchy, $compile, $document) {
        ...
        return {
          ...
          link: function (scope, element, attrs, ganttCtrl) {
            ...
            ganttCtrl.gantt.api.registerMethod(
              "groups",
              "refresh",
              refresh,
              this
            );
            ganttCtrl.gantt.$scope.$watchCollection(
              "gantt.rowsManager.filteredRows",
              function () {
                refresh();
              }
            );
            ...
            api.directives.on.new(
              ...
              function (directiveName, rowScope, rowElement) {
                if (directiveName === "ganttRow") {
                  ...
                  var taskGroupElement =
                    $document[0].createElement("gantt-task-group");
                  if (attrs.templateUrl !== undefined) {
                    angular
                      .element(taskGroupElement)
                      .attr("data-template-url", attrs.templateUrl);
                  }
                  if (attrs.template !== undefined) {
                    angular
...
(function () {
  "use strict";
  angular
    .module("gantt.labels", ["gantt", "gantt.labels.templates"])
    .directive("ganttLabels", [
      "ganttUtils",
      "$compile",
      "$document",
      "$log",
      function (utils, $compile, $document, $log) {
        // Provides the row sort functionality to any Gantt row
        // Uses the sortableState to share the current row
        ...
        return {
          ...
          link: function (scope, element, attrs, ganttCtrl) {
            ...
            $log.warn(
              "Angular Gantt Labels plugin is deprecated. Please use Table plugin instead."
            );
            ...
            api.directives.on.new(
              scope,
              function (directiveName, sideContentScope, sideContentElement) {
                if (directiveName === "ganttSideContent") {
                  ...
                  var labelsElement = $document[0].createElement(
            ...
            function fitSideWidthToLabels() {
              var labels =
                ganttCtrl.gantt.side.$element[0].getElementsByClassName(
                  "gantt-row-label"
                );
              var newSideWidth = 0;
            ...
            function (directiveName, taskScope, taskElement) {
              if (directiveName === "ganttTask") {
                ...
                var onPressEvents = function (evt) {
                  ...
                  var enabledValue = utils.firstProperty(
                    [taskMovable, rowMovable],
                    "enabled",
                    scope.enabled
                  );
                  var enabled = angular.isFunction(enabledValue)
                    ? enabledValue(evt)
                    : enabledValue;
                  if (enabled) {
                ...
                var moveTask = function (evt) {
                  ...
                  var taskOutOfRange =
                    taskScope.task.row.rowsManager.gantt.options.value(
                      "taskOutOfRange"
                    );
                  ...
                  if (taskScope.task.moveMode === "M") {
                    ...
                    if (allowRowSwitching) {
                      ...
                      var ganttBody = angular.element(
                        $document[0].querySelectorAll(".gantt-body")
                      );
                      ganttBody.css("pointer-events", "auto"); // pointer-events must be enabled for following to work.
                      var targetRowElement = dom.findElementFromPoint(
                        rowCenterLeft,
                        evt.clientY,
                        function (element) {
                  ...
                  if (
                    !oldTaskHasBeenChanged &&
                    taskHasBeenChanged &&
                    !taskHasBeenMovedFromAnotherRow
                  ) {
                    var backgroundElement =
                      taskScope.task.getBackgroundElement();
                    if (taskScope.task.moveMode === "M") {
                ...
                var getMoveMode = function (x) {
                  ...
                  if (allowResizing) {
                    distance =
                      foregroundElement[0].offsetWidth < taskWithSmallWidth
                  ...
                  if (
                    allowResizing &&
                    x > foregroundElement[0].offsetWidth - distance
                  ) {
                    return "E";
...
(function () {
  "use strict";
  angular
    .module("gantt.overlap", ["gantt", "gantt.overlap.templates"])
    .directive("ganttOverlap", [
      "moment",
      function (moment) {
        return {
          restrict: "E",
...
(function () {
  ...
  "use strict";
  angular
    .module("gantt.resizeSensor", ["gantt"])
    .directive("ganttResizeSensor", [
      function () {
        return {
          restrict: "E",
          require: "^gantt",
          ...
          link: function (scope, element, attrs, ganttCtrl) {
            ...
            function buildSensor() {
              var ganttElement = element
                .parent()
                .parent()
                .parent()[0]
                .querySelectorAll("div.gantt")[0];
              return new ResizeSensor(ganttElement, function () {
                ganttCtrl.gantt.$scope.ganttElementWidth =
                  ganttElement.clientWidth;
...
(function () {
  ...
  if (failedDependencies.length > 0) {
          ...
          link: function () {
            ...
            for (var i = 0, l = failedDependencies.length; i < l; i++) {
              ...
              if (
                ...
                failedDependency.exception.message
              ) {
    ...
    angular.module(moduleName, loadedDependencies).directive(directiveName, [
      "ganttUtils",
      "$compile",
        ...
        return {
          ...
          link: function (scope, element, attrs, ganttCtrl) {
            ...
            api.directives.on.new(
              ...
              function (directiveName, rowScope, rowElement) {
                if (
                  directiveName === "ganttRowLabel" &&
                  rowElement.attr("drag") === undefined
                ) {
...
(function () {
  "use strict";
  angular
    .module("gantt.table", ["gantt", "gantt.table.templates"])
    .directive("ganttTable", [
      "ganttUtils",
      "$compile",
      "$document",
      function (utils, $compile, $document) {
        // Provides the row sort functionality to any Gantt row
        // Uses the sortableState to share the current row
            ...
            api.directives.on.new(
              ...
              function (directiveName, sideContentScope, sideContentElement) {
                if (directiveName === "ganttSideContent") {
                  ...
                  var tableElement = $document[0].createElement(
...
(function () {
  "use strict";
  angular
    .module("gantt.tree", ["gantt", "gantt.tree.templates", "ui.tree"])
    .directive("ganttTree", [
      "ganttUtils",
      "$compile",
      "$document",
...
(function () {
  "use strict";
  angular.module("gantt.groups").controller("GanttGroupController", [
    "$scope",
    "GanttTaskGroup",
    "ganttUtils",
    function ($scope, TaskGroup, utils) {
      var updateTaskGroup = function () {
        ...
        if (enabledValue) {
          $scope.display = utils.firstProperty(
            [rowGroups],
            "display",
            $scope.pluginScope.display
          );
          $scope.taskGroup = new TaskGroup($scope.row, $scope.pluginScope);

          $scope.row.setFromTo();
          $scope.row.setFromToByValues(
            $scope.taskGroup.from,
            $scope.taskGroup.to
          );
        } else {
      ...
      $scope.gantt.api.tasks.on.change($scope, function (task) {
        if ($scope.taskGroup !== undefined) {
          if ($scope.taskGroup.tasks.indexOf(task) > -1) {
            ...
            var descendants = $scope.pluginScope.hierarchy.descendants(
              $scope.row
            );
...
(function () {
  "use strict";
  angular.module("gantt.groups").directive("ganttTaskGroup", [
    "GanttDirectiveBuilder",
    function (Builder) {
      var builder = new Builder(
        "ganttTaskGroup",
        "plugins/groups/taskGroup.tmpl.html"
      );
...
(function () {
  ...
  angular.module("gantt").factory("GanttTaskGroup", [
    "ganttUtils",
    "GanttTask",
    function (utils, Task) {
      var TaskGroup = function (row, pluginScope) {
        ...
        var getTaskDisplay = function (task) {
          ...
          var enabledValue = utils.firstProperty(
            [taskGroups, rowGroups, groupRowGroups],
            "enabled",
            self.pluginScope.enabled
          );

          if (enabledValue) {
            var display = utils.firstProperty(
              [taskGroups, rowGroups, groupRowGroups],
              "display",
              self.pluginScope.display
            );
            return display;
          }
        };
        ...
        self.width =
          row.rowsManager.gantt.getPositionByDate(self.to) - self.left;
      };
      return TaskGroup;
    },
  ]);
...
(function () {
  "use strict";
  angular.module("gantt").directive("ganttTaskOverview", [
    "GanttDirectiveBuilder",
    "moment",
    function (Builder, moment) {
      var builder = new Builder(
        "ganttTaskOverview",
        "plugins/groups/taskOverview.tmpl.html"
      );
...
(function () {
  "use strict";
  angular.module("gantt.labels").directive("ganttLabelsBody", [
    "GanttDirectiveBuilder",
    "ganttLayout",
    function (Builder, layout) {
      var builder = new Builder(
        "ganttLabelsBody",
        "plugins/labels/labelsBody.tmpl.html"
      );
...
(function () {
  "use strict";
  angular.module("gantt.labels").directive("ganttLabelsHeader", [
    "GanttDirectiveBuilder",
    function (Builder) {
      var builder = new Builder(
        "ganttLabelsHeader",
        "plugins/labels/labelsHeader.tmpl.html"
      );
      return builder.build();
    },
  ]);
...
(function () {
  "use strict";
  angular.module("gantt.labels").directive("ganttSideContentLabels", [
    "GanttDirectiveBuilder",
    function (Builder) {
      var builder = new Builder(
        "ganttSideContentLabels",
        "plugins/labels/sideContentLabels.tmpl.html"
      );
      return builder.build();
    },
  ]);
...
(function () {
  "use strict";
  angular.module("gantt.table").directive("ganttSideContentTable", [
    "GanttDirectiveBuilder",
    "ganttLayout",
    function (Builder, layout) {
      var builder = new Builder(
        "ganttSideContentTable",
        "plugins/table/sideContentTable.tmpl.html"
      );
      builder.controller = function ($scope) {
        var hScrollBarHeight = layout.getScrollBarHeight();
...
(function () {
  "use strict";
  angular.module("gantt.table").controller("TableColumnController", [
    "$scope",
    function ($scope) {
...
(function () {
  "use strict";
  angular.module("gantt.table").controller("TableColumnRowController", [
    "$scope",
    function ($scope) {
...
(function () {
  "use strict";
  angular.module("gantt.tree").directive("ganttRowTreeLabel", [
    "GanttDirectiveBuilder",
    function (Builder) {
...
(function () {
  "use strict";
  angular.module("gantt.tree").directive("ganttSideContentTree", [
    "GanttDirectiveBuilder",
    function (Builder) {
      var builder = new Builder(
        "ganttSideContentTree",
        "plugins/tree/sideContentTree.tmpl.html"
      );
      return builder.build();
    },
  ]);
...
(function () {
  ...
  angular
    .module("gantt.tree")
    .controller("GanttTreeController", [
      "$scope",
      "$filter",
      "GanttHierarchy",
      function ($scope, $filter, Hierarchy) {
        $scope.rootRows = [];
        ...
        $scope.pluginScope.$watchGroup(
          ["keepAncestorOnFilterRow", "enabled"],
          function (value) {
            var keepAncestor = value[0] && value[1];

            if (keepAncestor) {
              var filterImpl = function (
                sortedRows,
                filterRow,
                filterRowComparator
              ) {
                ...
                var filteredLeaves = $filter("filter")(
                  leaves,
                  filterRow,
                  filterRowComparator
                );
        ...
        var refresh = function () {
          $scope.rootRows = hierarchy.refresh(
            $scope.gantt.rowsManager.filteredRows
          );
        ...
        $scope.gantt.api.registerMethod(
          "tree",
          "isCollapsed",
          isRowCollapsed,
          this
    ...
    .controller("GanttUiTreeController", [
      "$scope",
      function ($scope) {
    ...
    .controller("GanttTreeNodeController", [
      "$scope",
      function ($scope) {
        ...
        $scope.isCollapseDisabled = function () {
          return (
            ...
            $scope.$parent.childrenRows.length === 0
          );
        };
        ...
        $scope.getRowContent = function () {
          ...
          var content =
            $scope.row.rowsManager.gantt.options.value("rowContent");
          if (content === undefined) {
        ...
        $scope.$watch("collapsed", function (newValue) {
          if ($scope.$modelValue._collapsed !== newValue) {
            ...
            if (oldValue !== undefined && newValue !== oldValue) {
              $scope.gantt.api.tree.raise.collapsed(
                $scope,
                $scope.$modelValue,
                newValue
              );
...
(function () {
  "use strict";
  angular.module("gantt.tree").directive("ganttTreeBody", [
    "GanttDirectiveBuilder",
    "ganttLayout",
    function (Builder, layout) {
      var builder = new Builder(
        "ganttTreeBody",
        "plugins/tree/treeBody.tmpl.html"
      );
      builder.controller = function ($scope) {
        var hScrollBarHeight = layout.getScrollBarHeight();
...
(function () {
  "use strict";
  angular.module("gantt.tree").directive("ganttTreeHeader", [
    "GanttDirectiveBuilder",
    function (Builder) {
      var builder = new Builder(
        "ganttTreeHeader",
        "plugins/tree/treeHeader.tmpl.html"
      );
      return builder.build();
    },
  ]);
...
angular.module("gantt.groups.templates", []).run([
  "$templateCache",
  function ($templateCache) {
    $templateCache.put(
      "plugins/groups/taskGroup.tmpl.html",
...
angular.module("gantt.labels.templates", []).run([
  "$templateCache",
  function ($templateCache) {
    $templateCache.put(
      "plugins/labels/labelsBody.tmpl.html",
...
angular
  .module("gantt.overlap.templates", [])
  .run(["$templateCache", function ($templateCache) {}]);
...
angular
  .module("gantt.resizeSensor.templates", [])
  .run(["$templateCache", function ($templateCache) {}]);
...
angular.module("gantt.table.templates", []).run([
  "$templateCache",
  function ($templateCache) {
    $templateCache.put(
      "plugins/table/sideContentTable.tmpl.html",
...
angular.module("gantt.tree.templates", []).run([
  "$templateCache",
  function ($templateCache) {
    $templateCache.put(
      "plugins/tree/sideContentTree.tmpl.html",
    ...
    $templateCache.put(
      "plugins/tree/treeBodyChildren.tmpl.html",
