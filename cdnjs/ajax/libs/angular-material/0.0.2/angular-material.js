...
(function () {
  ...
  angular
    .module("ngAnimateSequence", ["ngAnimate"])
            ...
            $$animateReflow(function () {
              $$animateAll(
                [
                  ...
                  function (done) {
                    cancelAnimation =
                      fn(element, animationDuration, done) || angular.noop;
                  },
                ],
                tick
              );
          ...
          return (self = {
            ...
            then: function (fn) {
              return addToChain(
                ...
                null,
                null,
                function (element, duration, done) {
                  fn(element, done);
                }
              );
            },
            ...
            animate: function (preOptions, postOptions, time) {
              ...
              return addToChain(
                time || postOptions.duration,
                preOptions,
                postOptions,
                function (_, duration, done) {
                  done();
                }
              );
            },
  ...
  angular
    .module("ngAnimateStylers", ["ngAnimateSequence"])
      ...
      function ($$animateStylerProvider) {
        ...
        $$animateStylerProvider.register("webAnimations", [
          ...
          function ($window, $sniffer) {
            ...
            return function (element, pre, duration, delay) {
              ...
              if (!node.animate) {
                throw new Error(
                  "WebAnimations (element.animate) is not defined for use within $$animationStylerProvider."
                );
              }
              ...
              return function (post, done) {
                ...
                if (missingProperties.length) {
                  pre = angular.extend(
                    pre,
                    computeStartingStyles(node, missingProperties)
                  );
                }
            ...
            function computeStartingStyles(node, props) {
              ...
              angular.forEach(props, function (prop) {
                ...
                if (
                  !isDefined(value) &&
                  webkit &&
                  specialStyles.indexOf(prop) >= 0
                ) {
                  prop =
                    "webkit" + prop.charAt(0).toUpperCase() + prop.substr(1);
        ...
        $$animateStylerProvider.register("gsap", function () {
          return function (element, pre, duration, delay) {
            ...
            if (!styler) {
              throw new Error(
                "GSAP TweenMax or TweenLite is not defined for use within $$animationStylerProvider."
              );
            }
  ...
  var Util = {
     ...
     */
    ancestorHasAttribute: function ancestorHasAttribute(
      element,
      attrName,
      limit
    ) {
  ...
  function MaterialEffects(
    ...
    var styler = angular.isDefined($rootElement[0].animate)
      ? "webAnimations"
      : angular.isDefined(window["TweenMax"] || window["TweenLite"])
      ? "gsap"
    ...
    function popIn(element, parentElement, clickElement, done) {
      ...
      element.css(TRANSFORM_PROPERTY, startPos).css("opacity", 0);
    ...
    function popOut(element, parentElement, done) {
      var endPos = $position.positionElements(
        parentElement,
        element,
        "bottom-center"
      );
  ...
  angular
    .module("material.animations")
    .service("$ripple", ["$$rAF", MaterialRippleService]);
  /**
  ...
  function MaterialRippleService($$rAF) {
        ...
        adjustBounds: function (canvas) {
          ...
          angular.forEach("width height".split(" "), function (style) {
            ...
            if (angular.isDefined(value)) {
              ...
              canvas.setAttribute(
                style,
                bounds[style] * self.pixelDensity + "px"
              );
            }
          });
          ...
          canvas.setAttribute(
            "height",
            bounds.height * this.pixelDensity + "px"
          );
    ...
    function waveRadiusFn(touchDownMs, touchUpMs, anim) {
      ...
      var waveRadius =
        Math.min(Math.sqrt(ww * ww + hh * hh), waveMaxRadius) * 1.1 + 5;
    ...
    function createWave(elem, width, height, startAt, recenter) {
      var wave = {
        ...
        maxRadius:
          distanceFromPointToFurthestCorner(startAt, { w: width, h: height }) *
          0.75,
    ...
    function drawWave(wave, anim) {
      ...
      if (wave.endPosition) {
        ...
        var translateFraction = Math.min(
          1,
          ((anim.radius / wave.containerSize) * 2) / Math.sqrt(2)
        );
  ...
  (function () {
    ...
    function MaterialRippleDirective(
      ...
      function compileWithCanvas(element, attrs) {
        ...
        element.replaceWith(angular.element($interpolate(tag)(options)));
        ...
        return function postLink(scope, element, attrs, noinkCtrl) {
          ...
          var ripple,
            ...
            makeRipple = $throttle({
              start: function () {
                ripple =
                  ripple || $materialEffects.inkRipple(element[0], options);
                watchMouse =
                  watchMouse || buildMouseWatcher(parent, makeRipple);
                  ...
                  switch (e.type) {
                    case "mousedown":
                      ...
                      if (!e.ctrlKey && e.button !== RIGHT_BUTTON) {
                        watchMouse(true);
                        ripple.createAt(
                          options.forceToCenter ? null : localToCanvas(e)
                        );
                      }
                      break;
  ...
  function MaterialButtonDirective(ngHrefDirectives, $expectAria) {
    ...
    return {
      ...
      transclude: true,
      template:
        '<material-ripple start="center" initial-opacity="0.25" opacity-decay-velocity="0.75"></material-ripple>',
   ...
   */
  angular
    .module("material.components.content", ["material.services.registry"])
  ...
  angular
    ...
    .directive("materialDialog", ["$$rAF", MaterialDialogDirective])
   ...
   */
  function MaterialDialogService(
    $timeout,
    $materialCompiler,
    $rootElement,
    $rootScope,
    ...
    function showDialog(options) {
      options = angular.extend(
        {
          ...
          transformTemplate: function (template) {
            return (
              '<div class="material-dialog-container">' + template + "</div>"
            );
          },
      ...
      recentDialog &&
        recentDialog.then(function (destroyDialog) {
          destroyDialog();
        });

      recentDialog = $materialCompiler
        .compile(options)
        .then(function (compileData) {
          ...
          if (options.hasBackdrop) {
            backdrop = angular.element(
              '<material-backdrop class="opaque ng-enter">'
            );
  ...
  angular
    ...
    .directive("materialInput", [materialInputDirective]);
    ...
    function selectButton(direction, parent, loop) {
      ...
      if (buttons.count()) {
        ...
        var target = !selected
          ? buttons.first()
          : direction == "previous"
          ? buttons.previous(selected)
          : buttons.next(selected);
   ...
   */
  angular
    .module("material.components.sidenav", ["material.services.registry"])
    .factory("$materialSidenav", [
      "$materialComponentRegistry",
      materialSidenavService,
    ])
    .directive("materialSidenav", ["$timeout", materialSidenavDirective])
   ...
   */
  function materialSidenavController(
    $scope,
    $element,
    $attrs,
    $timeout,
    ...
    return {
      ...
      link: function ($scope, $element, $attr, sidenavCtrl) {
        var backdrop = angular.element(
          '<material-backdrop class="material-sidenav-backdrop">'
        );
  ...
  function materialSliderDirective($window) {
    ...
    function link(scope, element, attr) {
      ...
      var trackEle = angular.element(
        element[0].querySelector(".material-track")
      );

      trackEle.append(
        '<div class="material-fill"><div class="material-thumb"></div></div>'
      );
      ...
      if (input.attr("step")) {
        ...
        var tickMarkersEle = angular.element(
          '<div class="material-tick-markers"></div>'
        );
        ...
        post: function tabsPostLink(
          ...
          function configureInk() {
            ...
            function refreshInkBar(skipAnimation) {
              ...
              if (
                tabElement &&
                tabElement.length &&
                angular.isDefined(inkBar)
              ) {
                var width = tabElement.prop("offsetWidth");
                var left =
                  headerContainer.prop("offsetLeft") +
                  tabElement.prop("offsetLeft") +
                  (scope.headerPos || 0);
                ...
                inkBar
                  .toggleClass("animate", skipAnimation !== true)
                  .css(styles);
              }
            }
          }
          ...
          function configurePagination() {
            ...
            function setHeaderPos(x) {
              header.css(
                $materialEffects.TRANSFORM_PROPERTY,
            ...
            function refreshPagination() {
              ...
              if (shouldPaginate) {
                pagination.pagesCount = Math.ceil(
                  (TAB_MIN_WIDTH * tabs.length) / tabsWidth
                );
                pagination.itemsPerPage = Math.max(
                  1,
                  Math.floor(tabs.length / pagination.pagesCount)
                );
            ...
            function setPage(page) {
              ...
              pagination.endIndex =
                pagination.startIndex + pagination.itemsPerPage - 1;

              pagination.hasPrev = page > 0;
              pagination.hasNext =
                (page + 1) * pagination.itemsPerPage < tabs.length;

              setHeaderPos(
                -page * pagination.itemsPerPage * pagination.tabWidth
              );

              if (
                scope.$selIndex < pagination.startIndex ||
            ...
            function paginationWatchSelectedTab(selectedIndex) {
              ...
              if (
                selectedIndex < pagination.startIndex ||
          ...
          function transcludeHeaderItems() {
            $transclude(function (content) {
              ...
              angular.forEach(content, function (node) {
                var intoHeader =
                  isNodeType(node, "material-tab") || isNgRepeat(node);
          ...
          function transcludeContentItems() {
            var cntr = findNode(".tabs-content", element),
              materialViewTmpl =
                '<div class="material-view" ng-show="active"></div>';
            ...
            function extractContent(tab) {
              ...
              var scope =
                content && content.length
                  ? angular.element(content[0]).scope()
                  : null;
  ...
  function TabDirective($attrBind) {
    ...
    function linkTab(scope, element, attrs, tabsController, $transclude) {
      ...
      function updateTabContent(scope) {
        ...
        var cntr = angular.element(
          element[0].querySelector("material-tab-label")
        );
  ...
  function TabsController(
    ...
    $materialComponentRegistry.register(
      self,
      $attrs.componentId || componentID
    );
  ...
  var isNgRepeat = function (node) {
    var COMMENT_NODE = 8;
    return (
      node.nodeType == COMMENT_NODE && node.nodeValue.indexOf("ngRepeat") > -1
    );
  };
   ...
   */
  angular
    .module("material.components.toast", ["material.services.compiler"])
    .directive("materialToast", [QpToastDirective])
  ...
  function QpToastService(
    ...
    function showToast(options) {
      ...
      recentToast &&
        recentToast.then(function (destroy) {
          destroy();
        });

      recentToast = $materialCompiler
        .compile(options)
        .then(function (compileData) {
   ...
   */
  angular
    .module("material.components.toolbar", ["material.components.content"])
  ...
  angular
    .module("material.services.expectAria", [])

    .service("$expectAria", ["$log", ExpectAriaService]);

  function ExpectAriaService($log) {
    var messageTemplate =
      'ARIA: Attribute "%s", required for accessibility, is missing on "%s"!';
  ...
  angular
    .module("material.services.attrBind", [])
    .factory("$attrBind", ["$parse", "$interpolate", MaterialAttrBind]);
    ...
    return function (scope, attrs, bindDefinition, bindDefaults) {
        ...
        switch (mode) {
          ...
          case "=": // Two-way binding...
            if (!bypassWithDefaults(attrName, scopeName)) {
              ...
              scope[scopeName] =
                attrs[attrName] === "" ? true : scope.$eval(attrs[attrName]);
      ...
      function bypassWithDefaults(attrName, scopeName, defaultVal) {
        if (!angular.isDefined(attrs[attrName])) {
          var hasDefault =
            bindDefaults && bindDefaults.hasOwnProperty(scopeName);
  ...
  function materialCompilerService(
    $q,
    $http,
    $injector,
    $compile,
  ...
  angular
    .module("material.services.position", [])
    .factory("$position", ["$document", "$window", MaterialPositionService]);
  ...
  angular
    .module("material.services.throttle", [])
    .factory("$throttle", ["$timeout", "$$q", "$log", MaterialThrottleService]);
  /**
  ...
  function MaterialThrottleService($timeout, $$q, $log) {
    ...
    function buildInstance(phases, done, otherwise) {
      ...
      function start() {
        return gotoState
          .apply(null, [STATE_START, phases.start])
      ...
      function end() {
        return gotoState
          .apply(null, [STATE_END, phases.end])
      ...
      function gotoState(nextState, targetFn) {
        ...
        try {
          ...
          cancel = fn.apply(null, [
            goNext ? resolved(dfd) : hasFn ? callbackToResolve(dfd) : dfd,
          ]);
        } catch (error) {
          dfd.reject(error);
        }
