...
(function () {
   ...
   */
  angular
    .module("material.animations", ["material.services.throttle"])
  ...
  function MaterialEffects($rootElement, $$rAF, $sniffer, $q) {
    ...
    return (self = {
      ...
      ANIMATIONEND_EVENT:
        "animationend" + (webkit ? " webkitAnimationEnd" : ""),
  ...
  angular
    ...
    .directive("inkRipple", ["$materialInkRipple", InkRippleDirective])
    ...
    function attach(element, options) {
      ...
      function enableMousedown() {
        ...
        function onPointerDown(ev) {
          ...
          var ripplePauseTimeout = $timeout(
            pauseRipple,
      ...
      function createRipple(left, top, positionsAreAbsolute) {
        var rippleEl = angular
          .element('<div class="material-ripple">')
          .css(
        ...
        if (!rippleContainer) {
          rippleContainer = angular.element(
            '<div class="material-ripple-container">'
          );
        ...
        var css = {
          "background-color":
            $window.getComputedStyle(rippleEl[0]).color ||
        ...
        css[$materialEffects.ANIMATION_DURATION] =
          options.fadeoutDuration + "ms";
        rippleEl.css(css);
    ...
    function showDialog(options) {
      ...
      recentDialog = $materialCompiler
        ...
        .then(function (compileData) {
          ...
          $materialEffects
            .popIn(element, options.appendTo, popInTarget)
            .then(function () {
  ...
  angular
    ...
    .directive("materialRadioButton", ["$aria", materialRadioButtonDirective]);
  ...
  angular
    ...
    .directive("materialSlider", [SliderDirective]);
  ...
  function SliderController(
    ...
    this.init = function init(ngModelCtrl) {
      ...
      var trackContainer = angular.element(
        element[0].querySelector(".slider-track-container")
      );
      var activeTrack = angular.element(
        element[0].querySelector(".slider-track-fill")
      );
      var tickContainer = angular.element(
        element[0].querySelector(".slider-track-ticks")
      );
      ...
      var debouncedRefreshDimensions = Util.debounce(
        refreshSliderDimensions,
        5000
      );
      refreshSliderDimensions();
  ...
  function MaterialSwitch(checkboxDirectives, radioButtonDirectives) {
    ...
    function compile(element, attr) {
      var thumb = angular.element(
        element[0].querySelector(".material-switch-thumb")
      );
  ...
  function linkTabInk(scope, element, tabsCtrl, $q, $materialEffects) {
    ...
    function updateInkBar(immediate) {
      ...
      if (!showInk || isHiding) {
        ...
        left =
          tabsHeader.prop("offsetLeft") +
          (scope.pagingOffset || 0) +
          selButton.prop("offsetLeft");
  ...
  function linkTabPagination(scope, element, tabsCtrl, $q, $materialEffects) {
    ...
    function updatePagination() {
      ...
      var needPagination =
        tabsWidth > 0 && TAB_MIN_WIDTH * tabs.length > tabsWidth;
    ...
    function selectPageAt(page, updateTabSelection) {
      ...
      pagination.startIndex = !pagination.active
        ? 0
        : page * pagination.itemsPerPage;
      pagination.endIndex = !pagination.active
        ? lastTab
        : pagination.startIndex + pagination.itemsPerPage - 1;
      pagination.hasPrev = !pagination.active ? false : page > 0;
      pagination.hasNext = !pagination.active
        ? false
        : page + 1 < pagination.pagesCount;
    ...
    function getPageAtTabIndex(tabIndex) {
      ...
      return numPages < 1
        ...
        : tabIndex > lastTab
        ? lastPage
        : Math.floor(tabIndex / pagination.itemsPerPage);
    }
    ...
    function isTabInRange(tabIndex) {
      return (
        tabIndex >= pagination.startIndex && tabIndex <= pagination.endIndex
      );
    }
  }
  ...
  function TabDirective($attrBind, $aria, $materialInkRipple) {
    ...
    return {
      ...
      scope: true,
      link: linkTab,
      template: "<material-tab-label></material-tab-label>",
    };
    ...
    function linkTab(scope, element, attrs, tabsCtrl, $transclude) {
      ...
      element
        .on("click", function onRequestSelect() {
      ...
      function configureAria(element, scope) {
        ...
        function buildAriaID() {
          return (
            attrs.id || ROLE.TAB + "_" + tabsCtrl.$scope.$id + "_" + scope.$id
          );
        }
      }
  ...
  angular
    .module("material.components.tabs")
    .factory("$materialTabs", ["$materialComponentRegistry", TabsService])
    ...
    function selectTab(tab, noUpdate) {
      ...
      if (tab != null) {
        ...
        $scope.$selIndex = selected
          ? String(selected.$index || list.indexOf(selected))
          : -1;
    ...
    function focusOn(which, from) {
      var tab =
        which === "current"
          ? selected
          : which === "next"
          ? list.next(from || selected, isEnabled)
          : which === "previous"
          ? list.previous(from || selected, isEnabled)
          : null;
    ...
    function compileTabsFn() {
      return {
        ...
        post: function tabsPostLink(
          ...
          var updateInk =
            linkTabInk(scope, element, tabsCtrl, $q, $materialEffects) ||
            angular.noop;
    ...
    return {
      ...
      link: function (scope, element, attr) {
        ...
        function setupScrollShrink() {
          ...
          var debouncedUpdateHeight = Util.debounce(
            updateToolbarHeight,
            5 * 1000
          );
  ...
  angular
    .module("material.services.aria", [])

    .service("$aria", ["$log", AriaService]);
  ...
  function AriaService($log) {
    ...
    function assignAttributes(element, options) {
      angular.forEach(
        options,
        Util.spread(function (attrValue, attrName) {
