...
(function () {
  angular.module("ngMaterial", [
    ...
    "material.components.circularProgress",
   ...
   * requirements.
   */
  angular.module("material.core", ["ng"]).run(function validateEnvironment() {
    if (angular.isUndefined(window.Hammer)) {
      throw new Error("$materialSwipe requires HammerJS to be preloaded.");
    }
  });
  ...
  angular
    ...
    .directive("materialBottomSheet", [MaterialBottomSheetDirective])
 ...
 */
  ...
  function materialRadioGroupDirective() {
    ...
    function link(scope, element, attr, ctrls) {
      ...
      function keydownListener(ev) {
        if (
          ev.which === Constant.KEY_CODE.LEFT_ARROW ||
          ev.which === Constant.KEY_CODE.UP_ARROW
        ) {
          ev.preventDefault();
          rgCtrl.selectPrevious();
        } else if (
          ev.which === Constant.KEY_CODE.RIGHT_ARROW ||
          ev.which === Constant.KEY_CODE.DOWN_ARROW
        ) {
          ev.preventDefault();
          rgCtrl.selectNext();
        }
    ...
    function changeSelectedButton(parent, increment) {
      ...
      if (buttons.count()) {
        var selected = parent[0].querySelector(
          "material-radio-button.material-checked"
        );
        var target =
  ...
  function SliderController(
    ...
    this.init = function init(ngModelCtrl) {
      ...
      var throttledRefreshDimensions = Util.throttle(
        refreshSliderDimensions,
        5000
      );
      refreshSliderDimensions();
   ...
   */
  angular
    .module("material.components.subheader", ["material.components.sticky"])
        ...
        return function postLink(scope, element, attr) {
          function getContent(el) {
            return angular.element(
              el[0].querySelector(".material-subheader-content")
            );
          }
  ...
  function MaterialTabInkDirective($materialEffects, $window, $$rAF, $timeout) {
    ...
    function postLink(scope, element, attr, ctrls) {
      ...
      function updateBar() {
        var selectedElement =
          tabsCtrl.selected() && tabsCtrl.selected().element;
        ...
        if (!selectedElement || tabsCtrl.count() < 2) {
          ...
          var left =
            selectedElement.prop("offsetLeft") + (tabsCtrl.$$pagingOffset || 0);
  ...
  function TabPaginationDirective(
    ...
    function postLink(scope, element, attr, tabsCtrl) {
      ...
      function userChangePage(increment) {
        ...
        if (
          !tabsCtrl.selected() ||
          getPageForTab(tabsCtrl.selected()) !== newPage
        ) {
          var startIndex;
          if (increment < 0) {
      ...
      function updatePagination() {
        ...
        var needPagination =
          tabsWidth && TAB_MIN_WIDTH * tabsCtrl.count() > tabsWidth;
        ...
        if (needPagination) {
          state.pagesCount = Math.ceil(
            (TAB_MIN_WIDTH * tabsCtrl.count()) / tabsWidth
          );
          state.itemsPerPage = Math.max(
            1,
            Math.floor(tabsCtrl.count() / state.pagesCount)
          );
      ...
      function slideTabButtons(x) {
        ...
        tabsParent.on(
          $materialEffects.TRANSITIONEND_EVENT,
  ...
  function MaterialTabsController(scope, element) {
    ...
    self.contentArea = angular.element(
      element[0].querySelector(".tabs-content")
    );
  ...
  angular
     ...
     *
     */
    .directive("materialTabs", ["$parse", TabsDirective]);
  ...
  angular
    ...
    .directive("materialToast", [MaterialToastDirective])
  ...
  function MaterialLinearProgressDirective($$rAF, $materialEffects) {
    ...
    function postLink(scope, element, attr) {
      ...
      attr.$observe("value", function (value) {
        ...
        bar2Style[$materialEffects.TRANSFORM] =
          linearProgressTransforms[clamped];
      });

      attr.$observe("secondaryvalue", function (value) {
        bar1Style[$materialEffects.TRANSFORM] =
          linearProgressTransforms[clamp(value)];
      });
  ...
  function MaterialCircularProgressDirective($$rAF, $materialEffects) {
    ...
    function postLink(scope, element, attr) {
      ...
      circle.style[$materialEffects.TRANSFORM] =
  ...
  (function () {
     ...
     */
    angular
      .module("material.components.swipe", ["ng"])
  ...
  angular
    ...
    .service("$materialAria", ["$log", AriaService]);
  ...
  angular
    .module("material.services.interimElement", ["material.services.compiler"])
