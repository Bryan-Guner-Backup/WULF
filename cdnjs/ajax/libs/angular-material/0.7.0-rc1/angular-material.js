...
(function () {
  ...
  angular.module("material.core").factory("$mdUtil", [
    "$cacheFactory",
    "$document",
    function ($cacheFactory, $document) {
      var Util;
      return (Util = {
      ...
      function attachDragBehavior(scope, element, options) {
        ...
        function startDrag(ev) {
          ...
          if (
            previousDrag &&
            previousDrag.pointerType !== eventType &&
        ...
        function updateDragState(ev) {
          ...
          drag.direction =
            drag.distance > 0 ? "left" : drag.distance < 0 ? "right" : "";
  ...
  function AriaService($$rAF, $log, $window) {
    ...
    function expect(element, attrName, defaultValue) {
      ...
      if (!node.hasAttribute(attrName) && !childHasAttribute(node, attrName)) {
        defaultValue =
          (angular.isString(defaultValue) && defaultValue.trim()) || "";
            ...
            show: function () {
              return $mdCompiler.compile(options).then(function (compileData) {
                ...
                return $q.when(ret).then(function () {
                  ...
                  (
                    options.onComplete || angular.noop
...
(function () {
  ...
  var DARK_SHADOW =
    "1px 1px 0px rgba(black, 0.4), -1px -1px 0px rgba(black, 0.4)";
  var LIGHT_SHADOW = "";
  ...
  function ThemingProvider() {
    ...
    function Theme(name) {
      ...
      THEME_COLOR_TYPES.forEach(function (colorType) {
        var defaultHues = (
          self.isDark ? DARK_DEFAULT_HUES : LIGHT_DEFAULT_HUES
        )[colorType];
  ...
  function parseRules(theme, colorType, rules) {
    ...
    rules = rules.replace(
      simpleVariableRegex,
      function (match, colorType, hue, opacity) {
    ...
    angular.forEach(color.hues, function (hueValue, hueName) {
      var newRule = rules.replace(
        hueRegex,
        function (match, _, colorType, hueType, opacity) {
          return rgba(
      ...
      angular.forEach(palette, function (hueValue, hueName) {
        ...
        function getContrastColor() {
          if (defaultContrast === "light") {
            return darkColors.indexOf(hueName) > -1
              ? DARK_CONTRAST_COLOR
              : LIGHT_CONTRAST_COLOR;
          } else {
            return lightColors.indexOf(hueName) > -1
              ? LIGHT_CONTRAST_COLOR
              : DARK_CONTRAST_COLOR;
          }
        }
  ...
  function MdBottomSheetProvider($$interimElementProvider) {
    ...
    function bottomSheetDefaults(
      ...
      function BottomSheet(element) {
        ...
        function getY(e) {
          var touch =
            e.touches && e.touches.length ? e.touches[0] : e.changedTouches[0];
        ...
        function adjustedDelta(delta) {
          if (delta < 0 && delta < -MAX_OFFSET + WIGGLE_AMOUNT) {
            ...
            delta = Math.max(
              ...
              -Math.min(
                MAX_OFFSET - 5,
                base + (WIGGLE_AMOUNT * (delta - base)) / MAX_OFFSET
              ) -
                delta / 50
            );
          }
    ...
    function dialogDefaultOptions(
      ...
      function onShow(scope, element, options) {
        ...
        function findCloseButton() {
          ...
          if (!closeButton) {
            var actionButtons =
              element[0].querySelectorAll(".md-actions button");
...
(function () {
  ...
  function MdProgressCircularDirective($$rAF, $mdConstant, $mdTheming) {
    ...
    function postLink(scope, element, attr) {
      ...
      circle.style[$mdConstant.CSS.TRANSFORM] =
  ...
  function mdRadioGroupDirective($mdUtil, $mdConstant, $mdTheming) {
    ...
    function linkRadioGroup(scope, element, attr, ctrls) {
      ...
      function keydownListener(ev) {
        if (
          ev.keyCode === $mdConstant.KEY_CODE.LEFT_ARROW ||
          ev.keyCode === $mdConstant.KEY_CODE.UP_ARROW
        ) {
          ev.preventDefault();
          rgCtrl.selectPrevious();
        } else if (
          ev.keyCode === $mdConstant.KEY_CODE.RIGHT_ARROW ||
          ev.keyCode === $mdConstant.KEY_CODE.DOWN_ARROW
        ) {
          ev.preventDefault();
          rgCtrl.selectNext();
        }
    ...
    function changeSelectedButton(parent, increment) {
      ...
      if (buttons.count()) {
        ...
        var target =
          buttons[increment < 0 ? "previous" : "next"](selected) ||
          buttons.first();
  ...
  function MdTooltipDirective(
    ...
    function postLink(scope, element, attr, contentCtrl) {
      ...
      function positionTooltip() {
        ...
        if (
          newPosition.top + tipRect.height >
...
(function () {
  ...
  function TabPaginationDirective(
    $mdConstant,
    $window,
    $$rAF,
    $$q,
    ...
    function postLink(scope, element, attr, tabsCtrl) {
      ...
      function userChangePage(increment) {
        ...
        var newPage = Math.max(
          0,
          Math.min(sizeData.pages.length - 1, state.page + increment)
        );
        var newTabIndex =
          sizeData.pages[newPage][
            increment > 0 ? "firstTabIndex" : "lastTabIndex"
          ];
      ...
      function slideTabButtons(x) {
        ...
        function onTabsParentTransitionEnd(ev) {
          ...
          if (ev.target === tabsParent[0]) {
            tabsParent.off(
              $mdConstant.CSS.TRANSITIONEND,
              onTabsParentTransitionEnd
            );
            deferred.resolve();
          }
        }
      ...
      function calculateTabData(noAdjust) {
        ...
        angular.forEach(tabs, function (tab, index) {
          ...
          data.page =
            Math.ceil(
              data.right /
                (pages.length === 1 && index === tabs.length - 1
                  ? clientWidth
                  : tabsWidth)
            ) - 1;
        ...
        function adjustForStretchedTabs() {
          ...
          var tabsPerPage = Math.min(
            Math.floor(canvasWidth / max),
            tabs.length
          );
...
(function () {
  ...
  function TabItemController(
    ...
    self.contentContainer = angular.element(
      '<div class="md-tab-content ng-hide">'
    );
  ...
  function MdTabDirective(
    ...
    function compile(element, attr) {
      ...
      return function postLink(scope, element, attr, ctrls) {
        ...
        function keydownListener(ev) {
          if (
            ev.keyCode == $mdConstant.KEY_CODE.SPACE ||
            ev.keyCode == $mdConstant.KEY_CODE.ENTER
          ) {
...
(function () {
  ...
  function MdTabsController($scope, $element, $mdUtil, $$rAF) {
    ...
    self.contentArea = angular.element(
      $element[0].querySelector(".md-tabs-content")
    );
    ...
    function add(tab, index) {
      ...
      if (
        ...
        !angular.isNumber($scope.selectedIndex) ||
    ...
    function next(tab, filterFn) {
      return tabsList.next(
        tab || self.getSelectedItem(),
        filterFn || isTabEnabled
      );
    }
    function previous(tab, filterFn) {
      return tabsList.previous(
        tab || self.getSelectedItem(),
        filterFn || isTabEnabled
      );
    }
...
(function () {
  ...
  function TabsDirective($mdTheming) {
    ...
    function postLink(scope, element, attr, tabsCtrl, transclude) {
      scope.stretchTabs = attr.hasOwnProperty("mdStretchTabs")
        ? attr.mdStretchTabs || "always"
        : "auto";
      ...
      transclude(scope.$parent, function (clone) {
        angular
          .element(element[0].querySelector(".md-header-items"))
          .append(clone);
      });
      ...
      function watchSelected() {
        scope.$watch(
          "selectedIndex",
          function watchSelectedIndex(newIndex, oldIndex) {
