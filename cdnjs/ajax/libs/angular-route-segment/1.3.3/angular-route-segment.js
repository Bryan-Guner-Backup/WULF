...
(function (angular) {
  ...
  mod.provider("$routeSegment", [
    ...
    function ($routeProvider) {
      ...
      function pointer(segment, parent) {
        if (!segment) throw new Error("Invalid pointer segment");
          ...
          within: function (childName) {
            ...
            if ((child = segment[camelCase(childName)])) {
              ...
              if (options.strictMode)
                throw new Error(
                  "Cannot get into unknown `" + childName + "` segment"
                );
              else {
                child = segment[camelCase(childName)] = {
                  params: {},
                  children: {},
                };
              }
        ...
        function (
          ...
          var $routeSegment = {
            ...
            getSegmentUrl: function (segmentName, routeParams) {
              ...
              if (!segmentRoutes[segmentName])
                throw new Error(
                  "Can not get URL for segment with name `" + segmentName + "`"
                );
            ...
            if (route && route.segment) {
              ...
              for (var i = 0; i < segmentNameChain.length; i++) {
                ...
                if (
                  ...
                  if (
                    $routeSegment.chain[i] &&
                    $routeSegment.chain[i].name == newSegment.name &&
                    updates.length == 0 &&
                    !isDependenciesChanged(newSegment)
                  )
              ...
              if (updates.length > 0) {
                for (var i = 0; i < updates.length; i++) {
                  (function (i) {
                    curSegmentPromise = curSegmentPromise
                      .then(function () {
                        return updateSegment(
                          updates[i].index,
                          updates[i].newSegment
                        );
                      })
                      .then(function (result) {
                        if (result.success != undefined) {
                          ...
                          for (
                            var j = updates[i].index + 1;
                            j < $routeSegment.chain.length;
                            j++
                          ) {
              ...
              curSegmentPromise
                .then(function () {
                  ...
                  if ($routeSegment.chain.length > segmentNameChain.length) {
                    ...
                    var shortenBy =
                      $routeSegment.chain.length - segmentNameChain.length;
                ...
                .then(function () {
                  ...
                  if (lastUpdateIndex == $routeSegment.chain.length - 1) {
                    var curSegment = getSegmentInChain(
                      lastUpdateIndex,
                      $routeSegment.name.split(".")
                    );

                    while (curSegment) {
                      var children = curSegment.children,
                        index = lastUpdateIndex + 1;
                      curSegment = null;
                      for (var i in children) {
                        (function (i, children, index) {
                          if (children[i].params["default"]) {
                            defaultChildUpdatePromise =
                              defaultChildUpdatePromise.then(function () {
          ...
          function updateSegment(index, segment) {
            ...
            if (segment.params.untilResolved) {
              return resolve(
                index,
                segment.name,
                segment.params.untilResolved
              ).then(function (result) {
                if (result.success != undefined) broadcast(index);
                return resolve(index, segment.name, segment.params);
              });
            } else return resolve(index, segment.name, segment.params);
          }
          ...
          function resolve(index, name, params) {
            ...
            if (options.autoLoadTemplates && params.templateUrl) {
              ...
              locals.$template = $http
                .get(locals.$template, { cache: $templateCache })
            ...
            return $q.all(locals).then(
              function (resolvedLocals) {
                if (resolvingSemaphoreChain[index] != name) return $q.reject();
                ...
                $routeSegment.chain[index] = {
                  ...
                  reload: function () {
                    var originalSegment = getSegmentInChain(
                      index,
                      $routeSegment.name.split(".")
                    );
                    updateSegment(index, originalSegment).then(function (
                      result
                    ) {
                      if (result.success != undefined) broadcast(index);
                    });
                ...
                if (params.watcher) {
                  var getWatcherValue = function () {
                    if (
                      !angular.isFunction(params.watcher) &&
                      !angular.isArray(params.watcher)
                    )
                      throw new Error(
                        "Watcher is not a function in segment `" + name + "`"
                      );
                  ...
                  $routeSegment.chain[index].clearWatcher = $rootScope.$watch(
                    getWatcherValue,
                    function (value) {
                      if (value == lastWatcherValue)
                        // should not being run when $digest-ing at first time
                        return;
                ...
                if (params.resolveFailed) {
                  ...
                  throw new Error(
                    "Resolving failed with a reason `" +
                      error +
                      "`, but no `resolveFailed` " +
                      "provided for segment `" +
                      name +
                      "`"
                  );
              }
          ...
          function broadcast(index) {
            ...
            $routeSegment.name = $routeSegment.name.substr(
              0,
              $routeSegment.name.length - 1
            );
          ...
          function getSegmentInChain(segmentIdx, segmentNameChain) {
            if (!segmentNameChain) return null;

            if (segmentIdx >= segmentNameChain.length) return null;
            ...
            for (var i = 0; i <= segmentIdx; i++) {
              ...
              if (i < segmentIdx) curSegment = curSegment.children;
            }
   ...
   */
  mod.filter("routeSegmentEqualsTo", [
    "$routeSegment",
    function ($routeSegment) {
   ...
   */
  mod.filter("routeSegmentStartsWith", [
    "$routeSegment",
    function ($routeSegment) {
      var filter = function (value) {
   ...
   */
  mod.filter("routeSegmentContains", [
    "$routeSegment",
    function ($routeSegment) {
      var filter = function (value) {
...
(function (angular) {
  angular
    .module("view-segment", ["route-segment"])
    .directive("appViewSegment", [
      "$route",
      "$compile",
      "$controller",
      "$routeParams",
          ...
          compile: function (tElement, tAttrs) {
            var defaultContent = tElement.html(),
              ...
              anchor = angular.element(
                document.createComment(" view-segment ")
              );
            ...
            return function ($scope) {
              ...
              $scope.$on("routeSegmentChange", function (event, args) {
                ...
                if (
                  args.index == viewSegmentIndex &&
                  currentSegment != args.segment
                )
                  update(args.segment);
              });
