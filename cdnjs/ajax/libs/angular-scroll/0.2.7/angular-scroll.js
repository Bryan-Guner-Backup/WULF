...
angular
  ...
  .factory("scroller", function ($window, requestAnimation, scrollPosition) {
    function easeout(x) {
    ...
    function scrollDelta(x, y, duration) {
      scrollTo(
        scrollPosition.x() + (x || 0),
        scrollPosition.y() + (y || 0),
        duration
      );
    }
    ...
    function scrollToElement(element, offset, duration) {
      ...
      scrollDelta(
        0,
        pos.top + (!offset || isNaN(offset) ? 0 : -offset),
        duration
      );
    }
    ...
    return {
      link: function ($scope, $element, $attr) {
        ...
        element.on("click", function (e) {
          ...
          scroller.scrollDelta(
            0,
            pos.top + (isNaN(offset) ? 0 : offset),
            duration
          );
        });
      },
