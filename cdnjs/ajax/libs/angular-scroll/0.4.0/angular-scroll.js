      ...
      return {
        observe: function (cb) {
          if (!deprecationWarned && console && console.warn) {
            console.warn(
              "scrollPosition.observe is deprecated, use $rootScope.$on('$duScrollChanged') instead"
            );
  ...
  .factory(
    ...
    function ($window, requestAnimation, scrollPosition, duScrollEasing) {
      ...
      function scrollDelta(x, y, duration, context) {
        scrollTo(
          scrollPosition.x(context) + (x || 0),
          scrollPosition.y(context) + (y || 0),
          duration,
          context
        );
      }
...
angular
  .module("duScroll.spyAPI", ["duScroll.scrollPosition"])
...
angular
  .module("duScroll.scrollContextAPI", [])
...
angular
  .module("duScroll.smoothScroll", [
    "duScroll.scroller",
    "duScroll.scrollContextAPI",
  ])
  .directive(
    "duSmoothScroll",
    function (scroller, duScrollDuration, duScrollContextAPI) {
      return {
...
angular
  .module("duScroll.scrollContext", ["duScroll.scrollContextAPI"])
