...
angular.module("duScroll.scrollHelpers", ["duScroll.requestAnimation"]).run([
  ...
  function (
    ...
    proto.scrollToElement = function (target, offset, duration, easing) {
      ...
      var top =
        this.scrollTop() + unwrap(target).getBoundingClientRect().top - offset;
      if (isElement(el)) {
    ...
    proto.scrollToElementAnimated = function (
      target,
      offset,
      duration,
      easing
    ...
    for (var methodName in overloaders) {
      proto[methodName] = proto[methodName]
        ? overloadScrollPos(proto[methodName], overloaders[methodName])
        : overloaders[methodName];
    }
  },
]);
    ...
    function (
        ...
        var handler = function () {
          ...
          for (i = 0; i < spies.length; i++) {
            ...
            if (
              ...
              pos.top * -1 + containerOffset < pos.height
            ) {
      ...
      var addSpy = function (spy) {
        ...
        if (!context.container || !isElementInDocument(context.container)) {
          ...
          context.container = scrollContainerAPI.getContainer(
            spy.$element.scope()
          );
          context.container
