...
(function (window, document, undefined) {
    ...
    function ($parse, $timeout, $compile) {
      return {
        ...
        link: function postLink(scope, element, attrs) {
          ...
          if (!attrs.bsAlert) {
            if (
              angular.isUndefined(attrs.closeButton) ||
              (attrs.closeButton !== "0" && attrs.closeButton !== "false")
            ) {
              element.prepend(
            ...
            scope.$watch(
              ...
              function (newValue, oldValue) {
                ...
                if (newValue.type || oldValue.type) {
                  oldValue.type &&
                    element.removeClass("alert-" + oldValue.type);
          ...
          var parentArray =
            attrs.ngRepeat && attrs.ngRepeat.split(" in ").pop();
                ...
                iElement.on(evName, function (ev) {
                  scope.$apply(function () {
                    controller.$setViewValue(
                      $(ev.target).closest("button").attr("value")
                    );
                  });
                });
                scope.$watch(iAttrs.ngModel, function (newValue, oldValue) {
                  if (newValue !== oldValue) {
                    var $btn = iElement.find(
      ...
      return {
        ...
        link: function postLink(scope, element, attrs, ctrl) {
          ...
          element.bind("click", function (ev) {
            ...
            newValue =
              index > values.length - 2 ? values[0] : values[index + 1];
            scope.$apply(function () {
              element.text(newValue);
              if (ctrl) {
  ...
  angular.module("$strap.directives").directive("bsDatepicker", [
    ...
    function ($timeout, $strapConfig) {
      ...
      var regexpForDateFormat = function regexpForDateFormatFn(
        format,
        language
      ) {
        var re = format,
        ...
        link: function postLink(scope, element, attrs, controller) {
          ...
          var getFormattedModelValue = function (modelValue, format, language) {
            if (
              modelValue &&
              type === "iso" &&
              ISODateRegexp.test(modelValue)
            ) {
              return $.fn.datepicker.DPGlobal.parseDate(
                new Date(modelValue),
          ...
          scope.$watch(
            ...
            function (newValue, oldValue) {
              if (newValue !== oldValue) {
                var oldLanguage = $.fn.datepicker.dates[oldValue]
                  ? oldValue
                  : "en";
                var oldFormat =
                  attrs.dateFormat ||
                  options.format ||
                  ($.fn.datepicker.dates[oldLanguage] &&
                    $.fn.datepicker.dates[oldLanguage].format) ||
                  "mm/dd/yyyy";
                var oldDate = $.fn.datepicker.DPGlobal.parseDate(
                  element.val(),
                  $.fn.datepicker.DPGlobal.parseFormat(oldFormat),
                  oldLanguage
                );
                var newLanguage = $.fn.datepicker.dates[newValue]
                  ? newValue
                  : "en";
                var newFormat =
                  ($.fn.datepicker.dates[newLanguage] &&
                    $.fn.datepicker.dates[newLanguage].format) ||
                  "mm/dd/yyyy";
                var newDateString = $.fn.datepicker.DPGlobal.formatDate(
                  oldDate,
                  $.fn.datepicker.DPGlobal.parseFormat(newFormat),
                  newLanguage
                );
                element.datepicker("remove");
                element.val("");
                init();
                var mValue = getFormattedModelValue(
                  newDateString,
                  newFormat,
                  newLanguage
                );
            ...
            return $q
              .when(
                $templateCache.get(templateUrl) ||
                  $http.get(templateUrl, { cache: true }).then(function (res) {
                    return res.data;
                  })
              )
              .then(function onSuccess(template) {
                var id =
                  templateUrl.replace(".html", "").replace(/[\/|\.|:]/g, "-") +
                  "-" +
                  scope.$id;
                ...
                $modal.on("shown" + evSuffix, function (ev) {
                  $("input[autofocus], textarea[autofocus]", $modal)
                    .first()
                    .trigger("focus");
                });
        ...
        return {
          ...
          link: function postLink(scope, iElement, iAttrs, controller) {
            ...
            $q.when($modal(options)).then(function onSuccess(modal) {
              iElement
                .attr("data-target", "#" + modal.attr("id"))
    ...
    function ($parse, $compile, $http, $timeout, $q, $templateCache) {
      ...
      return {
        ...
        link: function postLink(scope, element, attr, ctrl) {
          ...
          $q.when(
            options.content ||
              $templateCache.get(value) ||
              $http.get(value, { cache: true })
          ).then(function onSuccess(template) {
            ...
            popover.getPosition = function () {
              var r = $.fn.popover.Constructor.prototype.getPosition.apply(
                this,
                arguments
              );
            ...
            $timeout(function () {
              ...
              if (scope.panes.length && !active) {
                $panes
                  ...
                  .addClass("active" + (iAttrs.fade ? " in" : ""));
        ...
        link: function postLink(scope, element, attrs, controller) {
          ...
          if (controller) {
              ...
              if (controller.$viewValue) {
                if (type === "date") {
                  ...
                  if (timeRegExp.test(controller.$viewValue)) {
                    ...
                    var hours = !isNaN(parseInt(dateParts[0], 10))
                      ? parseInt(dateParts[0], 10)
                      : "0";
                    var minutes = !isNaN(parseInt(dateParts[1], 10))
                    ...
                    var modelRender =
                      renderedDate !== "Invalid Date"
                        ? pad(renderedDate.getHours()) +
                          ":" +
                          pad(renderedDate.getMinutes())
                        : "";
            ...
            controller.$parsers.unshift(function (viewValue) {
              if (
                !viewValue ||
                (type === "string" && timeRegExp.test(viewValue))
              ) {
                ...
                viewValue.toString() !== "Invalid Date"
              ) {
                controller.$setValidity("time", true);
                return viewValue;
              } else {
      ...
      return {
        ...
        link: function postLink(scope, element, attrs, ctrl) {
          ...
          tooltip.show = function () {
            var r = $.fn.tooltip.Constructor.prototype.show.apply(
              this,
              arguments
            );
            this.tip().data(dataPrefix + type, this);
