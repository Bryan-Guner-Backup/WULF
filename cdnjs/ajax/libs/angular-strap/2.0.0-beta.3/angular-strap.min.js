      ...
      .provider("$select", function () {
        ...
        this.$get = [
          ...
          function (a, d, e, f) {
            function g(d, e) {
              var g = {},
                h = angular.extend({}, b, e),
                i = h.controller;
              if (!i) throw "ngModelController required";
              g = f(d, h);
              var j = e.scope,
                k = g.$scope;
              (k.$matches = []),
                (k.$activeIndex = 0),
                (k.$isMultiple = h.multiple),
                ...
                (k.$isActive = function (a) {
                  return g.$isActive(a);
                }),
                (g.update = function (a) {
                  (k.$matches = a),
                    i.$modelValue && a.length
                      ? (k.$activeIndex =
                          h.multiple && angular.isArray(i.$modelValue)
                            ? i.$modelValue.map(function (a) {
                                return g.$getIndex(a);
                              })
                            : g.$getIndex(i.$modelValue))
                      : k.$activeIndex >= a.length &&
                        (k.$activeIndex = h.multiple ? [] : 0);
                }),
                (g.activate = function (a) {
                  return (
                    h.multiple
                      ? (k.$activeIndex.sort(),
                        g.$isActive(a)
                          ? k.$activeIndex.splice(k.$activeIndex.indexOf(a), 1)
                          : k.$activeIndex.push(a),
                        h.sort && k.$activeIndex.sort())
                      : (k.$activeIndex = a),
                    k.$activeIndex
                  );
                }),
                (g.select = function (a) {
                  var b = k.$matches[a].value;
                  g.activate(a),
                    h.multiple
                      ? i.$setViewValue(
                          k.$activeIndex.map(function (a) {
                            return k.$matches[a].value;
                          })
                        )
                      : i.$setViewValue(b),
                    i.$render(),
                    ...
                    h.multiple ||
                      ("focus" === h.trigger
                        ? d[0].blur()
                        : g.$isShown && g.hide()),
                    k.$emit("$select.select", b, a);
                }),
                (g.$isVisible = function () {
                  return h.minLength && i
                    ? k.$matches.length && i.$viewValue.length >= h.minLength
                    : k.$matches.length;
                }),
                (g.$isActive = function (a) {
                  return h.multiple
                    ? -1 !== k.$activeIndex.indexOf(a)
                    : k.$activeIndex === a;
                }),
                (g.$getIndex = function (a) {
                  var b = k.$matches.length,
                    c = b;
                  if (b) {
                    for (c = b; c-- && k.$matches[c].value !== a; );
                    if (!(0 > c)) return c;
                  }
                }),
                (g.$onElementMouseDown = function (b) {
                  a.document.activeElement === d[0] &&
                    (d[0].blur(), b.preventDefault(), b.stopPropagation());
                }),
                (g.$onClick = function () {
                  c && d[0].focus();
                }),
                ...
                (g.$onKeyDown = function (a) {
                  if (/(38|40|13)/.test(a.keyCode)) {
                    if (
                      ...
                      return g.select(k.$activeIndex);
                    38 === a.keyCode && k.$activeIndex > 0
                      ? k.$activeIndex--
                      : 40 === a.keyCode &&
                        k.$activeIndex < k.$matches.length - 1
                      ? k.$activeIndex++
                      : angular.isUndefined(k.$activeIndex) &&
                        (k.$activeIndex = 0),
                      k.$digest();
                  }
                });
              var l = g.init;
              g.init = function () {
                l(), d.on("click", g.$onClick);
              };
              var m = g.destroy;
              g.destroy = function () {
                m(), d.off("click", g.$onClick);
              };
              var n = g.show;
              g.show = function () {
                n(),
                  ...
                  setTimeout(function () {
                    d.on(c ? "touchstart" : "mousedown", g.$onElementMouseDown),
                      g.$element.on(
                        c ? "touchstart" : "mousedown",
                        g.$onMouseDown
                      ),
                      h.keyboard && d.on("keydown", g.$onKeyDown);
                  });
              };
              var o = g.hide;
              return (
                (g.hide = function () {
                  d.off(c ? "touchstart" : "mousedown", g.$onElementMouseDown),
                    g.$element.off(
                      c ? "touchstart" : "mousedown",
                      g.$onMouseDown
                    ),
                    h.keyboard && d.off("keydown", g.$onKeyDown),
                    o();
                }),
                g
              );
            }
            angular.element(a.document.body);
            return (g.defaults = b), g;
          },
        ];
      })
      .directive("bsSelect", [
