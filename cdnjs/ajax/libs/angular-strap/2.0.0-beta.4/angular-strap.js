...
(function (window, document, undefined) {
  ...
  angular
    .module("mgcrea.ngStrap.datepicker", ["mgcrea.ngStrap.tooltip"])
        ...
        function (
          ...
          function DatepickerFactory(element, controller, config) {
            ...
            scope.$toggleMode = function () {
              $datepicker.setMode(
                ($datepicker.$mode + 1) % $datepicker.$views.length
              );
            };
          ...
          function ($locale) {
            ...
            var DateParserFactory = function (options) {
              ...
              var regExpMap = {
                ...
                EEE:
                  "((?:" + $locale.DATETIME_FORMATS.SHORTDAY.join("|") + "))",
                dd: "((?:(?:[0-2]?[0-9]{1})|(?:[3][01]{1})))",
                d: "((?:(?:[0-2]?[0-9]{1})|(?:[3][01]{1})))",
                MMMM: "((?:" + $locale.DATETIME_FORMATS.MONTH.join("|") + "))",
                MMM:
                  "((?:" + $locale.DATETIME_FORMATS.SHORTMONTH.join("|") + "))",
                MM: "((?:[0]?[1-9]|[1][012]))",
                M: "((?:[0]?[1-9]|[1][012]))",
          ...
          link: function postLink(scope, element, attr, controller) {
            ...
            controller.$parsers.unshift(function (viewValue) {
              ...
              if (!parsedDate || isNaN(parsedDate.getTime())) {
                ...
                var isValid =
                  parsedDate.getTime() >= options.minDate &&
                  parsedDate.getTime() <= options.maxDate;
            ...
            controller.$formatters.push(function (modelValue) {
              controller.$dateValue = angular.isDate(modelValue)
                ? modelValue
                : new Date(modelValue);
              return controller.$dateValue;
            });
            controller.$render = function () {
              element.val(
                controller.$isEmpty(controller.$viewValue)
                  ? ""
                  : dateFilter(controller.$viewValue, options.dateFormat)
              );
            };
            scope.$on("$destroy", function () {
              datepicker.destroy();
        ...
        function ($locale, $sce, dateFilter) {
          return function (picker) {
            ...
            var weekDaysLabels = weekDaysMin
              .slice(options.weekStart)
              .concat(weekDaysMin.slice(0, options.weekStart));
            ...
            var views = [
              {
                ...
                update: function (date, force) {
                  if (
                    force ||
                    date.getUTCFullYear() !== viewDate.year ||
                    date.getUTCMonth() !== viewDate.month
                  ) {
                    angular.extend(viewDate, {
                ...
                build: function () {
                  ...
                  var firstDayOfMonth = new Date(
                    Date.UTC(viewDate.year, viewDate.month, 1)
                  );
                  var firstDate = new Date(
                    +firstDayOfMonth -
                      (firstDayOfMonth.getUTCDay() + 1 - options.weekStart) *
                        86400000
                  );
                  for (var i = 0; i < 35; i++) {
                ...
                isSelected: function (date) {
                  return (
                    date.getUTCFullYear() === picker.$date.getUTCFullYear() &&
                    date.getUTCMonth() === picker.$date.getUTCMonth() &&
                    date.getUTCDate() === picker.$date.getUTCDate()
                  );
                },
                isDisabled: function (date) {
                  return (
                    date.getTime() < options.minDate ||
              ...
              },
              {
                ...
                update: function (date) {
                  if (
                    parseInt(date.getUTCFullYear() / 20, 10) !==
                ...
                isDisabled: function (date) {
                  var lastDate = +new Date(
                    Date.UTC(date.getUTCFullYear(), 1, 0)
                  );
                  return (
                    lastDate < options.minDate ||
                    date.getTime() > options.maxDate
        ...
        function (
          ...
          function ModalFactory(config) {
            ...
            $modal.$promise = $q.when(
              $templateCache.get(options.template) ||
