        ...
        return {
          ...
          link: function postLink(scope, element, attr, transclusion) {
            ...
            angular.forEach(["keyboard", "html"], function (key) {
              if (angular.isDefined(attr[key]))
                options[key] = !falseValueRegExp.test(attr[key]);
            });

            if (angular.isDefined(attr.backdrop)) {
              options.backdrop = falseValueRegExp.test(attr.backdrop)
                ? false
                : attr.backdrop;
            }
      ...
      function ($window, $animate, $tab, $parse) {
        ...
        return {
          ...
          link: function postLink(scope, element, attrs, controllers) {
            ...
            if (ngModelCtrl) {
              console.warn(
                "Usage of ngModel is deprecated, please use bsActivePane instead!"
              );
        ...
        function ($window, $rootScope, $tooltip, $timeout) {
          ...
          function TypeaheadFactory(element, controller, config) {
            ...
            $typeahead.update = function (matches) {
              ...
              if (/^(bottom|bottom-left|bottom-right)$/.test(options.placement))
                return;
            ...
            $tooltip.$applyPlacement = function () {
              ...
              if (autoPlace) {
                ...
                if (
                  ...
                  elementPosition.bottom + tipHeight > containerPosition.bottom
                ) {
                  placement = originalPlacement.replace("bottom", "top");
                } else if (
                  originalPlacement.indexOf("top") >= 0 &&
                  elementPosition.top - tipHeight < containerPosition.top
                ) {
