...
(function (root, factory) {
  ...
  function $translateMessageFormatInterpolation(
    ...
    $translateInterpolator.interpolate = function (
      ...
      var interpolatedText = $cache.get(
        string + angular.toJson(interpolationParams)
      );
        ...
        for (var key in interpolationParams) {
          if (interpolationParams.hasOwnProperty(key)) {
            ...
            if (
              ...
              "" + number === interpolationParams[key]
            ) {
  ...
  $translateMessageFormatInterpolation.$inject = [
    "$translateSanitization",
    "$cacheFactory",
    "TRANSLATE_MF_INTERPOLATION_CACHE",
  ];

  $translateMessageFormatInterpolation.displayName =
    "$translateMessageFormatInterpolation";
