angular.module("pascalprecht.translate").provider("$translatePartialLoader", [
  function () {
    function Part(name) {
      this.name = name;
      this.isActive = true;
    ...
    Part.prototype.parseUrl = function (urlTemplate, targetLang) {
      return urlTemplate
        .replace(/\{part\}/g, this.name)
        .replace(/\{lang\}/g, targetLang);
    };
    Part.prototype.getTable = function (
      lang,
      $q,
      $http,
      urlTemplate,
    ...
    this.setPart = function (lang, part, table) {
      ...
      if (!isStringValid(part)) {
        throw new TypeError(
          "Invalid type of a second argument, a non-empty string expected."
        );
      }
      if (typeof table !== "object" || table === null) {
        throw new TypeError(
          "Invalid type of a third argument, an object expected."
        );
      }
      if (!hasPart(part)) {
      ...
      function ($rootScope, $injector, $q, $http) {
        var service = function (options) {
          ...
          if (!isStringValid(options.urlTemplate)) {
            throw new TypeError(
              "Unable to load data, a urlTemplate is not a non-empty string."
            );
          }
