angular.module("pascalprecht.translate", ["ng"]).run([
  ...
  function ($translate) {
    ...
    if (storage) {
      ...
      if (
        angular.isString($translate.fallbackLanguage()) &&
        $translate.fallbackLanguage() !== $translate.preferredLanguage()
      ) {
      ...
      function ($log, $injector, $rootScope, $q) {
        ...
        var $translate = function (
          ...
          if ($uses && $fallbackLanguage && $uses !== $fallbackLanguage) {
            var translation =
              $translationTable[$fallbackLanguage][translationId];
            if (translation) {
              var returnVal;
              Interpolator.setLocale($fallbackLanguage);
              returnVal = Interpolator.interpolate(
                translation,
                interpolateParams
              );
