      ...
      return {
        ...
        link: function (scope, element, attrs) {
          ...
          scope.$watch("numPages + currentPage + maxSize", function () {
            ...
            var maxSize =
              scope.maxSize && scope.maxSize < scope.numPages
                ? scope.maxSize
                : scope.numPages;
            ...
            if (startPage + maxSize - 1 > scope.numPages) {
              startPage =
                startPage - (startPage + maxSize - 1 - scope.numPages);
            }

            // Add page number links
            for (
              var number = startPage, max = startPage + maxSize;
              number < max;
              number++
            ) {
              var page = makePage(
...
angular
      ...
      return {
        ...
        link: function (originalScope, element, attrs, modelCtrl) {
          ...
          var getMatchesAsync = function (inputValue) {
            ...
            $q.when(parserResult.source(scope, locals)).then(function (
              matches
            ) {
  ...
  .filter("typeaheadHighlight", function () {
    return function (matchItem, query) {
      return query
        ? matchItem.replace(new RegExp(query, "gi"), "<strong>$&</strong>")
        : query;
    };
  });
