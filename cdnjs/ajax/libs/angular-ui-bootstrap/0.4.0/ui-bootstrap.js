      ...
      return {
        ...
        link: function (scope, element, attrs, ngModelCtrl) {
          ...
          ngModelCtrl.$render = function () {
            element.toggleClass(
              activeClass,
              angular.equals(ngModelCtrl.$modelValue, trueValue)
            );
          };
          ...
          element.bind(toggleEvent, function () {
            scope.$apply(function () {
              ngModelCtrl.$setViewValue(
                element.hasClass(activeClass) ? falseValue : trueValue
              );
              ngModelCtrl.$render();
            });
          });
      ...
      return {
        ...
        link: function (scope, element, attrs) {
          ...
          var selected = new Date(),
            ...
            maxDate,
            format = {};
          format.day = angular.isDefined(attrs.dayFormat)
            ? scope.$eval(attrs.dayFormat)
            : datepickerConfig.dayFormat;
          format.month = angular.isDefined(attrs.monthFormat)
            ? scope.$eval(attrs.monthFormat)
            : datepickerConfig.monthFormat;
          format.year = angular.isDefined(attrs.yearFormat)
            ? scope.$eval(attrs.yearFormat)
            : datepickerConfig.yearFormat;
          format.dayHeader = angular.isDefined(attrs.dayHeaderFormat)
            ? scope.$eval(attrs.dayHeaderFormat)
            : datepickerConfig.dayHeaderFormat;
          format.dayTitle = angular.isDefined(attrs.dayTitleFormat)
            ? scope.$eval(attrs.dayTitleFormat)
            : datepickerConfig.dayTitleFormat;
          format.monthTitle = angular.isDefined(attrs.monthTitleFormat)
            ? scope.$eval(attrs.monthTitleFormat)
            : datepickerConfig.monthTitleFormat;
          var startingDay = angular.isDefined(attrs.startingDay)
            ? scope.$eval(attrs.startingDay)
            : datepickerConfig.startingDay;
          var yearRange = angular.isDefined(attrs.yearRange)
            ? scope.$eval(attrs.yearRange)
          ...
          function isDisabled(date) {
            return (
              (minDate && compare(date, minDate) > 0) ||
              (maxDate && compare(date, maxDate) < 0) ||
              (scope.dateDisabled &&
                scope.dateDisabled({ date: date, mode: scope.mode }))
            );
          }
          ...
          var fill = {
            day: function () {
              ...
              addDays(
                lastDate || d,
                getDaysInMonth(selected.getFullYear(), selected.getMonth()),
                true
              );
              ...
              updateCalendar(
                split(days, 7),
                labels,
                dateFilter(selected, format.dayTitle)
              );
            },
            month: function () {
              var months = [],
              ...
              updateCalendar(
                split(months, 3),
                [],
                dateFilter(selected, format.monthTitle)
              );
            },
            year: function () {
              var years = [],
                year =
                  parseInt((selected.getFullYear() - 1) / yearRange, 10) *
                    yearRange +
                  1;
            ...
            if (scope.model && scope.model.getFullYear() === dt.getFullYear()) {
              ...
              if (scope.model.getMonth() === dt.getMonth()) {
                return (
                  scope.mode === "month" ||
                  (scope.mode === "day" &&
                    scope.model.getDate() === dt.getDate())
                );
              }
            }
            return false;
      ...
      return {
        ...
        link: function (scope, element, attrs) {
          ...
          var firstText = angular.isDefined(attrs.firstText)
            ? scope.$parent.$eval(attrs.firstText)
            : paginationConfig.firstText;
          var previousText = angular.isDefined(attrs.previousText)
            ? scope.$parent.$eval(attrs.previousText)
            : paginationConfig.previousText;
          var nextText = angular.isDefined(attrs.nextText)
            ? scope.$parent.$eval(attrs.nextText)
            : paginationConfig.nextText;
          var lastText = angular.isDefined(attrs.lastText)
            ? scope.$parent.$eval(attrs.lastText)
            : paginationConfig.lastText;
          var rotate = angular.isDefined(attrs.rotate)
            ? scope.$eval(attrs.rotate)
          ...
          scope.$watch("numPages + currentPage + maxSize", function () {
            ...
            var isMaxSized =
              angular.isDefined(scope.maxSize) &&
            ...
            if (isMaxSized) {
              if (rotate) {
                ...
                startPage =
                  (Math.ceil(scope.currentPage / scope.maxSize) - 1) *
                    scope.maxSize +
                  1;

                // Adjust last page if limit is exceeded
                endPage = Math.min(
                  startPage + scope.maxSize - 1,
                  scope.numPages
                );
              }
            }
      ...
      return {
        ...
        link: function (scope, element, attrs, paginationCtrl) {
          ...
          var nextText = angular.isDefined(attrs.nextText)
            ? scope.$parent.$eval(attrs.nextText)
            : config.nextText;
          var align = angular.isDefined(attrs.align)
          ...
          return {
            ...
            link: function link(scope, element, attrs) {
              ...
              function hide() {
                ...
                if (
                  angular.isDefined(scope.tt_animation) &&
                  scope.tt_animation()
                ) {
              ...
              attrs.$observe(prefix + "Animation", function (val) {
                scope.tt_animation = angular.isDefined(val)
                  ? $parse(val)
                  : function () {
...
angular
  ...
  .controller("ProgressBarController", [
    ...
    function ($scope, $attrs, progressConfig) {
      ...
      var animate = angular.isDefined($attrs.animate)
        ? $scope.$eval($attrs.animate)
        : progressConfig.animate;
      var autoType = angular.isDefined($attrs.autoType)
        ? $scope.$eval($attrs.autoType)
        : progressConfig.autoType;
      var stackedTypes = angular.isDefined($attrs.stackedTypes)
        ? $scope.$eval("[" + $attrs.stackedTypes + "]")
      ...
      this.makeBar = function (newBar, oldBar, index) {
        ...
        var type =
          angular.isObject(newBar) && angular.isDefined(newBar.type)
            ? newBar.type
            : autoType
            ? getStackedType(index || 0)
            : null;
          ...
          function (newValue, oldValue) {
            ...
            if (angular.isArray(newValue)) {
              ...
              for (var i = 0, n = newValue.length; i < n; i++) {
                controller.addBar(
                  controller.makeBar(newValue[i], oldValue[i], i)
                );
              }
            } else {
              // Simple bar
  ...
  .directive("rating", [
    ...
    function (ratingConfig, $parse) {
      return {
        ...
        replace: true,
        link: function (scope, element, attrs) {
          var maxRange = angular.isDefined(attrs.max)
            ? scope.$eval(attrs.max)
            : ratingConfig.max;
      ...
      return {
        ...
        link: function (scope, element, attrs, ngModelCtrl) {
          ...
          function refreshTemplate() {
            ...
            scope.minutes =
              keyboardChange === "m" ? minutes : padFilter(minutes);
            scope.validMinutes = true;

            scope.meridian = scope.showMeridian
      ...
      return {
        ...
        link: function (originalScope, element, attrs, modelCtrl) {
          ...
          modelCtrl.$render = function () {
            ...
            element.val(
              parserResult.viewMapper(scope, locals) || modelCtrl.$viewValue
            );
            selected = undefined;
          };
