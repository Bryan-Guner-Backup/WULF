          ...
          element.bind(toggleEvent, function () {
            scope.$apply(function () {
              ngModelCtrl.$setViewValue(
                element.hasClass(activeClass) ? getFalseValue() : getTrueValue()
              );
              ngModelCtrl.$render();
            });
          });
    ...
    function ($scope, $attrs, dateFilter, dtConfig) {
      ...
      this.isDisabled = function (date, mode) {
        ...
        return (
          ...
          ($scope.dateDisabled &&
            $scope.dateDisabled({ date: date, mode: currentMode.name }))
        );
      };
    },
  ])
    ...
    function (
      ...
      return {
        ...
        link: function (originalScope, element, attrs, ngModel) {
          var closeOnDateSelection = angular.isDefined(
            attrs.closeOnDateSelection
          )
            ? scope.$eval(attrs.closeOnDateSelection)
            : datepickerPopupConfig.closeOnDateSelection;
          var dateFormat =
            attrs.datepickerPopup || datepickerPopupConfig.dateFormat;
          ...
          var popupEl = angular.element(
            "<datepicker-popup-wrap><datepicker></datepicker></datepicker-popup-wrap>"
          );
          popupEl.attr({
            "ng-model": "date",
          ...
          if (attrs.datepickerOptions) {
            datepickerEl.attr(
              angular.extend({}, originalScope.$eval(attrs.datepickerOptions))
            );
          }
            ...
            function (value) {
              if (angular.isString(value)) {
                ...
                if (value && !date) {
                  ...
                  throw new Error(
                    value + " cannot be parsed to a date object."
                  );
                } else {
                  value = date;
                }
    ...
    function (config) {
      return {
        ...
        link: function (scope, element, attrs, paginationCtrl) {
          ...
          var boundaryLinks = paginationCtrl.getAttributeValue(
              attrs.boundaryLinks,
              config.boundaryLinks
            ),
            directionLinks = paginationCtrl.getAttributeValue(
              attrs.directionLinks,
              config.directionLinks
            ),
            firstText = paginationCtrl.getAttributeValue(
              attrs.firstText,
              config.firstText,
              true
            ),
            previousText = paginationCtrl.getAttributeValue(
              attrs.previousText,
              config.previousText,
              true
            ),
            nextText = paginationCtrl.getAttributeValue(
              attrs.nextText,
              config.nextText,
              true
            ),
            lastText = paginationCtrl.getAttributeValue(
              attrs.lastText,
              config.lastText,
              true
            ),
            rotate = paginationCtrl.getAttributeValue(
              attrs.rotate,
              config.rotate
            );
          ...
          scope.$watch("numPages + currentPage + maxSize", function () {
            ...
            if (isMaxSized) {
              if (rotate) {
                ...
                startPage = Math.max(
                  paginationCtrl.currentPage - Math.floor(scope.maxSize / 2),
                  1
                );
                ...
                startPage =
                  (Math.ceil(paginationCtrl.currentPage / scope.maxSize) - 1) *
                    scope.maxSize +
                  1;
            ...
            for (var number = startPage; number <= endPage; number++) {
              var page = makePage(
                number,
                number,
                paginationCtrl.isActive(number),
                false
              );
  ...
  .directive("pager", [
    ...
    function (config) {
      return {
        ...
        link: function (scope, element, attrs, paginationCtrl) {
          // Setup configuration parameters
          var previousText = paginationCtrl.getAttributeValue(
              attrs.previousText,
              config.previousText,
              true
            ),
      ...
      compile: function (elm, attrs, transclude) {
        return function (scope, element, attrs, tabsetCtrl) {
          ...
          scope.type = angular.isDefined(attrs.type)
            ...
            : "tabs";
          scope.direction = angular.isDefined(attrs.direction)
            ? scope.$parent.$eval(attrs.direction)
            : "top";
