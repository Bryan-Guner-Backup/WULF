...
(function (window, angular, undefined) {
  ...
  UrlMatcher.prototype.format = function (values) {
    ...
    for (; /**/ i < nTotal; i++) {
      ...
      if (value != null) {
        result +=
          (search ? "&" : "?") + params[i] + "=" + encodeURIComponent(value);
        search = true;
      }
    }
  ...
  function $StateProvider($urlRouterProvider, $urlMatcherFactory) {
    ...
    function registerState(state) {
      ...
      if (isString(url)) {
        ...
        isObject(url) &&
        isFunction(url.exec) &&
        isFunction(url.format) &&
        isFunction(url.concat)
      ) {
      ...
      if (parent) {
        forEach(parent.params, function (p) {
          if (!paramNames[p]) {
            throw new Error(
              "Missing required parameter '" + p + "' in state '" + name + "'"
            );
          }
          paramNames[p] = false;
        });
      ...
      var includes = (state.includes = parent
        ? extend({}, parent.includes)
        : {});
      includes[name] = true;
    ...
    function $get(
      ...
      $state.transitionTo = function transitionTo(
        to,
        toParams,
        updateLocation
      ) {
      ...
      function resolveState(state, params, paramsAreFiltered, inherited, dst) {
        ...
        forEach(state.views, function (view, name) {
          ...
          promises.push(
            $q
              .when(
                $templateFactory.fromConfig(view, $stateParams, locals) || ""
              )
              .then(function (result) {
  ...
  function $RouteProvider($stateProvider, $urlRouterProvider) {
    ...
    function $get($state, $rootScope, $routeParams) {
      ...
      $rootScope.$on(
        "$stateChangeError",
        function (ev, to, toParams, from, fromParams, error) {
          $rootScope.$broadcast(
            "$routeChangeError",
            stateAsRoute(to),
            stateAsRoute(from),
