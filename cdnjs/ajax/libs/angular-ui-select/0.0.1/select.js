...
angular
    ...
    function ($sce, uiSelectConfig) {
      ...
      return {
        ...
        templateUrl: function (tElement) {
          ...
          var theme =
            tElement.parent().attr("theme") || uiSelectConfig.defaultTheme;
        ...
        compile: function (tElement, tAttrs) {
          ...
          return function (scope, element, attrs, uiSelectCtrl) {
            ...
            var container = element.hasClass("ui-select-choices-content")
              ? element
              : element.querySelectorAll(".ui-select-choices-content");
              ...
              if (evt.which === 40) {
                ...
                if (scope.$select.activeIdx < rows.length) {
                  scope.$select.activeIdx =
                    (scope.$select.activeIdx + 1) % rows.length ||
                    rows.length - 1;
                  ensureHighlightVisible();
                  scope.$digest();
                }
                ...
                // enter(13) and tab(9)
                angular
                  .element(rows[scope.$select.activeIdx])
  ...
  .directive("match", [
    "$compile",
    "uiSelectConfig",
    function ($compile, uiSelectConfig) {
      return {
        restrict: "E",
        transclude: true,
        replace: true,
        ...
        link: function (scope, element, attrs) {
          scope.placeholder =
            attrs.placeholder || uiSelectConfig.defaultPlaceholder;
        },
      };
    },
  ])
...
angular.module("ui.select").run([
  ...
  function ($templateCache) {
    $templateCache.put(
      ...
      '<ul class="ui-select-choices ui-select-choices-content select2-results"> <li class="ui-select-choices-row" ng-class="{\'select2-highlighted\': $select.activeIdx==$index}"> <div class="select2-result-label" ng-transclude></div> </li> </ul> '
    );
    $templateCache.put(
      "select2/match.tpl.html",
      '<a class="select2-choice ui-select-match" ng-class="{\'select2-default\': $select.selected == undefined}" ng-click="uiSelectCtrl.activate($event)"> <span ng-hide="$select.selected" class="select2-chosen">{{placeholder}}</span> <span ng-show="$select.selected" class="select2-chosen" ng-transclude></span> <span class="select2-arrow"><b></b></span> </a> '
    );
    $templateCache.put(
      "select2/select.tpl.html",
      '<div class="select2 select2-container" ng-class="{\'select2-container-active select2-dropdown-open\': open}"> <div class="ui-select-match"></div> <div ng-class="{\'select2-display-none\': !open}" class="select2-drop select2-with-searchbox select2-drop-active"> <div class="select2-search"> <input type="text" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" class="ui-select-search select2-input" ng-model="$select.search"> </div> <div class="ui-select-choices"></div> </div> </div> '
    );
    $templateCache.put(
      "selectize/choices.tpl.html",
      '<div ng-show="open" class="ui-select-choices selectize-dropdown single"> <div class="ui-select-choices-content selectize-dropdown-content"> <div class="ui-select-choices-row" ng-class="{\'active\': $select.activeIdx==$index}" ng-click="$select(item)" ng-mouseenter="$select.index=$index"> <div class="option" data-selectable ng-transclude></div> </div> </div> </div> '
    );
    $templateCache.put(
    ...
    $templateCache.put(
      ...
      '<div class="selectize-control single"> <div class="selectize-input" ng-class="{\'focus\': open}" ng-click="uiSelectCtrl.activate($event)"> <div class="ui-select-match"></div> <input type="text" class="ui-select-search" autocomplete="off" tabindex="" placeholder="{{placeholder}}" ng-model="$select.search" ng-hide="$select.selected && !open"> </div> <div class="ui-select-choices"></div> </div> '
    );
  },
]);
