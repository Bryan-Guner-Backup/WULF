      ...
      function (uiSelectMinErr, $parse) {
        ...
        self.parse = function (expression) {
          var match = expression.match(
            /^\s*(?:([\s\S]+?)\s+as\s+)?([\s\S]+?)\s+in\s+([\s\S]+?)(?:\s+track\s+by\s+([\s\S]+?))?\s*$/
          );
        ...
        ctrl.parseRepeatAttr = function (repeatAttr, groupByExp) {
          function updateGroups(items) {
            ...
            angular.forEach(items, function (item) {
              ...
              var groupValue = angular.isFunction(groupFn)
                ? groupFn(item)
                : item[groupFn];
...
angular.module("ui.select").run([
  ...
  function ($templateCache) {
    $templateCache.put(
      ...
      '<ul class="ui-select-choices ui-select-choices-content dropdown-menu" role="menu" aria-labelledby="dLabel" ng-show="$select.items.length > 0"><li class="ui-select-choices-group"><div class="divider" ng-show="$index > 0"></div><div ng-show="$select.isGrouped" class="ui-select-choices-group-label dropdown-header">{{$group}}</div><div class="ui-select-choices-row" ng-class="{active: $select.isActive(this)}"><a href="javascript:void(0)" class="ui-select-choices-row-inner"></a></div></li></ul>'
    );
    $templateCache.put(
