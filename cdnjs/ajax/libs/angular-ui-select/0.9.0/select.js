        ...
        _searchInput.on("keyup", function (e) {
          ...
          if (ctrl.tagging.isActivated && ctrl.search.length > 0) {
            ...
            if (ctrl.tagging.fct !== undefined) {
              ...
              if (
                ctrl.selected.filter(function (selection) {
                  return selection.toUpperCase() === ctrl.search.toUpperCase();
                }).length > 0
              ) {
        ...
        return {
          ...
          link: function (scope, element, attrs, ctrls, transcludeFn) {
            ...
            $select.closeOnSelect =
              angular.isDefined(attrs.closeOnSelect) &&
              attrs.closeOnSelect.toLowerCase() === "false"
                ? false
                : uiSelectConfig.closeOnSelect;
...
angular.module("ui.select").run([
  ...
  function ($templateCache) {
    ...
    $templateCache.put(
      ...
      '<button type="button" class="btn btn-default form-control ui-select-match" tabindex="-1" ng-hide="$select.open" ng-disabled="$select.disabled" ng-class="{\'btn-default-focus\':$select.focus}" ;="" ng-click="$select.activate()"><span ng-show="$select.isEmpty()" class="text-muted">{{$select.placeholder}}</span> <span ng-hide="$select.isEmpty()" ng-transclude=""></span> <span class="caret ui-select-toggle" ng-click="$select.toggle($event)"></span></button>'
    );
    $templateCache.put(
