      ...
      link: function (scope, elm, attrs, ngModel) {
        ...
        deferCodeMirror = function () {
          ...
          for (var i = 0, n = events.length, aEvent; i < n; ++i) {
            aEvent =
              opts[
                "on" + events[i].charAt(0).toUpperCase() + events[i].slice(1)
              ];
    ...
    return function (mode, scope, elm, attrs) {
      ...
      elm.bind(mode, function (event) {
        ...
        angular.forEach(combinations, function (combination) {
          var mainKeyPressed =
            combination.keys[keysByCode[event.keyCode]] ||
            combination.keys[event.keyCode.toString()] ||
            false;
    ...
    return {
      ...
      link: function ($scope, element, attrs, controller) {
        ...
        controller.$parsers.push(function (value) {
          ...
          var isValid =
            element.isMaskValid() ||
            (angular.isUndefined(element.isMaskValid()) &&
        ...
        if (ngModel) {
          ...
          onStop = function (e, ui) {
            ...
            if (ui.item.sortable.resort && !ui.item.sortable.relocate) {
              ...
              ui.item.sortable.resort.$modelValue.splice(
                end,
                0,
                ui.item.sortable.resort.$modelValue.splice(start, 1)[0]
              );
            }
            if (ui.item.sortable.resort || ui.item.sortable.relocate) {
              scope.$apply();
