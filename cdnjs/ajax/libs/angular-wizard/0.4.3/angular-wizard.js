...
angular.module("mgo-angular-wizard").directive("wizard", function () {
  return {
    ...
    controller: [
      ...
      "$log",
      "WizardHandler",
      function ($scope, $element, $log, WizardHandler) {
        ...
        $scope.$watch("currentStep", function (step) {
          ...
          var stepTitle =
            $scope.selectedStep.title || $scope.selectedStep.wzTitle;
          if ($scope.selectedStep && stepTitle !== $scope.currentStep) {
            //invoking goTo() with step title as argument
            $scope.goTo(
              _.findWhere($scope.steps, { title: $scope.currentStep })
            );
          }
        });
        ...
        $scope.goTo = function (step) {
          ...
          if (firstRun) {
            ...
            if (
              typeof $scope.steps[thisStep].canexit === "undefined" ||
              $scope.steps[thisStep].canexit($scope.context) === true
            ) {
              exitallowed = true;
            }
            ...
            if (
              (exitallowed && step.canenter === undefined) ||
              (exitallowed && step.canenter($scope.context) === true)
            ) {
              enterallowed = true;
            }
...
function wizardButtonDirective(action) {
  angular.module("mgo-angular-wizard").directive(action, function () {
    return {
      restrict: "A",
      replace: false,
      require: "^wizard",
