...
angular.module("mgo-angular-wizard").directive("wizard", function () {
  return {
    ...
    controller: [
      ...
      "WizardHandler",
      "$q",
      function ($scope, $element, $log, WizardHandler, $q) {
        ...
        $scope.goTo = function (step) {
          ...
          if (firstRun) {
            ...
            $q.all([
              canExitStep($scope.steps[thisStep], step),
              canEnterStep(step),
            ]).then(function (data) {
              if (data[0] && data[1]) {
                //deselect all steps so you can set fresh below
        ...
        function canExitStep(step, stepTo) {
          ...
          if (
            typeof step.canexit === "undefined" ||
            $scope.getStepNumber(stepTo) < $scope.currentStepNumber()
          ) {
            return true;
          }
