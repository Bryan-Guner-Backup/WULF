 ...
 */
  ...
  Template.prototype = {
    link: function (element, scope) {
      ...
      if (this.newScope) {
        childScope = isFunction(this.newScope)
          ? scope.$new(this.newScope(scope))
          : scope.$new();
      ...
      if (directives) {
        ...
        eachAttribute(element, function (value, name) {
          ...
          if (fn) {
            ...
            template.addLinkFn(
              directiveFns[name].call(selfApi, value, element)
            );
          }
        });
      }
 ...
 */
   ...
   */
    $eval: function (expr) {
      var fn = isString(expr) ? expressionCompile(expr) : expr || noop;
      return fn(this);
    },
   ...
   */
    ...
    $broadcast: function (name, args) {
      var target = this,
        current = target,
        next = target,
        event = { name: name, targetScope: target },
        listenerArgs = concat([event], arguments, 1);
   ...
   */
    count: function (array, condition) {
      ...
      var fn = angularFunction.compile(condition),
        count = 0;
 ...
 */
  ...
  angularServiceInject(
    "$location",
    function ($browser, $sniffer, $config, $document) {
      var scope = this,
        currentUrl,
      ...
      if ($config.html5Mode) {
        if ($sniffer.history) {
          currentUrl = new LocationUrl(
            convertToHtml5Url(initUrl, basePath, hashPrefix),
            pathPrefix
          );
        } else {
          currentUrl = new LocationHashbangUrl(
        ...
        var u = currentUrl,
          absUrlPrefix =
            composeProtocolHostPort(u.protocol(), u.host(), u.port()) +
            pathPrefix;
        ...
        $document.bind("click", function (event) {
          ...
          if (
            uppercase(event.target.nodeName) != "A" ||
            event.ctrlKey ||
            event.which == 2
          )
            return;
          ...
          if (!href || isDefined(elm.attr("ng:ext-link")) || elm.attr("target"))
            return;
          ...
          href =
            href.indexOf(pathPrefix) === 0
              ? href.substr(pathPrefix.length)
              : href;
 ...
 */
  angularDirective("ng:controller", function (expression) {
    this.scope(function (scope) {
      var Controller =
        getter(scope, expression, true) || getter(window, expression, true);
 ...
 */
  angularDirective("ng:style", function (expression, element) {
    ...
    return function (element) {
      ...
      this.$watch(function (scope) {
        var style = scope.$eval(expression) || {},
          key,
          mergedStyle = {};
        for (key in style) {
    ...
    return function (iterStartElement) {
      ...
      this.$watch(function (scope) {
        var index = 0,
          ...
          fragment = document.createDocumentFragment(),
          addFragmentTo =
            childCount < collectionLength ? childElements[childCount] : null,
          childScope,
          key;
 ...
 */
  angularWidget("ng:pluralize", function (element) {
    ...
    return annotate("$locale", function ($locale, element) {
      ...
      forEach(whens, function (expression, key) {
        whensExpFns[key] = compileBindTemplate(
          expression.replace(/{}/g, "{{" + numberExp + "-" + offset + "}}")
        );
      });
  ...
  function browserTrigger(element, type) {
    ...
    if (msie < 9) {
      ...
      finalProcessDefault = !(
        appWindow.angular["ff-684208-preventDefault"] || !fakeProcessDefault
      );
    ...
    chain.click = function () {
      return this.addFutureAction(
        ...
        function ($window, $document, done) {
          ...
          if (
            href &&
            elements[0].nodeName.toUpperCase() === "A" &&
            eventProcessDefault
          ) {
