...
(function (I, h) {
  ...
  function z(a, d) {
    ...
    function g(a, b) {
      var c = 0,
        g;
      if ((b = h.lowercase(b)))
        for (c = e.length - 1; c >= 0; c--) if (e[c] == b) break;
      if (c >= 0) {
    ...
    for (
      ...
      if (!e.last() || !q[e.last()]) {
        if (a.indexOf("<!--") === 0)
          ...
          if ((b = a.match(r)))
            (a = a.substring(b[0].length)), b[0].replace(r, g), (f = !1);
        } else if (C.test(a) && (b = a.match(s)))
          (a = a.substring(b[0].length)), b[0].replace(s, c), (f = !1);
        f &&
          ((b = a.indexOf("<")),
          (f = b < 0 ? a : a.substring(0, b)),
          (a = b < 0 ? "" : a.substring(b)),
          d.chars && d.chars(k(f)));
      } else
        (a = a.replace(
  ...
  h.module("ngSanitize").directive("ngBindHtml", [
    "$sanitize",
    function (a) {
    ...
    return function (c, g) {
      ...
      for (; (b = f.match(a)); )
        (i = b[0]),
          b[2] == b[3] && (i = "mailto:" + i),
          (k = b.index),
          j.chars(f.substr(0, k)),
          (l.href = i),
          j.start("a", l),
          j.chars(b[0].replace(d, "")),
          j.end("a"),
          (f = f.substring(k + b[0].length));
      j.chars(f);
      return e.join("");
    };
  });
