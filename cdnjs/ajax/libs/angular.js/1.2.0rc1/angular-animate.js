    ...
    .config([
      ...
      */
            function performAnimation(
              ...
              if (
                (parent.inheritedData(NG_ANIMATE_STATE) || disabledAnimation)
                  .running
              ) {
              ...
              forEach(animations, function (animation, index) {
                ...
                if (animation.start) {
                  if (event == "addClass" || event == "removeClass") {
                    animation.endFn = animation.start(
                      element,
                      baseClassName,
                      fn
                    );
              ...
              function startAnimation() {
                ...
                forEach(element, function (element) {
                  if (element.nodeType == ELEMENT_NODE) {
                    ...
                    var transitionDelay = Math.max(
                      parseMaxTime(elementStyles[w3cTransitionProp + delayKey]),
                      parseMaxTime(
                        elementStyles[vendorTransitionProp + delayKey]
                      )
                    );

                    var animationDelay = Math.max(
                      parseMaxTime(elementStyles[w3cAnimationProp + delayKey]),
                      parseMaxTime(
                        elementStyles[vendorAnimationProp + delayKey]
                      )
                    );

                    var transitionDuration = Math.max(
                      parseMaxTime(
                        elementStyles[w3cTransitionProp + durationKey]
                      ),
                      parseMaxTime(
                        elementStyles[vendorTransitionProp + durationKey]
                      )
                    );
                    ...
                    if (animationDuration > 0) {
                      animationDuration *= Math.max(
                        parseInt(
                          elementStyles[
                            w3cAnimationProp + animationIterationCountKey
                          ]
                        ) || 0,
                        parseInt(
                          elementStyles[
                            vendorAnimationProp + animationIterationCountKey
                          ]
                        ) || 0,
                        1
                      );
