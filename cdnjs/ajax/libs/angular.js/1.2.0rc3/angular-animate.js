    ...
    .config([
      ...
      */
            ...
            function cancelChildAnimations(element) {
              angular.forEach(
                element[0].querySelectorAll("." + NG_ANIMATE_CLASS_NAME),
                function (element) {
        ...
        $animateProvider.register("", [
          ...
          function ($window, $sniffer, $timeout) {
                ...
                forEach(element, function (element) {
                  if (element.nodeType == ELEMENT_NODE) {
                    ...
                    transitionDuration = Math.max(
                      parseMaxTime(elementStyles[transitionProp + durationKey]),
                      transitionDuration
                    );

                    if (!onlyCheckTransition) {
                      transitionDelay = Math.max(
                        parseMaxTime(elementStyles[transitionProp + delayKey]),
                        transitionDelay
                      );

                      animationDelay = Math.max(
                        parseMaxTime(elementStyles[animationProp + delayKey]),
                        animationDelay
                      );

                      var aDuration = parseMaxTime(
                        elementStyles[animationProp + durationKey]
                      );
                      ...
                      if (aDuration > 0) {
                        aDuration *=
                          parseInt(
                            elementStyles[
                              animationProp + animationIterationCountKey
                            ]
                          ) || 1;
                      }
            ...
            function animate(element, className, done) {
              ...
              if (
                getElementAnimationDetails(element, cacheKey, true)
           ...
           that only has a delay and no duration */
              ...
              if (maxDuration > 0) {
                ...
                var css3AnimationEvents =
                  animationendEvent + " " + transitionendEvent;
